...
BOOL WTSVirtualChannelManagerCheckFileDescriptor(HANDLE hServer)
{
	...
	if ((vcm->drdynvc_state == DRDYNVC_STATE_NONE) && vcm->client->activated)
	{
		...
		if (channel)
		{
			...
			WLog_INFO(TAG, "WTSVirtualChannelManagerCheckFileDescriptor, DVC Capabilities Request PDU, channelId: %i", (int)vcm->drdynvc_channel->channelId);
...
BOOL WINAPI FreeRDP_WTSVirtualChannelClose(HANDLE hChannelHandle)
{
	...
	WLog_WARN(TAG, "FreeRDP_WTSVirtualChannelClose--- channel: %p", channel);
	if (channel)
	{
		vcm = channel->vcm;
		...
		{
			WLog_WARN(TAG, "FreeRDP_WTSVirtualChannelClose, RDP_PEER_CHANNEL_TYPE_SVC, index: %i, channelCount: %i", (int)channel->index, (int)mcs->channelCount);
			if (channel->index < mcs->channelCount)
				mcs->channels[channel->index].handle = NULL;
		}
		else
		{
			WLog_WARN(TAG, "FreeRDP_WTSVirtualChannelClose. not RDP_PEER_CHANNEL_TYPE_SVC");
			ArrayList_Remove(vcm->dynamicVirtualChannels, channel);
		...
		if (channel->receiveData) {
			WLog_WARN(TAG, "FreeRDP_WTSVirtualChannelClose, channel->receiveData: %p", channel->receiveData);
			Stream_Free(channel->receiveData, TRUE);
		}

		if (channel->queue) {
			WLog_WARN(TAG, "FreeRDP_WTSVirtualChannelClose, channel->queue: %p", channel->queue);
			MessageQueue_Free(channel->queue);
			channel->queue = NULL;
		}

		WLog_WARN(TAG, "FreeRDP_WTSVirtualChannelClose, free(channel: %p)", channel);
		free(channel);
	}

	WLog_WARN(TAG, "---FreeRDP_WTSVirtualChannelClose X");
	return ret;
}
                                           ...
                                           PULONG pBytesWritten)
{
	...
	if (channel->channelType == RDP_PEER_CHANNEL_TYPE_SVC)
	{
		...
		totalWritten = Length;
		if (Length == 4) {
			WLog_INFO(TAG, "FreeRDP_WTSVirtualChannelWrite, len(4), DVC Capabilities Request PDU, channelId: %i", (int)channel->channelId);
		}
