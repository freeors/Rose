...
#define TAG FREERDP_TAG("core.settings")
#include <util_c.h>
extern int rose_gethostname(char* name, int namelen);
...
rdpSettings* freerdp_settings_new(DWORD flags)
{
	...
	// gethostname2(settings->ClientHostname, 31);
	settings->ClientHostname[31] = 0;
	settings->OsMajorType = freerdp_os();
	settings->OsMinorType = OSMINORTYPE_UNSPECIFIED;
	WLog_INFO(TAG, "{dbg}freerdp_settings_new, post gethostname, ClientHostname: %s, settings: %p, OsMajorType: %u", settings->ClientHostname, settings, settings->OsMajorType);
	...
	if (!settings->ServerMode)
	{
		...
		// preferences is completely taken over by Rose, and HomePath/ConfigPath of freerdp is no longer used.
/*
