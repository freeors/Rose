...
BOOL rdp_client_connect_mcs(rdpRdp* rdp)
{
/*
	{
		int ii = 0;
		rdp_client_reset_codecs(rdp->context);
	}
*/
...
BOOL rdp_server_accept_nego(rdpRdp* rdp, wStream* s)
{
	...
	// By now, this client has been counted into peer->was_clients.
	if (peer->was_clients >= SUPPORTED_MAX_CLIENTS + 1) {
		SelectedProtocol = PROTOCOL_FAILED_NEGO;
		// Client hope to a separate error code to know this error occurred, 
		// but according to rdp-pdf can not find corresponding value, it is temporarily replaced with SSL_CERT_NOT_ON_SERVER.
		WLog_ERR(TAG, "so far, up to %i clients is supported", SUPPORTED_MAX_CLIENTS);
		SelectedProtocol |= SSL_CERT_NOT_ON_SERVER;

	} else if ((settings->NlaSecurity) && (RequestedProtocols & PROTOCOL_HYBRID))
	{
		// SelectedProtocol = PROTOCOL_HYBRID; // freerdp-rose-fix
	}
	...
	if (SelectedProtocol & PROTOCOL_HYBRID) {
		// status = transport_accept_nla(rdp->transport); // freerdp-rose-fix
...
BOOL rdp_server_accept_mcs_connect_initial(rdpRdp* rdp, wStream* s)
{
	...
	WLog_INFO(TAG, "Accepted client: %s, rdp->settings: %p", rdp->settings->ClientHostname, rdp->settings);
