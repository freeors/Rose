                      ...
                      UINT32* pDstSize, BYTE** ppAuxDstData, UINT32* pAuxDstSize)
{
...

	{
		...
		static uint32_t total_times = 0;
		static uint32_t last_report_times = 0;
		...
		total_ms += stop - start;
		total_times ++;
		if (total_times - last_report_times == 100) {
			WLog_INFO(TAG, "#%u avc444_compress, h264->subsystem->Compress[#1], average spend %u ms", total_times, total_ms / total_times);
			last_report_times = total_times;
		}
	}
...
/*
	{
		...
		total_times ++;
		if (total_times - last_report_times == 100) {
			WLog_INFO(TAG, "#%u avc444_compress, h264->subsystem->Compress[#2], average spend %u ms", total_times, total_ms / total_times);
			last_report_times = total_times;
		}
	}
	*ppAuxDstData = coded;
