	...
	for (i = 0; i < dvcman->num_listeners; i++)
	{
		...
		if (strcmp(listener->channel_name, ChannelName) == 0)
		{
			...
			if ((error = listener->listener_callback->OnNewChannelConnection(
			    ...
			    bAccept)
			{
				WLog_Print(drdynvc->log, WLOG_INFO, "listener %s created new channel %" PRIu32 "",
                                               ...
                                               wStream* s)
{
	...
	WLog_Print(drdynvc->log, WLOG_INFO, "capability_request Sp=%d cbChId=%d", Sp, cbChId);
	Stream_Seek(s, 1); /* pad */
...
static DWORD WINAPI drdynvc_virtual_channel_client_thread(LPVOID arg)
{
	...
	WLog_Print(drdynvc->log, WLOG_INFO, "drdynvc_virtual_channel_client_thread---");
	...
	WLog_Print(drdynvc->log, WLOG_INFO, "---drdynvc_virtual_channel_client_thread, X");
