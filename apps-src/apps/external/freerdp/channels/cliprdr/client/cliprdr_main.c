                                      ...
                                      UINT16 flags)
{
	...
	WLog_Print(cliprdr->log, WLOG_INFO, "ServerCapabilities");
...
static DWORD WINAPI cliprdr_virtual_channel_client_thread(LPVOID arg)
{
	...
	WLog_INFO(TAG, "cliprdr_virtual_channel_client_thread---");
	...
	WLog_INFO(TAG, "---cliprdr_virtual_channel_client_thread, X");
	ExitThread(error);
	return error;
}
