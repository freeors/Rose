...
typedef void (*did_rose_signal_update)(void* rose_delegate);

struct rdp_shadow_server
{
	void* ext;
	HANDLE thread; // change shadow_server_thread to win_shadow_subsystem_thread
	HANDLE StopEvent;
	wArrayList* clients;
...
extern "C"
{
	...
	int rose_shadow_server_init(rdpShadowServer* server);
	int rose_shadow_server_uninit(rdpShadowServer* server);
