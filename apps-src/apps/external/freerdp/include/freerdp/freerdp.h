...
#define freerdp_set_last_error_log(context, lastError) \
	...
	typedef SSIZE_T (*did_rose_read_layer)(rdpContext* context, BYTE* data, size_t bytes);
	typedef int (*did_rose_write_layer)(rdpContext* context, BYTE* data, size_t bytes);
	FREERDP_API void rose_register_extra(rdpContext* context, did_rose_read_layer read_layer, did_rose_write_layer write_layer, void* delegate, void* connection);
	FREERDP_API BOOL rose_send_suppress_output(rdpContext* context, BOOL suppress);

	FREERDP_API void did_h264_frame(void* custom, const uint8_t* data, size_t len);
#ifdef __cplusplus
}
#endif
