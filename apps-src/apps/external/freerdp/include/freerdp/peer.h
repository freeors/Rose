...
// typedef SSIZE_T (*did_rose_read_layer)(freerdp_peer* client, BYTE* data, size_t bytes);
// typedef int (*did_rose_write_layer)(freerdp_peer* client, BYTE* data, size_t bytes);
...
struct rdp_freerdp_peer
{
	...
	int sockfd12; // freerdp-rose-fix. socket is handled by chromium, in order to find error, remark it.
	char hostname[50];
	...
	// did_rose_read_layer rose_read_layer;
	// did_rose_write_layer rose_write_layer;
	// void* rose_delegate;
	// void* rose_connection;
	int was_clients; // client when it's OnConnect
};
