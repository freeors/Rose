...
static sspi_gss_OID_desc g_SSPI_GSS_C_NT_USER_NAME = {
	10, (void*)"\x2a\x86\x48\x86\xf7\x12\x01\x02\x01\x01"
};
static sspi_gss_OID_desc g_SSPI_GSS_C_NT_MACHINE_UID_NAME = {
	10, (void*)"\x2a\x86\x48\x86\xf7\x12\x01\x02\x01\x02"
};
static sspi_gss_OID_desc g_SSPI_GSS_C_NT_STRING_UID_NAME = {
	10, (void*)"\x2a\x86\x48\x86\xf7\x12\x01\x02\x01\x03"
};
static sspi_gss_OID_desc g_SSPI_GSS_C_NT_HOSTBASED_SERVICE_X = {
	6, (void*)"\x2b\x06\x01\x05\x06\x02"
};
static sspi_gss_OID_desc g_SSPI_GSS_C_NT_HOSTBASED_SERVICE = {
...
UINT32 SSPI_GSSAPI sspi_gss_init_sec_context(
    ...
    sspi_gss_buffer_t input_token, sspi_gss_OID* actual_mech_type, sspi_gss_buffer_t output_token,
    UINT32* ret_flags, UINT32* time_rec)
{
	SECURITY_STATUS status;
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, NULL, NULL);
	...
	WLog_DBG(TAG, "gss_init_sec_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_accept_sec_context(
    ...
    sspi_gss_OID* mech_type, sspi_gss_buffer_t output_token, UINT32* ret_flags, UINT32* time_rec,
    sspi_gss_cred_id_t* delegated_cred_handle)
{
	...
	status = g_GssApi->gss_accept_sec_context(
	    ...
	    src_name, mech_type, output_token, ret_flags, time_rec, delegated_cred_handle);
	WLog_DBG(TAG, "gss_accept_sec_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}
                                    ...
                                    sspi_gss_buffer_t message_token)
{
	...
	WLog_DBG(TAG, "gss_get_mic: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_verify_mic(UINT32* minor_status, sspi_gss_ctx_id_t context_handle,
                                       ...
                                       sspi_gss_buffer_t message_token, sspi_gss_qop_t* qop_state)
{
	SECURITY_STATUS status;
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, NULL, NULL);
	...
	status = g_GssApi->gss_verify_mic(minor_status, context_handle, message_buffer, message_token,
	                                  qop_state);
	WLog_DBG(TAG, "gss_verify_mic: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                   ...
                                   sspi_gss_qop_t* qop_state)
{
	...
	WLog_DBG(TAG, "gss_unwrap: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                           ...
                                           UINT32* message_context, sspi_gss_buffer_t status_string)
{
	...
	WLog_DBG(TAG, "gss_display_status: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}
                                         ...
                                         sspi_gss_OID* output_name_type)
{
	...
	WLog_DBG(TAG, "gss_display_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}

UINT32 SSPI_GSSAPI sspi_gss_import_name(UINT32* minor_status, sspi_gss_buffer_t input_name_buffer,
                                        sspi_gss_OID input_name_type, sspi_gss_name_t* output_name)
{
	SECURITY_STATUS status;
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, NULL, NULL);
	...
	WLog_DBG(TAG, "gss_import_name: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                         ...
                                         sspi_gss_OID_set* mechanisms)
{
	...
	status = g_GssApi->gss_inquire_cred(minor_status, cred_handle, name, lifetime, cred_usage,
	                                    mechanisms);
	WLog_DBG(TAG, "gss_inquire_cred: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}

UINT32 SSPI_GSSAPI sspi_gss_inquire_context(UINT32* minor_status, sspi_gss_ctx_id_t context_handle,
                                            sspi_gss_name_t* src_name, sspi_gss_name_t* targ_name,
                                            UINT32* lifetime_rec, sspi_gss_OID* mech_type,
                                            UINT32* ctx_flags, int* locally_initiated, int* open)
{
	...
	WLog_DBG(TAG, "gss_inquire_context: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}

UINT32 SSPI_GSSAPI sspi_gss_wrap_size_limit(UINT32* minor_status, sspi_gss_ctx_id_t context_handle,
                                            int conf_req_flag, sspi_gss_qop_t qop_req,
                                            UINT32 req_output_size, UINT32* max_input_size)
{
	SECURITY_STATUS status;
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, NULL, NULL);
	...
	WLog_DBG(TAG, "gss_wrap_size_limit: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}
                                               ...
                                               sspi_gss_name_t* output_name)
{
	...
	WLog_DBG(TAG, "gss_import_name_object: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}

UINT32 SSPI_GSSAPI sspi_gss_export_name_object(UINT32* minor_status, sspi_gss_name_t input_name,
                                               sspi_gss_OID desired_name_type, void** output_name)
{
	SECURITY_STATUS status;
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, NULL, NULL);
	...
	WLog_DBG(TAG, "gss_export_name_object: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_add_cred(UINT32* minor_status, sspi_gss_cred_id_t input_cred_handle,
                                     ...
                                     sspi_gss_cred_usage_t cred_usage, UINT32 initiator_time_req,
                                     ...
                                     UINT32* acceptor_time_rec)
{
	...
	status = g_GssApi->gss_add_cred(
	    ...
	    acceptor_time_req, output_cred_handle, actual_mechs, initiator_time_rec, acceptor_time_rec);
	WLog_DBG(TAG, "gss_add_cred: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                                 ...
                                                 sspi_gss_cred_usage_t* cred_usage)
{
	...
	WLog_DBG(TAG, "gss_inquire_cred_by_mech: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}
                                 ...
                                 sspi_gss_buffer_t message_token)
{
	...
	WLog_DBG(TAG, "gss_sign: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}

UINT32 SSPI_GSSAPI sspi_gss_verify(UINT32* minor_status, sspi_gss_ctx_id_t context_handle,
                                   sspi_gss_buffer_t message_buffer, sspi_gss_buffer_t token_buffer,
                                   int* qop_state)
{
	SECURITY_STATUS status;
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, NULL, NULL);
	...
	WLog_DBG(TAG, "gss_verify: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                 ...
                                 sspi_gss_buffer_t output_message_buffer)
{
	...
	status = g_GssApi->gss_seal(minor_status, context_handle, conf_req_flag, qop_req,
	                            input_message_buffer, conf_state, output_message_buffer);
	WLog_DBG(TAG, "gss_seal: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
                                   ...
                                   int* qop_state)
{
	...
	status = g_GssApi->gss_unseal(minor_status, context_handle, input_message_buffer,
	                              output_message_buffer, conf_state, qop_state);
	WLog_DBG(TAG, "gss_unseal: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
...
UINT32 SSPI_GSSAPI sspi_gss_duplicate_name(UINT32* minor_status, const sspi_gss_name_t input_name,
...
UINT32 SSPI_GSSAPI sspi_gss_pseudo_random(UINT32* minor_status, sspi_gss_ctx_id_t context,
                                          ...
                                          SSIZE_T desired_output_len, sspi_gss_buffer_t prf_out)
{
	SECURITY_STATUS status;
	InitOnceExecuteOnce(&g_Initialized, sspi_GssApiInit, NULL, NULL);
	...
	status = g_GssApi->gss_pseudo_random(minor_status, context, prf_key, prf_in, desired_output_len,
	                                     prf_out);
	WLog_DBG(TAG, "gss_pseudo_random: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status),
	         status);
	return status;
}
                                       ...
                                       sspi_gss_cred_usage_t* cred_usage_stored)
{
	...
	WLog_DBG(TAG, "gss_store_cred: %s (0x%08" PRIX32 ")", GetSecurityStatusString(status), status);
	return status;
}
