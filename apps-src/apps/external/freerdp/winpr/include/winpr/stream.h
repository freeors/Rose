...
#define Stream_Peek_INT64_BE(_s, _v) _stream_read_n64_be(INT64, _s, _v, FALSE)
	...
	{
		if ((size_t)(_s->pointer - _s->buffer + 1) > _s->capacity) {
			int ii = 0;
		}
		*_s->pointer++ = (UINT8)(_v);
	}
	...
	{
		if ((size_t)(_s->pointer - _s->buffer + 2) > _s->capacity) {
			int ii = 0;
		}
		*_s->pointer++ = (_v)&0xFF;
	...
	{
		if ((size_t)(_s->pointer - _s->buffer + 4) > _s->capacity) {
			int ii = 0;
		}
		*_s->pointer++ = (_v)&0xFF;
		*_s->pointer++ = ((_v) >> 8) & 0xFF;
	...
	{
		if ((size_t)(_s->pointer - _s->buffer + 6) > _s->capacity) {
			int ii = 0;
		}
	...
	static INLINE void Stream_Write(wStream* _s, const void* _b, size_t _n)
	{
		if (_n > 0)
		{
			if ((size_t)(_s->pointer - _s->buffer + _n) > _s->capacity) {
				int ii = 0;
			}
