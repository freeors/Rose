...
static void rose_write_file(const char* filename, const uint8_t* data, int size)
{
...
/*
	{
		static int times = 0;
		if (times == 0) {
			rose_write_file("c:/ddksample/pixels.dat", pDstData, nDstStep * surface->height);
		}
		times ++;
		int ii = 0;
		return 1;
...
static int win_shadow_subsystem_start(rdpShadowSubsystem* arg)
{
	...
	if (!(subsystem->base.server->thread = CreateThread(NULL, 0, win_shadow_subsystem_thread, (void*)subsystem, 0, NULL)))
	{
