...
static BOOL shadow_client_context_new(freerdp_peer* peer, rdpShadowClient* client)
{
	...
	if (server->CertificateFile != NULL && !(settings->CertificateFile = _strdup(server->CertificateFile)))
		goto fail_cert_file;

	if (server->PrivateKeyFile != NULL  && !(settings->PrivateKeyFile = _strdup(server->PrivateKeyFile)))
		goto fail_privkey_file;

	if (settings->PrivateKeyFile != NULL  && !(settings->RdpKeyFile = _strdup(settings->PrivateKeyFile)))
...
static BOOL shadow_client_suppress_output(rdpShadowClient* client, BYTE allow, RECTANGLE_16* area)
{
	...
	client->suppressOutputTicks = SDL_GetTicks();
...
*/
		{
			int ii = 0;
			avc444.LC = 1;
			avc444.bitstream[0].data = pSrcData;
			avc444.bitstream[0].length = nSrcStep;
...
BOOL can_xmit_screen_surface(rdpShadowServer* server, freerdp_peer* peer, SHADOW_GFX_STATUS* pgfxstatus)
{
	rdpShadowClient* client = (rdpShadowClient*)peer->context;
	rdpContext* context = (rdpContext*)client;
	return client->activated && !client->suppressOutput && context->settings->GfxH264 && pgfxstatus->gfxOpened;
}

void* rose_did_shadow_peer_connect(rdpShadowServer* server, freerdp_peer* peer, SHADOW_GFX_STATUS* pgfxstatus)
{
	rdpShadowClient* client;
...
void rose_did_update_peer_send(rdpShadowServer* server, freerdp_peer* peer, SHADOW_GFX_STATUS* pgfxstatus, void* UpdateSubscriber)
{
	rdpShadowClient* client = (rdpShadowClient*)peer->context;
	...
	{
		if (server->surface->h264Length != 0) {
			...
			rdpContext* context = (rdpContext*)client;
			if (client->activated && !client->suppressOutput && context->settings->GfxH264 && pgfxstatus->gfxOpened)
			{
				...
				if (shadow_client_recalc_desktop_size(client))
				{
					...
					if (!shadow_client_send_resize(client, pgfxstatus))
					{
				...
				{
					/* Send frame */
					if (!shadow_client_send_surface_update(client, pgfxstatus))
					{
			...
			else
			{
				...
				if (!shadow_client_no_surface_update(client, pgfxstatus))
				{
			...
			if (WTSVirtualChannelManagerIsChannelJoined(client->vcm, "drdynvc"))
			{
				switch (WTSVirtualChannelManagerGetDrdynvcState(client->vcm))
				{
					...
					case DRDYNVC_STATE_NONE:
						...
						WLog_INFO(TAG, "rose_did_update_peer_send, DRDYNVC_STATE_NONE, send DVC Capabilities Request PDU, activated: %s", client->activated? "true": "false");
...
void rose_did_shadow_peer_disconnect(rdpShadowServer* server, freerdp_peer* peer, SHADOW_GFX_STATUS* pgfxstatus, void* UpdateSubscriber)
{
	rdpShadowClient* client = (rdpShadowClient*)peer->context;
	rdpShadowSubsystem* subsystem = server->subsystem;

	{
		WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 1");
		int ii = 0;
	}
	...
	if (client->audin && !IFCALLRESULT(TRUE, client->audin->IsOpen, client->audin))
	{
		{
			int ii = 0;
			WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 2");
		}
	...
	if (pgfxstatus->gfxOpened)
	{
		if (pgfxstatus->gfxSurfaceCreated)
		{
			if (!shadow_client_rdpgfx_release_surface(client))
		...
		{
			WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 3");
			int ii = 0;
		}
		(void)client->rdpgfx->Close(client->rdpgfx);
		pgfxstatus->gfxOpened = FALSE;
	}

	{
		WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 4");
		int ii = 0;
	}
	shadow_client_channels_free(client);

	{
		WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 5");
		int ii = 0;
	}
	if (UpdateSubscriber)
	{
		{
			WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 6");
			int ii = 0;
		}
	...
	{
		{
			WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 7");
			int ii = 0;
		}
...
// out:
	{
		WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 8");
		int ii = 0;
	}
	peer->Disconnect(peer);
	{
		WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 9");
		int ii = 0;
	}
	freerdp_peer_context_free(peer);
	freerdp_peer_free(peer);
	{
		WLog_WARN(TAG, "rose_did_shadow_peer_disconnect, 10");
		int ii = 0;
	}
}
