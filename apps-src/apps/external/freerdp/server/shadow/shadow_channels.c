...
UINT shadow_client_channels_post_connect(rdpShadowClient* client)
{
	...
	{
		shadow_client_cliprdr_init(client);
	}

	if (WTSVirtualChannelManagerIsChannelJoined(client->vcm, LEAGOR_SVC_CHANNEL_NAME))
	{
		shadow_client_leagor_init(client);
	}
...
void shadow_client_channels_free(rdpShadowClient* client)
{
	{
		int ii = 0;
		SDL_Log("shadow_client_channels_free---");
	}
	if (client->context.settings->SupportGraphicsPipeline)
	{
	...
	{
		int ii = 0;
		SDL_Log("shadow_client_channels_free 1");
	}
	...
	{
		int ii = 0;
		SDL_Log("shadow_client_channels_free 2");
	}

	shadow_client_cliprdr_uninit(client);

	shadow_client_leagor_uninit(client);

	{
		int ii = 0;
		SDL_Log("shadow_client_channels_free 3");
	}
	shadow_client_rdpsnd_uninit(client);

	{
		int ii = 0;
		SDL_Log("shadow_client_channels_free 4");
	}
	shadow_client_remdesk_uninit(client);

	{
		int ii = 0;
		SDL_Log("shadow_client_channels_free 5");
	}
	shadow_client_encomsp_uninit(client);

	{
		int ii = 0;
		SDL_Log("---shadow_client_channels_free X");
	}
}
