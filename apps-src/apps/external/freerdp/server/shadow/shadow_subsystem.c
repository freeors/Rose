...
{
	shadow_multiclient_publish_and_wait(subsystem->updateEvent, subsystem->server);
}
...
rdpShadowServer* rose_init_subsystem()
{
	...
	rose_shadow_server_init(server);
	shadow_server_start(server);
	shadow_subsystem_start(server->subsystem);
	return server;
}

void rose_release_subsystem(rdpShadowServer* server)
{
	...
	rose_shadow_server_uninit(server);
