...
void URLRequest::StartJob(std::unique_ptr<URLRequestJob> job) {
  ...
  net_log_.BeginEvent(NetLogEventType::URL_REQUEST_START_JOB, [&] {
    return NetLogURLRequestStartParams(
        ...
        isolation_info_.network_isolation_key(), initiator_,
...
bool URLRequest::CanGetCookies() const {
  ...
  if (network_delegate()) {
    can_get_cookies =
        network_delegate()->CanGetCookies(*this, true);
  }
...
URLRequest* URLRequest::http_instance = nullptr;
void URLRequest::NotifySendBodyCompleted(int sofar_bytes, int expected_bytes)
{
	delegate_->OnSendBodyCompleted(this, sofar_bytes, expected_bytes);
}
