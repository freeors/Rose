...
RoseURLRequestContext::RoseURLRequestContext(const std::string& cert)
    : context_storage_(this)
    , cert_(cert) {
}

RoseURLRequestContext::~RoseURLRequestContext() {
  DCHECK(initialized_);
  AssertNoURLRequests();
}

void RoseURLRequestContext::Init() {
  DCHECK(!initialized_);
  initialized_ = true;
  ...
  if (!host_resolver()) {
    ...
    context_storage_.set_host_resolver(HostResolver::CreateStandaloneResolver(nullptr));
  }
  ...
  if (!cert_verifier()) {
    context_storage_.set_cert_verifier(
        CertVerifier::CreateDefault(nullptr));
  }
  ...
  if (!ssl_config_service()) {
    std::unique_ptr<SSLConfigService> ssl_config;
    if (cert_.empty()) {
      ssl_config.reset(new SSLConfigServiceDefaults());
    } else {
      ssl_config.reset(new SSLConfigServiceDefaults());
      // ssl_config.reset(new SSLConfigServiceBindCert(cert_));
    }
    context_storage_.set_ssl_config_service(std::move(ssl_config));
...
std::unique_ptr<URLRequest> RoseURLRequestContext::CreateFirstPartyRequest(
    const GURL& url,
...
/*
RoseURLRequestContextGetter::RoseURLRequestContextGetter(
...
RoseURLRequestContextGetter::RoseURLRequestContextGetter(
    const scoped_refptr<base::SingleThreadTaskRunner>& network_task_runner,
    std::unique_ptr<RoseURLRequestContext> context)
    : network_task_runner_(network_task_runner), context_(std::move(context)) {
  DCHECK(network_task_runner_.get());
}

RoseURLRequestContextGetter::~RoseURLRequestContextGetter() = default;

RoseURLRequestContext* RoseURLRequestContextGetter::GetURLRequestContext() {
  if (is_shut_down_)
    return nullptr;

  if (!context_.get())
    context_.reset(new RoseURLRequestContext);
  return context_.get();
}

void RoseURLRequestContextGetter::NotifyContextShuttingDown() {
...
RoseURLRequestContextGetter::GetNetworkTaskRunner() const {
  return network_task_runner_;
}
*/
const int RoseDelegate::kBufferSize;

RoseDelegate::RoseDelegate()
    : executor_(gui2::tprogress_::instance)
    , buf_(base::MakeRefCounted<IOBuffer>(kBufferSize)) {}

RoseDelegate::~RoseDelegate() = default;

void RoseDelegate::RunUntilComplete() {
...
void RoseDelegate::RunUntilRedirect() {
  use_legacy_on_complete_ = false;
  base::RunLoop run_loop;
...
void RoseDelegate::RunUntilAuthRequired() {
  use_legacy_on_complete_ = false;
  base::RunLoop run_loop;
...
int RoseDelegate::OnConnected(URLRequest* request, const TransportInfo& info) {
  transports_.push_back(info);
  return on_connected_result_;
}

void RoseDelegate::OnReceivedRedirect(URLRequest* request,
...
void RoseDelegate::OnAuthRequired(URLRequest* request,
...
void RoseDelegate::OnSSLCertificateError(URLRequest* request,
                                         int net_error,
...
void RoseDelegate::OnReadCompleted(URLRequest* request, int bytes_read) {
  ...
  if (bytes_read >= 0) {
    ...
    if (executor_.progress && expected_size > 0) {
      executor_.OnReadCompleted();
	  executor_.progress->set_percentage(100.0 * received_bytes_count_ / expected_size);
	  executor_.progress->set_message(format_i64size(received_bytes_count_) + "/" + format_i64size(expected_size));
	}
...
void RoseDelegate::OnResponseCompleted(URLRequest* request) {
  response_completed_ = true;
  if (use_legacy_on_complete_) {
    VALIDATE(false, "task of show_slice conitnue post to queue, Not chance to execute DoIdleWork");
    base::RunLoop::QuitCurrentWhenIdleDeprecated();
  } else {
    std::move(on_complete_).Run();
  }
...
void RoseDelegate::OnSendBodyCompleted(URLRequest* request, int sofar_bytes, int expected_bytes)
{
	VALIDATE(expected_bytes > 0, null_str);
	if (executor_.progress && expected_bytes > 0) {
		executor_.progress->set_percentage(100.0 * sofar_bytes / expected_bytes);
		executor_.progress->set_message(format_i64size(sofar_bytes) + "/" + format_i64size(expected_bytes));
	}
}
