...
// class Backend; <= std::string
// class Entry;   <= std::string
// class EntryResult; <= net::Error
}  // namespace disk_cache
...
class NET_EXPORT HttpCache : public HttpTransactionFactory {
 public:
  ...
  int GetBackend(std::string** backend,
 ...
 private:
  ...
  struct NET_EXPORT_PRIVATE ActiveEntry {
    ActiveEntry(const std::string& key, bool opened_in);
    ~ActiveEntry();
    ...
    std::unique_ptr<char> writers; // must be null always.
  ...
  int CreateBackend(std::string** backend,
                    CompletionOnceCallback callback);
  ...
  // cache entry.
  ActiveEntry* ActivateEntry(const std::string& key, bool opened);
  ...
  std::unique_ptr<std::string> disk_cache_;
