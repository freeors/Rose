...
int HttpCache::DefaultBackend::CreateBackend(
    NetLog* net_log,
    std::unique_ptr<std::string>* backend,
    CompletionOnceCallback callback) {
  DCHECK_GE(max_bytes_, 0);
  *backend = std::make_unique<std::string>("fake MemBackendImpl");
  return net::OK;
/*
...
HttpCache::ActiveEntry::ActiveEntry(const std::string& key, bool opened_in)
    : cache_key(key), opened(opened_in) {}
...
struct HttpCache::PendingOp {
  PendingOp()
      : cache_key(), entry_opened(false), callback_will_delete(false) {}
  ~PendingOp() = default;
...
class HttpCache::WorkItem {
 public:
  ...
  void NotifyTransaction(int result, ActiveEntry* entry) {
    DCHECK(!entry || !entry->cache_key.empty());
    if (entry_)
      *entry_ = entry;
    if (transaction_)
  ...
  bool DoCallback(int result, std::string* backend) {
    if (backend_)
      *backend_ = backend;
...
int HttpCache::GetBackend(std::string** backend,
                          CompletionOnceCallback callback) {
...
std::string* HttpCache::GetCurrentBackend() const {
...
int HttpCache::CreateBackend(std::string** backend,
                             CompletionOnceCallback callback) {
...
*/
  rv = ERR_FAILED; // reference to MemBackendImpl::DoomEntry
  if (rv == ERR_IO_PENDING) {
    pending_op->callback_will_delete = true;
    return rv;
  }
...
HttpCache::ActiveEntry* HttpCache::ActivateEntry(const std::string& cache_key,
                                                 bool opened) {
  DCHECK(!FindActiveEntry(cache_key));
  ActiveEntry* entry = new ActiveEntry(cache_key, opened);
  active_entries_[cache_key] = base::WrapUnique(entry);
  return entry;
}

void HttpCache::DeactivateEntry(ActiveEntry* entry) {
  DCHECK(!entry->doomed);
  DCHECK(!entry->cache_key.empty());
  DCHECK(entry->SafeToDestroy());

  std::string key = entry->cache_key;
  if (key.empty())
...
void HttpCache::DeletePendingOp(PendingOp* pending_op) {
  std::string key;
  if (!pending_op->cache_key.empty())
    key = pending_op->cache_key;
...
int HttpCache::OpenOrCreateEntry(const std::string& key,
  ...
  if (rv != OK)
    return rv;
/*
  net::Error entry_result = disk_cache_->OpenOrCreateEntry(
      key, transaction->priority(),
      base::BindOnce(&HttpCache::OnPendingCreationOpComplete, GetWeakPtr(),
                     pending_op));
  rv = entry_result;
*/
  rv = ERR_FAILED; // reference to MemBackendImpl::OpenEntry
  if (rv == ERR_IO_PENDING) {
    pending_op->callback_will_delete = true;
    return ERR_IO_PENDING;
  }
...
int HttpCache::OpenEntry(const std::string& key,
  ...
  if (rv != OK)
    return rv;
/*
  net::Error entry_result = disk_cache_->OpenEntry(
      key, transaction->priority(),
      base::BindOnce(&HttpCache::OnPendingCreationOpComplete, GetWeakPtr(),
                     pending_op));
  rv = entry_result;
...
int HttpCache::CreateEntry(const std::string& key,
  ...
  if (rv != OK)
    return rv;
/*
  net::Error entry_result = disk_cache_->CreateEntry(
      key, transaction->priority(),
      base::BindOnce(&HttpCache::OnPendingCreationOpComplete, GetWeakPtr(),
                     pending_op));
  rv = entry_result;
*/
  pending_op->cache_key = key;
  rv = OK; // // reference to MemBackendImpl::CreateEntry
  if (rv == ERR_IO_PENDING) {
    pending_op->callback_will_delete = true;
    return ERR_IO_PENDING;
  }
...
void HttpCache::WritersDoomEntryRestartTransactions(ActiveEntry* entry) {
  DCHECK(false); // don't use disk_cache, so should not enter it always;
}
...
void HttpCache::DoomEntryValidationNoMatch(ActiveEntry* entry) {
  ...
  DoomActiveEntry(entry->cache_key);
...
void HttpCache::AddTransactionToWriters(
    ...
    Transaction* transaction,
    ParallelWritingPattern parallel_writing_pattern) {
  DCHECK(false); // should not enter it.
}
...
void HttpCache::OnIOComplete(int result, PendingOp* pending_op) {
  ...
  if (result == OK) {
    if (op == WI_DOOM_ENTRY) {
      ...
      key = pending_op->cache_key;
      entry = ActivateEntry(pending_op->cache_key, pending_op->entry_opened);
    } else {
/*
...
void HttpCache::OnPendingCreationOpComplete(
    ...
    PendingOp* pending_op,
    net::Error result) {
  CHECK(false); // don't use disk_cache, so should not enter it always;
}
