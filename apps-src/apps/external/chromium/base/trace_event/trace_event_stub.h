...
class BASE_EXPORT MemoryDumpProvider2 {
 public:
  MemoryDumpProvider2(const MemoryDumpProvider2&) = delete;
  MemoryDumpProvider2& operator=(const MemoryDumpProvider2&) = delete;
  virtual ~MemoryDumpProvider2();

  virtual bool OnMemoryDump(const MemoryDumpArgs& args,
                            ProcessMemoryDump* pmd) = 0;

 protected:
  MemoryDumpProvider2() = default;
};
