...
class BASE_EXPORT ThreadControllerWithMessagePumpImpl
 ...
 public:
  ...
  void set_runloop_quit_with_delete_tasks(bool with_delete_tasks) override {
    runloop_quit_with_delete_tasks_ = with_delete_tasks;
	if (with_delete_tasks) {
	  // once generate nested-runloop, will result very-difficult BUG! Must nest.
      // RunLoop::DisallowNestingOnCurrentThread();
	}
  }

  void SetRequireDeletePendingTasks(base::WaitableEvent& e) override {
      DCHECK(!e.IsSignaled());
      delete_pending_tasks_e_= &e;
  }

  void DeletePendingTasksEx(bool force) override;
...
protected:
  bool runloop_quit_with_delete_tasks_;
  base::WaitableEvent* delete_pending_tasks_e_;
};
