...
void TaskQueueImpl::DeletePendingTasks() {
    {
        size_t s1 = main_thread_only().delayed_work_queue->Size();
        size_t s2 = main_thread_only().immediate_work_queue->Size();
        size_t s3 = main_thread_only().delayed_incoming_queue.size();
        size_t s4 = any_thread_.immediate_incoming_queue.size();
        int ii = 0;
    }
