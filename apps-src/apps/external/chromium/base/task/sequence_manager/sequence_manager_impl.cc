...
void SequenceManagerImpl::set_runloop_quit_with_delete_tasks(bool with_delete_tasks)
{
    controller_->set_runloop_quit_with_delete_tasks(with_delete_tasks);
}

void SequenceManagerImpl::SetRequireDeletePendingTasks(base::WaitableEvent& e)
{
    DCHECK(!main_thread_only().task_execution_stack.empty())
      << "SetRequireDeletePendingTasks should be called inside RunLoop's task. if outside, call DeletePendingTasks directly";
    controller_->SetRequireDeletePendingTasks(e);
}
