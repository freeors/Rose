...
Optional<TimeDelta> TimeDelta::FromString(StringPiece duration_string) {
  ...
  while (!duration_string.empty()) {
    ...
    if (number.frac_part != 0)
      delta += (double{1.0 * sign} * number.frac_part / number.frac_scale) * unit;
  }
  return delta;
}
...
double TimeDelta::InSecondsF() const {
  if (!is_inf())
    return double{1.0* delta_} / Time::kMicrosecondsPerSecond;
...
double TimeDelta::InMillisecondsF() const {
  if (!is_inf())
    return double{1.0 * delta_} / Time::kMicrosecondsPerMillisecond;
  return (delta_ < 0) ? -std::numeric_limits<double>::infinity()
                      : std::numeric_limits<double>::infinity();
}
...
Time Time::FromTimeSpec(const timespec& ts) {
  return FromDoubleT(ts.tv_sec + double{1.0 * ts.tv_nsec} / kNanosecondsPerSecond);
}
#endif
