...
WeChatQRCode::WeChatQRCode(const char* detector_prototxt_data, size_t detector_prototxt_len,
                         const char* detector_caffe_model_data, size_t detector_caffe_model_len,
                         const char* super_resolution_prototxt_data, size_t super_resolution_prototxt_len,
                         const char* super_resolution_caffe_model_data, size_t super_resolution_caffe_model_len) {
    p = makePtr<WeChatQRCode::Impl>();
    if (detector_caffe_model_data != nullptr && detector_prototxt_data != nullptr) {
        // initialize detector model (caffe)
        p->use_nn_detector_ = true;
        CV_Assert(detector_prototxt_len != 0);
        CV_Assert(detector_caffe_model_len != 0);
        p->detector_ = make_shared<SSDDetector>();
        auto ret = p->detector_->init(detector_prototxt_data, detector_prototxt_len, detector_caffe_model_data, detector_caffe_model_len);
        CV_Assert(ret == 0);
    } else {
    ...
    if (super_resolution_prototxt_data != nullptr && super_resolution_caffe_model_data != nullptr) {
        p->use_nn_sr_ = true;
        // initialize dnn model (caffe format)
        CV_Assert(super_resolution_prototxt_len != 0);
        CV_Assert(super_resolution_caffe_model_len != 0);
        auto ret = p->super_resolution_model_->init(super_resolution_prototxt_data, super_resolution_prototxt_len,
                                                    super_resolution_caffe_model_data, super_resolution_caffe_model_len);
        CV_Assert(ret == 0);
    } else {
        p->use_nn_sr_ = false;
    }
