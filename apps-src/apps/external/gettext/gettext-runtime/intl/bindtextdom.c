...
static void
set_binding_values (const char *domainname, const char **dirnamep, const char **codesetp)
{
...
void unbindtextdomain(const char* domainname)
{
	struct binding* binding;
	...
	for (binding = _nl_domain_bindings; binding != NULL; binding = binding->next) {
		...
		if (compare == 0) {
			...
			if (prev_binding != NULL) {
				...
				_nl_domain_bindings = binding->next;
			}
			break;
		}
		if (compare < 0) {
	...
	if (binding != NULL) {
		...
		if (binding->codeset != _nl_default_dirname) {
			free(binding->codeset);
		}
		free (binding);
	}
