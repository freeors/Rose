...
bool TranslateMediaTypeToVideoCaptureCapability(
  ...
  RTC_LOG(LS_INFO) << "TranslateMediaTypeToVideoCaptureCapability width:"
                   ...
                   << " Compression:0x" << std::hex << bih->biCompression;
  ...
  } else if (sub_type == MEDIASUBTYPE_NV12 && // Fix: FaceTime HD Camera (Built-in)[macbookpro15,2]
      ...
      : capability_(capability),
        format_preference_order_(
            {// Default preferences, sorted by cost-to-convert-to-i420.
             ...
             VideoType::kUYVY, VideoType::kMJPEG, VideoType::kNV12}) { // Fix: FaceTime HD Camera (Built-in)[macbookpro15,2]
  ...
  static void SetMediaInfoFromVideoType(VideoType video_type,
    ...
    switch (video_type) {
      ...
      case VideoType::kMJPEG:
        ...
        break;
      case VideoType::kNV12: // Fix: FaceTime HD Camera (Built-in)[macbookpro15,2]
        bitmap_header->biCompression = MAKEFOURCC('N', 'V', '1', '2');
        bitmap_header->biBitCount = 12;  // bit per pixel
