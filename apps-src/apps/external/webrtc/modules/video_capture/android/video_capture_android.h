...
public:
  ...
  class JavaVideoCapturer {
   public:
    ...
    bool Init();
    void Close();
    bool StartCapture(int width, int height, int fps, int id);
  ...
  explicit VideoCaptureAndroid(int at);
...
private:
	...
	void verbose_fps(int width, int height, int rotation);
...
private:
	...
	uint32_t verbose_startup_ms_;
	uint32_t verbose_last_ms_;
	int verbose_frames_;
};
