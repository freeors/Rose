...
VideoCaptureAndroid::JavaVideoCapturer::JavaVideoCapturer(
    ...
    : audio_manager_(std::move(audio_manager)),
      ...
      start_capture_(native_reg->GetMethodId("startCapture", "(IIII)Z")) {
...
bool VideoCaptureAndroid::JavaVideoCapturer::StartCapture(int width,
                                                          int height,
                                                          int fps,
                                                          int id) {
...
VideoCaptureAndroid::VideoCaptureAndroid(int at)
    : VideoCaptureImpl()
	, at_(at)
	, is_capturing_(false)
	, initialized_(false)
	, verbose_startup_ms_(0)
	, verbose_last_ms_(0)
	, verbose_frames_(0)
{
...
rtc::scoped_refptr<VideoCaptureModule> VideoCaptureAndroid::Create(const char* deviceUniqueIdUTF8) 
{
  ...
  int id = -1;
  for (std::vector<std::string>::iterator it = DeviceInfoAndroid::device_names.begin(); it != DeviceInfoAndroid::device_names.end(); ++ it) {
	  const std::string& name = *it;
	  if (!strcmp(name.c_str(), deviceUniqueIdUTF8)) {
		  id = std::distance(DeviceInfoAndroid::device_names.begin(), it);
		  break;
      }
  }
  ALOGD("Create%s, deviceUniqueIdUTF8: %s, id: %i", GetThreadInfo().c_str(), deviceUniqueIdUTF8, id);

  rtc::scoped_refptr<VideoCaptureAndroid> capture_module(
      new rtc::RefCountedObject<VideoCaptureAndroid>(id));
...
int32_t VideoCaptureAndroid::StartCapture(
  ...
  if (!InitCaptureAndroidJNI() ||
      !j_video_capturer_->StartCapture(capability.width, capability.height,
                                       capability.maxFPS, at_)) {
    UninitCaptureAndroidJNI();
    ALOGD("StartCapture%s, (%i x %i x %i) Fail", GetThreadInfo().c_str(),
...
struct tnative_registration {
  tnative_registration() : refcount(0) {}

  void set(std::unique_ptr<NativeRegistration> _reg)
  {
	  ALOGD("tnative_registration::set, E, _reg: %p, reg: %p, refcount: %i", _reg.get(), reg.get(), refcount);

    RTC_CHECK(_reg.get() && !reg.get() && refcount == 0);
    reg = std::move(_reg);
	refcount++;
  }
  ...
  void AddRef()
  {
	  ALOGD("tnative_registration::AddRef, E, reg: %p, refcount: %i", reg.get(), refcount);

	  RTC_CHECK(reg.get() && refcount >= 1);
	  refcount ++;
  }

  void Release()
  {
	  ALOGD("tnative_registration::Release, E, reg: %p, refcount: %i", reg.get(), refcount);

	  if (!reg.get()) {
		  ALOGD("tnative_registration::Release, X, ref is nullptr, do nothing");
		  RTC_CHECK(refcount == 0);
		  return;
	  }

	  RTC_CHECK(refcount >= 1);
      if (refcount == 1) {
		  ALOGD("refcount is 1, reg.reset()");
        reg.reset();
      } else {
		  ALOGD("refcount(%i) isn't 1, don't reg.reset()", refcount);
      }
      refcount --;
  }
...
static tnative_registration native_registration;

bool VideoCaptureAndroid::InitCaptureAndroidJNI() {
  attach_thread_if_needed_.reset(new AttachCurrentThreadIfNeeded);
  std::unique_ptr<JNIEnvironment> j_environment_ =
      JVM::GetInstance()->environment();

  ALOGD("InitCaptureAndroidJNI%s", GetThreadInfo().c_str());
  RTC_CHECK(j_environment_);

  if (native_registration.refcount == 0) {
    JNINativeMethod native_methods[] = {
        {"nativeCapturerStarted", "(JZ)V",
         reinterpret_cast<void*>(&VideoCaptureAndroid::CapturerStarted)},
        {"nativeCapturerStopped", "(J)V",
         reinterpret_cast<void*>(&VideoCaptureAndroid::CapturerStopped)},
        {"nativeOnByteBufferFrameCaptured", "(J[BIIIIJ)V",
         reinterpret_cast<void*>(
             &VideoCaptureAndroid::OnByteBufferFrameCaptured)},
        {"nativeOnTextureFrameCaptured", "(JIII[FIJ)V",
         reinterpret_cast<void*>(&VideoCaptureAndroid::OnTextureFrameCaptured)},
        {"nativeCreateSurfaceTextureHelper", "(JLorg/webrtc/EglBase$Context;)V",
         reinterpret_cast<void*>(
             &VideoCaptureAndroid::CreateSurfaceTextureHelper)},
        {"nativeInitializeVideoCapturer",
         "(JLorg/webrtc/VideoCapturer;Lorg/webrtc/"
         "VideoCapturer$CapturerObserver;)V",
         reinterpret_cast<void*>(
             &VideoCaptureAndroid::InitializeVideoCapturer)}};

    std::unique_ptr<NativeRegistration> j_native_registration = j_environment_->RegisterNatives(
        "org/webrtc/videoengine/WebRtcVideoCapturer", native_methods,
        arraysize(native_methods));
    native_registration.set(std::move(j_native_registration));

  } else {
	native_registration.AddRef();
  }

  j_video_capturer_.reset(
      new JavaVideoCapturer(native_registration.reg.get(),
                            native_registration.reg->NewObject(
...
void JNICALL VideoCaptureAndroid::CapturerStarted(JNIEnv* env,
                                                  jobject obj,
                                                  jlong j_source,
                                                  jboolean success) {
  ALOGD("CapturerStarted%s, %s", GetThreadInfo().c_str(),
        success ? "Success" : "Fail");
}

void JNICALL VideoCaptureAndroid::CapturerStopped(JNIEnv* env,
                                                  jobject obj,
                                                  jlong j_source) {
...
void JNICALL VideoCaptureAndroid::OnByteBufferFrameCaptured(JNIEnv* jni,
                                                            jclass,
                                                            jlong j_source,
                                                            jbyteArray data,
                                                            jint length,
                                                            ...
                                                            jlong timestamp) 
{
  ...
  VideoCaptureAndroid* this_object =
      reinterpret_cast<VideoCaptureAndroid*>(j_source);

  const jni::JavaParamRef<jbyteArray>& j_frame =
      jni::JavaParamRef<jbyteArray>(jni, data);

  jbyte* bytes = jni->GetByteArrayElements(j_frame.obj(), nullptr);
  this_object->onByteBufferFrameCaptured(
      bytes, length, width, height, jintToVideoRotation(rotation), timestamp);
  jni->ReleaseByteArrayElements(j_frame.obj(), bytes, JNI_ABORT);
}

void JNICALL
VideoCaptureAndroid::OnTextureFrameCaptured(JNIEnv* jni,
                                            jobject obj,
                                            jlong j_source,
                                            jint j_width,
                                            jint j_height,
                                            ...
                                            jint j_rotation,
                                            jlong j_timestamp) {
  // ALOGD("OnTextureFrameCaptured%s, (%dx%d)", GetThreadInfo().c_str(),
  // j_width, j_height);
  VideoCaptureAndroid* this_object =
      reinterpret_cast<VideoCaptureAndroid*>(j_source);

  this_object->onTextureFrameCaptured(
      j_width, j_height, j_rotation, j_timestamp,
      jni::NativeHandleImpl(
          jni, j_oes_texture_id,
          jni::JavaParamRef<jfloatArray>(j_transform_matrix)));
}

void JNICALL
VideoCaptureAndroid::CreateSurfaceTextureHelper(JNIEnv* env,
                                                jobject obj,
                                                jlong j_source,
                                                jobject j_egl_context) {
  // ALOGD("CreateSurfaceTextureHelper%s, j_source(0x%lx)",
  // GetThreadInfo().c_str(), j_source);
  ...
  this_object->set_surface_texture_helper(jni::SurfaceTextureHelper::create(
      env, "Camera SurfaceTextureHelper",
      jni::JavaParamRef<jobject>(j_egl_context)));
}

void JNICALL
VideoCaptureAndroid::InitializeVideoCapturer(JNIEnv* jni,
                                             jclass,
                                             jlong j_source,
...
void VideoCaptureAndroid::set_surface_texture_helper(
    const rtc::scoped_refptr<jni::SurfaceTextureHelper>& helper) {
  ALOGD("set_surface_texture_helper, this: %p, helper: %p", this, helper.get());
  surface_texture_helper_ = helper;
}

void VideoCaptureAndroid::initializeVideoCapturer(JNIEnv* jni,
                                                  jobject j_video_capturer,
                                                  jobject j_frame_observer) {
  ALOGD("initializeVideoCapturer%s", GetThreadInfo().c_str());
  ...
  jobject context =
      jni->CallStaticObjectMethod(context_utils, getApplicationContext_method);
  ...
  jni->CallVoidMethod(
      ...
      surface_texture_helper_->GetJavaSurfaceTextureHelper().obj(), context,
      j_frame_observer);
...
void VideoCaptureAndroid::onByteBufferFrameCaptured(const void* frame_data,
  ...
  // verbose_fps(width, height, rotation);
...
void VideoCaptureAndroid::onTextureFrameCaptured(
    int width,
    int height,
    int rotation,
    int64_t timestamp_ns,
    const jni::NativeHandleImpl& handle) {
  // RTC_DCHECK(camera_thread_checker_.CalledOnValidThread());
  // verbose_fps(width, height, rotation);
  ...
  // ALOGD("onTextureFrameCaptured#1, (%dx%d), do_rotate: %s, rotation: %d",
  // adapted_width, adapted_height, do_rotate? "true": "false", rotation);
  ...
  // ALOGD("onTextureFrameCaptured#2, (%dx%d)", converted_frame.width(),
  // converted_frame.height());

  // rtc::scoped_refptr<VideoFrameBuffer>
  // converted_buffer(converted_frame.video_frame_buffer()->NativeToI420Buffer());
  ...
  // ALOGD("onTextureFrameCaptured#3, (%dx%d)", converted_frame.width(),
  // converted_frame.height());
...
void VideoCaptureAndroid::verbose_fps(int width, int height, int rotation)
{
	verbose_frames_ ++;
	uint32_t now = SDL_GetTicks();
    if (verbose_startup_ms_ == 0) {
		verbose_startup_ms_ = now;
		verbose_last_ms_ = verbose_startup_ms_;
    }
    if (now - verbose_last_ms_ >= 1000) {
		verbose_last_ms_ = now;
		uint32_t elapsed_second = (now - verbose_startup_ms_) / 1000;
		SDL_Log("[VideoCaptureAndroid]#%u, %u fps, %ix%i, rotation(%i)", 
			elapsed_second, verbose_frames_ / elapsed_second, width, height, rotation);
    }
}