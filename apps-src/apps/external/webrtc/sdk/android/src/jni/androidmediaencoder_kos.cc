...
// NOTICE: androidmediaencoder_kos.h must be included before
...
public:
	...
	MediaCodecVideoEncoder(const cricket::VideoCodec& codec);
	...
	bool FillInputBuffer(int input_buffer_index,
...
private:
	...
	bool MaybeReconfigureEncoder(const VideoFrame& frame);

	// Returns true if the frame is a texture frame and we should use surface
	// based encoding.
	bool IsTextureFrame(const VideoFrame& frame);

	bool EncodeByteBuffer(bool key_frame,
						const VideoFrame& frame,
						int input_buffer_index);

	// Deliver any outputs pending in the MediaCodec to our |callback_| and return
	// true on success.
	bool DeliverPendingOutputs();
	...
	MediaCodecVideoEncoder_l j_media_codec_video_encoder_;
...
MediaCodecVideoEncoder::MediaCodecVideoEncoder(const cricket::VideoCodec& codec)
    : codec_(codec),
      callback_(NULL),
      inited_(false),
      use_surface_(false),
...
bool MediaCodecVideoEncoder::EncodeTask::Run() {
  ...
  encoder_->DeliverPendingOutputs();
                                                   ...
                                                   bool use_surface)
{
	...
	RTC_CHECK(!use_surface) << "EGL context not set.";
	...
	const bool encode_status = j_media_codec_video_encoder_.initEncode(codec_type, profile_, width,
		height, kbps, fps);
	if (!encode_status) {
	...
	if (!use_surface) {
		switch (j_media_codec_video_encoder_.getColorFormat()) {
		...
		const int buffers = j_media_codec_video_encoder_.getInputBufferLength();
		RTC_CHECK(buffers > 0);

		kosABuffer abuff;
		for (int i = 0; i < buffers; i ++) {
			j_media_codec_video_encoder_.getInputBuffer(i, abuff);
			int64_t yuv_buffer_capacity = abuff.mCapacity;
    ...
    const std::vector<FrameType>* frame_types)
{
	...
	if (codec_mode_ == kRealtimeVideo) {
		...
		if (last_frame_received_ms_ != -1 && (now_ms - last_frame_received_ms_) > kFrameDiffThresholdMs) {
	...
	if (!use_surface_) {
		j_input_buffer_index = j_media_codec_video_encoder_.dequeueInputBuffer();
	...
	RTC_CHECK(input_frame.video_frame_buffer()->type() != VideoFrameBuffer::Type::kNative);
	encode_status = EncodeByteBuffer(key_frame, input_frame, j_input_buffer_index);
...
bool MediaCodecVideoEncoder::MaybeReconfigureEncoder(const VideoFrame& frame)
{
  RTC_DCHECK_CALLED_SEQUENTIALLY(&encoder_queue_checker_);

  bool is_texture = IsTextureFrame(frame);
...
bool MediaCodecVideoEncoder::IsTextureFrame(const VideoFrame& frame)
{
	RTC_CHECK(frame.video_frame_buffer()->type() != VideoFrameBuffer::Type::kNative);
	return false;
}

bool MediaCodecVideoEncoder::EncodeByteBuffer(bool key_frame,
                                              const VideoFrame& frame,
                                              int input_buffer_index)
{
	RTC_DCHECK_CALLED_SEQUENTIALLY(&encoder_queue_checker_);
	...
	if (!FillInputBuffer(input_buffer_index, i420_buffer->DataY(),
						i420_buffer->StrideY(), i420_buffer->DataU(),
						i420_buffer->StrideU(), i420_buffer->DataV(),
						i420_buffer->StrideV())) {
		return false;
	}
	bool encode_status = j_media_codec_video_encoder_.encodeBuffer(
		key_frame, input_buffer_index, yuv_size_, current_timestamp_us_);
	return encode_status;
}

bool MediaCodecVideoEncoder::FillInputBuffer(int input_buffer_index,
                                             uint8_t const* buffer_y,
                                             int stride_y,
                                             uint8_t const* buffer_u,
                                             int stride_u,
...
{
	kosABuffer abuff;
	j_media_codec_video_encoder_.getInputBuffer(input_buffer_index, abuff);
	uint8_t* yuv_buffer = abuff.mData;
...
int32_t MediaCodecVideoEncoder::Release()
{
  ...
  j_media_codec_video_encoder_.release();
...
int32_t MediaCodecVideoEncoder::SetRateAllocation(
    const BitrateAllocation& rate_allocation, uint32_t frame_rate)
{
	RTC_DCHECK_CALLED_SEQUENTIALLY(&encoder_queue_checker_);
	...
	bool ret = j_media_codec_video_encoder_.setRates(last_set_bitrate_kbps_, last_set_fps_);
	if (!ret) {
		ProcessHWError(true /* reset_if_fallback_unavailable */);
...
bool MediaCodecVideoEncoder::DeliverPendingOutputs()
{
	RTC_DCHECK_CALLED_SEQUENTIALLY(&encoder_queue_checker_);

	kosABuffer abuff;
	while (true) {
		MediaCodecVideoEncoder_l::OutputBufferInfo j_output_buffer_info =
			j_media_codec_video_encoder_.dequeueOutputBuffer();
		if (!j_output_buffer_info.valid()) {
			break;
		}

		int output_buffer_index = j_output_buffer_info.index;
		...
		j_media_codec_video_encoder_.getOutputBuffer(output_buffer_index, abuff);
		bool key_frame = j_output_buffer_info.isKeyFrame;
		...
		last_output_timestamp_ms_ = j_output_buffer_info.presentationTimestampUs / rtc::kNumMicrosecsPerMillisec;
		...
		// Extract payload.
		size_t payload_size = abuff.mCapacity;
		uint8_t* payload = abuff.mData;

		// Callback - return encoded frame.
		const VideoCodecType codec_type = GetCodecType();
		EncodedImageCallback::Result callback_result(EncodedImageCallback::Result::OK);
		if (callback_) {
		...
		bool success = j_media_codec_video_encoder_.releaseOutputBuffer(output_buffer_index);
		if (!success) {
			ProcessHWError(true /* reset_if_fallback_unavailable */);
			return false;
		}
...
VideoEncoder::ScalingSettings MediaCodecVideoEncoder::GetScalingSettings() const 
{
...
MediaCodecVideoEncoderFactory::MediaCodecVideoEncoderFactory()
{
	...
	bool is_vp8_hw_supported = MediaCodecVideoEncoder_l::isVp8HwSupported();
	if (is_vp8_hw_supported) {
		ALOGD << "VP8 HW Encoder supported.";
		supported_codecs_.push_back(cricket::VideoCodec(cricket::kVp8CodecName));
	}

	bool is_vp9_hw_supported = MediaCodecVideoEncoder_l::isVp9HwSupported();
	...
	bool is_h264_high_profile_hw_supported = MediaCodecVideoDecoderFactory::IsH264HighProfileSupported();
	...
	bool is_h264_hw_supported = MediaCodecVideoEncoder_l::isH264HwSupported();
...
VideoEncoder* MediaCodecVideoEncoderFactory::CreateVideoEncoder(const cricket::VideoCodec& codec)
{
  ...
  if (FindMatchingCodec(supported_codecs(), codec)) {
    ...
    return new MediaCodecVideoEncoder(codec);
  }
