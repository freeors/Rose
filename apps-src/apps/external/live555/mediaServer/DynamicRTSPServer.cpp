...
DynamicRTSPServer::createNew(UsageEnvironment& env, Port ourPort,
			     UserAuthenticationDatabase* authDatabase, bool use_chromium,
...
void DynamicRTSPServer::OnConnect(int connection_id)
{
	...
	connection_map_[connection_id] = RTSPServerSupportingHTTPStreaming::createNewClientConnection(-1, clientAddr, 
		server_->FindSocket(connection_id), connection_id);
}
...
::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession)
{
	...
	if (strcmp(livevideo_urlsuffix, streamName) != 0) {
		// First, check whether the specified "streamName" exists as a local file:
		const std::string fullname = game_config::preferences_dir + "/" + streamName;
		Boolean fileExists = SDL_IsFile(fullname.c_str());
		SDL_Log("SDL_IsFile(%s), %s", fullname.c_str(), fileExists? "true": "false");
	}
  ...
  if (!fileExists) {
	...
	// ((BasicHashTable*)fServerMediaSessions)->dump("pre-removeServerMediaSession's fServerMediaSessions");
    if (sms == NULL) {
...
static ServerMediaSession* createNewSMS(UsageEnvironment& env, char const* fileName)
{
	...
	if (strcmp(fileName, livevideo_urlsuffix) == 0) {
		// Assumed to be a H.264 Video Elementary Stream file:
		NEW_SMS("Live Video");
		// OutPacketBuffer::maxSize = 100000; // allow for some possibly large H.264 frames
		OutPacketBuffer::maxSize = 1920 * 1080 * 3 / 2;
		sms->addSubsession(H264LiveVideoServerMediaSubsession::createNew(env, fileName, reuseSource));
		return sms;
	}
