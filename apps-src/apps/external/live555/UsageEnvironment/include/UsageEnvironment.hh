...
#define SPECIAL_CALL_MAGIC	-999999
#define ENVIR_SERVER_ID     -1 // should < 0
...
public:
  ...
  net::CompletionOnceCallback get_iocomplete()
  {
    if (iocomplete_RTSPClient) {
        return RTSPClient_iocomplete(iocomplete_arg0);
    }
    return SocketDescriptor_iocomplete(iocomplete_arg0);
  }
  ...
  const bool use_chromium;
  const int id;
  const bool send_rtcp;
  // net::CompletionOnceCallback iocomplete;
  bool iocomplete_RTSPClient;
  void* iocomplete_arg0;
  scoped_refptr<net::IOBufferWithSize> read_buf;
  bool setup_finished;
  bool in_io_pending;
protected:
  UsageEnvironment(TaskScheduler& scheduler, bool use_chromium, int id, bool send_rtcp); // abstract base class
