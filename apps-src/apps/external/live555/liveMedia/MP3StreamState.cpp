...
MP3StreamState::MP3StreamState(UsageEnvironment& env)
  : fEnv(env), fFid(NULL), fFid2(nullptr), fPresentationTimeScale(1) {
}

MP3StreamState::~MP3StreamState() {
  // Close our open file or socket:
  if (fFid2 != NULL && fFid2->valid()) {
    if (fFidIsReallyASocket) {
      intptr_t fid_long = (intptr_t)fFid;
      closeSocket((int)fid_long);
    } else {
      CloseInputFile(fFid2);
    }
  }
}

void MP3StreamState::assignStream(tfile* fid, unsigned fileSize) {
  fFid2 = fid;
...
void MP3StreamState::seekWithinFile(unsigned seekByteNumber) {
  ...
  SeekFile64(fFid2, seekByteNumber, SEEK_SET);
}
...
#endif
    return posix_fread(fFid2->fp, buf, numChars);
  }
}
