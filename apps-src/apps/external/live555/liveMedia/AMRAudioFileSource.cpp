...
AMRAudioFileSource::createNew(UsageEnvironment& env, char const* fileName) {
  ...
  do {
    ...
    if (posix_fread(fid->fp, buf, 6) < 6) break;
    ...
    if (buf[5] == '-') {
      ...
      if (posix_fread(fid->fp, &buf[bytesRead], 3) < 3) break;
    ...
    if (buf[bytesRead-1] == '_') {
      ...
      if (posix_fread(fid->fp, &buf[bytesRead], 6) < 6) break;
      if (strncmp(&buf[bytesRead], "MC1.0\n", 6) != 0) break; // bad magic #
      bytesRead += 6;

      // The next 4 bytes contain the number of channels:
      char channelDesc[4];
      if (posix_fread(fid->fp, channelDesc, 4) < 4) break;
...
AMRAudioFileSource
::AMRAudioFileSource(UsageEnvironment& env, tfile* fid,
...
/*
  // require use tfile's method replace below code.
...
*/
  ...
  while (1) {
    if (posix_fread(fFid->fp, &fLastFrameHeader, 1) < 1) {
      handleClosure();
      return;
    }
  ...
  fFrameSize = posix_fread(fFid->fp, fTo, fFrameSize);
