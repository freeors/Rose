...
void RTCPInstance::setStreamSocket(int sockNum, net::StreamSocket* netSock, 
				   unsigned char streamChannelId) {
  // Turn off background read handling:
  fRTCPInterface.stopNetworkReading();

  // Switch to RTCP-over-TCP:
  fRTCPInterface.setStreamSocket(sockNum, netSock, streamChannelId);
...
void RTCPInstance::addStreamSocket(int sockNum, net::StreamSocket* netSock, 
				   unsigned char streamChannelId) {
  ...
  fRTCPInterface.addStreamSocket(sockNum, netSock, streamChannelId);
...
void RTCPInstance::incomingReportHandler1() {
  do {
	...
	if (!fRTCPInterface.use_chromium()) {
      readResult = fRTCPInterface.handleRead(&fInBuf[fNumBytesAlreadyRead], maxRTCPPacketSize - fNumBytesAlreadyRead,
				  numBytesRead, fromAddress,
				  tcpSocketNum, tcpStreamChannelId,
				  packetReadWasIncomplete);
	} else {
		readResult = fRTCPInterface.handleRead_chromium(&fInBuf[fNumBytesAlreadyRead], maxRTCPPacketSize - fNumBytesAlreadyRead,
				  numBytesRead, fromAddress,
				  tcpSocketNum, tcpStreamChannelId,
				  packetReadWasIncomplete);
	}
...
Boolean RTCPInstance::addReport(Boolean alwaysAdd) {
  ...
  if (fSource != NULL) {
    ...
    if (alwaysAdd) {
        // [librose] in common, although sending rtcp, not send RR(Receiver Report)
        // this rtcp inluce a SDES only.
        addRR();
    }
  }
...
void RTCPInstance::enqueueCommonReportPrefix(unsigned char packetType,
  ...
  // SDL_Log("[RTCP] enqueueCommonReportPrefix, SSRC: 0x%08x, numReportingSources: %u", SSRC, numReportingSources);
...
/*
  SDL_Log("[RTCP] enqueueReportBlock, SSRC: 0x%08x highest(%u) %u %i, %u %i %u, %u %u", 
      fSource->SSRC(),
      highestExtSeqNumReceived, totNumExpected, totNumLost,
      numExpectedSinceLastReset, numLostSinceLastReset, (unsigned)lossFraction,
      NTPmsw, NTPlsw);
*/
...
#endif
  if (envir().use_chromium) {
      const base::TimeDelta timeout = base::TimeDelta::FromMilliseconds(secondsToDelay * 1000);
	  task_runner_->PostDelayedTask(FROM_HERE, base::Bind(&live555::rtcp_onExpire_chromium, envir().id, this), timeout);
  } else {
...
extern "C" void Schedule(double nextTime, event_spec e) {
...
extern "C" void Reschedule(double nextTime, event_spec e) {
  RTCPInstance* instance = (RTCPInstance*)e;
  if (instance == NULL) return;
...
extern "C" void SendRTCPReport(event_spec e) {
  RTCPInstance* instance = (RTCPInstance*)e;
  if (instance == NULL) return;
...
extern "C" void SendBYEPacket(event_spec e) {
  RTCPInstance* instance = (RTCPInstance*)e;
  if (instance == NULL) return;
...
extern "C" int TypeOfEvent(event_spec e) {
  RTCPInstance* instance = (RTCPInstance*)e;
...
extern "C" int SentPacketSize(event_spec e) {
  RTCPInstance* instance = (RTCPInstance*)e;
