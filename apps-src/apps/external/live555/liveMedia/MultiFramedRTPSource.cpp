...
#include "liveMedia/include/MultiFramedRTPSource.hh"
...
::MultiFramedRTPSource(UsageEnvironment& env, Groupsock* RTPgs,
		       ...
		       BufferedPacketFactory* packetFactory, bool useChromium)
  : RTPSource(env, RTPgs, rtpPayloadFormat, rtpTimestampFrequency)
  , fUseChromium(useChromium) {
  reset();
...
void MultiFramedRTPSource::doGetNextFrame1() {
  while (fNeedDelivery) {
	...
	// I doesn't know why enter here. Need to read more code later.
		  dbg_variable.fExceptionTimes ++;
		  if (!fUseChromium) {
			nextTask() = envir().taskScheduler().scheduleDelayedTask(0,
								 (TaskFunc*)FramedSource::afterGetting, this);
		  } else {
			  // base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&FramedSource::afterGetting, base::Unretained(this)));
			  // I can not sure how to solve it. call afterGetting(this) directly. !!risk of a long chain of recursion
			  afterGetting(this);		  
		  }
      }
    } else {
...
/*
    if (timestampFrequency() == 90000 && envir().send_rtcp) {
        int ii = 0;
        static int times = 0;
        SDL_Log("#%i stats. rtpSeqNo: %i, rtpTimestamp: %u, presentationTime: %d.%d, size: %u", 
            times, (int)rtpSeqNo, rtpTimestamp, presentationTime.tv_sec, presentationTime.tv_usec, bPacket->dataSize());
        times ++;
    }
*/
...
Boolean BufferedPacket::fillInData(RTPInterface& rtpInterface, struct sockaddr_in& fromAddress,
  ...
  if (!rtpInterface.use_chromium()) {
    ...
    if (!rtpInterface.handleRead_chromium(&fBuf[fTail], maxBytesToRead,
			       numBytesRead, fromAddress,
			       tcpSocketNum, tcpStreamChannelId,
			       packetReadWasIncomplete)) {
      return False;
