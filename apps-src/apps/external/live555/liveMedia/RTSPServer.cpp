...
#include "liveMedia/include/RTSPRegisterSender.hh"
...
void RTSPServer
::noteTCPStreamingOnSocket(char const* key, RTSPClientSession* clientSession, unsigned trackNum) {
  streamingOverTCPRecord* sotcpCur
    = (streamingOverTCPRecord*)fTCPStreamingDatabase->Lookup(key);
  streamingOverTCPRecord* sotcpNew
    = new streamingOverTCPRecord(clientSession->fOurSessionId, trackNum, sotcpCur);
  fTCPStreamingDatabase->Add(key, sotcpNew);
}

void RTSPServer
::unnoteTCPStreamingOnSocket(char const* key, RTSPClientSession* clientSession, unsigned trackNum)
{
  if (key == nullptr) {
	  return;
  }
  ...
  if (sotcp == sotcpHead) {
    ...
    if (sotcpHead == NULL) {
      ...
      fTCPStreamingDatabase->Remove(key);
    } else {
      // Add the rest of the list into the hash table (replacing the original):
      fTCPStreamingDatabase->Add(key, sotcpHead);
    }
  } else {
...
void RTSPServer::stopTCPStreamingOnSocket(char const* key) {
  ...
  if (sotcp != NULL) {
	...
	= (RTSPServer::RTSPClientSession*)lookupClientSession(sotcp->fSessionId);
      if (clientSession != NULL) {
		  SDL_Log("RTSPServer::stopTCPStreamingOnSocket, deleteStreamByTrack");
...
::RTSPClientConnection(RTSPServer& ourServer, int clientSocket, struct sockaddr_in clientAddr, net::StreamSocket* netSock, int sessionid)
  : GenericMediaServer::ClientConnection(ourServer, clientSocket, clientAddr, netSock, sessionid),
    fOurRTSPServer(ourServer), fClientInputSocket(fOurSocket)
	, fClientOutputSocket(fOurSocket)
	, fClientNetSock(netSock)
...
RTSPServer::RTSPClientConnection::~RTSPClientConnection() {
  ...
  SDL_Log("RTSPClientConnection::~RTSPClientConnection, E, fSessionid: %i", fSessionid);
  closeSocketsRTSP();
  SDL_Log("RTSPClientConnection::~RTSPClientConnection, X, fSessionid: %i", fSessionid);
}
...
void RTSPServer::RTSPClientConnection::closeSocketsRTSP() {
  ...
  char const* key = fClientOutputSocket != -1? (char const*)fClientOutputSocket: (char const*)fClientNetSock;
  SDL_Log("RTSPClientConnection::closeSocketsRTSP, stopTCPStreamingOnSocket");
  fOurRTSPServer.stopTCPStreamingOnSocket(key);
...
int RTSPServer::RTSPClientConnection::handleRequestBytes_chromium(int connection_id, const char* buf, int newBytesRead)
{
	...
	do {
		...
		if (!has_copied) {
			memcpy(&fRequestBuffer[fRequestBytesAlreadySeen], buf, newBytesRead);
			has_copied = true;
		}
		...
		if (parseSucceeded) {
			...
			if (strcmp(cmdName, "OPTIONS") == 0) {
				...
				|| strcmp(cmdName, "SET_PARAMETER") == 0) {
				if (clientSession != NULL) {
					SDL_Log("handleRequestBytes_chromium, clientSession->handleCmd_withinSession");
					clientSession->handleCmd_withinSession(this, cmdName, urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);
				} else {
					handleCmd_sessionNotFound();
				}
	...
	--fRecursionCount;
	if (!fIsActive) {
		fOurServer.rtsp_server().PostCloseConnection(connection_id);
/*
		if (fRecursionCount > 0) {
			SDL_Log("RTSPClientConnection::handleRequestBytes_chromium fail, will closeSockets()");
			closeSockets(); 
		} else { 
			SDL_Log("RTSPClientConnection::handleRequestBytes_chromium fail, will delete this");
			delete this;
		}
		// Note: The "fRecursionCount" test is for a pathological situation where we reenter the event loop and get called recursively
		// while handling a command (e.g., while handling a "DESCRIBE", to get a SDP description).
...
*/
	...
	return fRequestBufferBytesLeft - 1;
}
...
RTSPServer::RTSPClientSession::~RTSPClientSession() {
  SDL_Log("RTSPClientSession::~RTSPClientSession(), E");
  reclaimStreamStates();
  SDL_Log("RTSPClientSession::~RTSPClientSession(), X");
}

void RTSPServer::RTSPClientSession::deleteStreamByTrack(unsigned trackNum) {
  SDL_Log("RTSPClientSession::deleteStreamByTrack(%i), E", trackNum);
  if (trackNum >= fNumStreamStates) return; // sanity check; shouldn't happen
  if (fStreamStates[trackNum].subsession != NULL) {
	SDL_Log("RTSPClientSession::deleteStreamByTrack, deleteStream");
...
void RTSPServer::RTSPClientSession::reclaimStreamStates()
{
	for (unsigned i = 0; i < fNumStreamStates; ++i) {
		if (fStreamStates[i].subsession != NULL) {
			char const* key = fStreamStates[i].tcpSocketNum != -1? (char const*)fStreamStates[i].tcpSocketNum: (char const*)fStreamStates[i].tcpNetSock;
			fOurRTSPServer.unnoteTCPStreamingOnSocket(key, this, i);
			SDL_Log("RTSPClientSession::reclaimStreamStates, i: %i/%i, delete deleteStream", i, fNumStreamStates);
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  do {
	...
	fStreamStates[i].tcpNetSock = nullptr;
    ...
    if (token != NULL) {
	  ...
	  char const* key = fStreamStates[trackNum].tcpSocketNum != -1? (char const*)fStreamStates[trackNum].tcpSocketNum: (char const*)fStreamStates[trackNum].tcpNetSock;
      fOurRTSPServer.unnoteTCPStreamingOnSocket(key, this, trackNum);
	  SDL_Log("RTSPClientSession::handleCmd_SETUP, delete deleteStream");
    ...
    if (streamingMode == RTP_TCP) {
	  ...
	  fStreamStates[trackNum].tcpNetSock = ourClientConnection->fClientNetSock;
	  char const* key = fStreamStates[trackNum].tcpSocketNum != -1? (char const*)fStreamStates[trackNum].tcpSocketNum: (char const*)fStreamStates[trackNum].tcpNetSock;
      fOurRTSPServer.noteTCPStreamingOnSocket(key, this, trackNum);
    }
    ...
    subsession->getStreamParameters(fOurSessionId, ourClientConnection->fClientAddr.sin_addr.s_addr,
				    ...
				    fStreamStates[trackNum].tcpSocketNum, ourClientConnection->fNetSock, rtpChannelId, rtcpChannelId,
...
::handleCmd_withinSession(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  if (strcmp(cmdName, "TEARDOWN") == 0) {
	SDL_Log("RTSPClientSession::handleCmd_withinSession, handleCmd_TEARDOWN");
...
::handleCmd_TEARDOWN(RTSPServer::RTSPClientConnection* ourClientConnection,
		     ServerMediaSubsession* subsession) {
  unsigned i;
  SDL_Log("RTSPClientSession::handleCmd_TEARDOWN, E");
  for (i = 0; i < fNumStreamStates; ++i) {
	...
	char const* key = fStreamStates[i].tcpSocketNum != -1? (char const*)fStreamStates[i].tcpSocketNum: (char const*)fStreamStates[i].tcpNetSock;
	fOurRTSPServer.unnoteTCPStreamingOnSocket(key, this, i);
	SDL_Log("RTSPClientSession::handleCmd_TEARDOWN, deleteStream");
	fStreamStates[i].subsession->deleteStream(fOurSessionId, fStreamStates[i].streamToken);
  ...
  if (noSubsessionsRemain) {
	  // fOurServer.rtsp_server().PostCloseConnection();
	  delete this;
  }
}
...
RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr, net::StreamSocket* netSock, int sessionid) {
  return new RTSPClientConnection(*this, clientSocket, clientAddr, netSock, sessionid);
}
