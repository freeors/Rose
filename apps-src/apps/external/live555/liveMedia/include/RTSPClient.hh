...
public:
  ...
  enum State {
	...
	STATE_TRANSPORT_WRITE_COMPLETE,
	STATE_TRANSPORT_READ,
	STATE_TRANSPORT_READ_COMPLETE,
    STATE_NONE,
  };
  ...
  int DoTransportWriteComplete(int result);
  int DoTransportReadComplete(int result);

  void Start_chromium(responseHandler* responseHandler, Authenticator* authenticator = nullptr);
  ...
  int DoWriteLoop(net::NetworkTrafficAnnotationTag traffic_annotation);
  void OnWriteCompleted(int client_id, int rv);
  int HandleWriteResult(int rv);

  uint32_t ipv4() const { return fServerAddress; }
...
protected:
  RTSPClient(UsageEnvironment& env, char const* rtspURL,
	     int verbosityLevel, char const* applicationName, portNumBits tunnelOverHTTPPortNum, int socketNumToServer, bool use_chromium = false);
  ...
  unsigned sendRequest1(RequestRecord* request);

  void resetTCPSockets();
  virtual void socket_io_fail() { resetTCPSockets(); }
...
private:
  ...
  int openConnection1(); // result values: -1: failure; 0: pending; 1: success
  ...
  void handleRequestError1(RequestRecord* request, int resultCode);
  ...
  Boolean handleResponseBytes1(int newBytesRead);
...
protected:
  ...
  net::NetLogWithSource net_log_dns_;
  ...
  State next_state_;
  int resolve_result_;
  net::ClientSocketFactory* const socket_factory_;
  bool use_chromium_;
  responseHandler* describeResponseHandler_;
  Authenticator* describeAuthenticator_;
  int socket_op_result_;

  std::unique_ptr<net::StreamSocket> ctrl_socket_;
  net::StreamSocket* fOutputSocket_;
  bool read_by_rtpinterface_;
  const scoped_refptr<net::HttpConnection::QueuedWriteIOBuffer> write_buf2_;

  struct tin_rtpinterface_iocomplete_lock {
	  tin_rtpinterface_iocomplete_lock(RTSPClient& client)
		  : client_(client)
	  {
		  DCHECK(client_.in_rtpinterface_iocomplete_ == false);
		  client_.in_rtpinterface_iocomplete_ = true;
	  }
	  ~tin_rtpinterface_iocomplete_lock()
	  {
		  client_.in_rtpinterface_iocomplete_ = false;
	  }
	  RTSPClient& client_;
  };
  bool in_rtpinterface_iocomplete_;
