...
struct tdbg_variable {
	unsigned tcpReadHandler1_times;
	int fOriginalTCPReadingState;
	int fTCPReadingState;
	int fNextTCPReadSize;
	bool isCurrentlyAwaitingData;
	unsigned fExceptionTimes;
	uint32_t fWritePendingTimes;
	int fSendFails;
};
extern tdbg_variable dbg_variable;

class tsessionid_lock
{
public:
	static int sessionid;

	tsessionid_lock(int id);
	~tsessionid_lock();
};
