...
public:
  ...
  void setStreamSocket(int sockNum, net::StreamSocket* netSock, unsigned char streamChannelId);
  void addStreamSocket(int sockNum, net::StreamSocket* netSock, unsigned char streamChannelId);
  void removeStreamSocket(int sockNum, net::StreamSocket* netSock, unsigned char streamChannelId);
  static void setServerRequestAlternativeByteHandler(UsageEnvironment& env, int socketNum, net::StreamSocket* netSock,
						     ServerRequestAlternativeByteHandler* handler, void* clientData);
  static void clearServerRequestAlternativeByteHandler(UsageEnvironment& env, int socketNum, net::StreamSocket* netSock);
  ...
  Boolean handleRead_chromium(unsigned char* buffer, unsigned bufferMaxSize,
		     // out parameters:
		     unsigned& bytesRead, struct sockaddr_in& fromAddress,
		     int& tcpSocketNum, unsigned char& tcpStreamChannelId,
		     Boolean& packetReadWasIncomplete);
  bool use_chromium() const { return envir().use_chromium; }
  ...
  void chromium_slice(int sockNum, net::StreamSocket* netSock);
  void OnIOComplete(int result);

private:
  // Helper functions for sending a RTP or RTCP packet over a TCP connection:
  bool sendRTPorRTCPPacketOverTCP(unsigned char* packet, unsigned packetSize,
				     int socketNum, net::StreamSocket* netSock, unsigned char streamChannelId);
  bool sendDataOverTCP(int socketNum, net::StreamSocket* netSock, u_int8_t const* data, unsigned dataSize, Boolean forceSendToSucceed);
