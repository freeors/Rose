...
protected:
  MultiFramedRTPSink(UsageEnvironment& env,
		     ...
		     unsigned numChannels = 1, bool isLiveVideo = false);
...
private:
  void buildAndSendPacket(Boolean isFirstPacket, bool isSendNextCall);
  void packFrame();
  void sendPacketIfNecessary();
  static void sendNext_chromium(int sessionid, void* firstArg);
...
private:
  ...
  unsigned fSendFrames;
  unsigned fLastSendFrames;
  uint32_t startup_ms;
  uint32_t last_ms;
  bool fIsLiveVideo;
};
