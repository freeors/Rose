...
class GenericMediaServer: public Medium, public net::RtspServer::Delegate {
public:
  static GenericMediaServer* instance;
  ...
  net::RtspServer& rtsp_server() { return *server_.get(); }
  void setup_chromium(uint32_t ipaddr);

  void OnConnect(int connection_id) override {}
  int OnRtspRequest(int connection_id, const char* buf, int buf_len) override { return 0; }
...
public: // should be protected, but some old compilers complain otherwise
  ...
  public:
    virtual int handleRequestBytes_chromium(int connection_id, const char* buf, int buf_len) = 0;
  ...
  protected:
	...
	net::StreamSocket* fNetSock; // used in chromium
	int fSessionid;
...
protected:
  virtual ClientConnection* createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr, net::StreamSocket* netSock, int sessionid) = 0;
...
protected:
  ...
  unsigned fReclamationSeconds;
	...
	std::unique_ptr<net::RtspServer> server_;
	net::IPEndPoint server_address_;
	std::map<int /* connection_id */, ClientConnection* /* ClientConnection */> connection_map_;
