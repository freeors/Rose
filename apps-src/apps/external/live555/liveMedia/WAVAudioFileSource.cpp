...
#include "liveMedia/include/WAVAudioFileSource.hh"
...
WAVAudioFileSource::createNew(UsageEnvironment& env, char const* fileName) {
  do {
	...
	FILE* fid = nullptr; // Fix in future
    if (fid == NULL) break;
    ...
    // newSource->fFileSize = (unsigned)GetFileSize(fileName, fid); Fix in future
...
void WAVAudioFileSource::setScaleFactor(int scale) {
  ...
  if (fScaleFactor < 0 && TellFile64(fFid) > 0) {
    ...
    // SeekFile64(fFid, -bytesPerSample, SEEK_CUR); Fix in future
  }
}
...
void WAVAudioFileSource::seekToPCMByte(unsigned byteNumber) {
  ...
  // SeekFile64(fFid, byteNumber, SEEK_SET); Fix in future
}
...
WAVAudioFileSource::WAVAudioFileSource(UsageEnvironment& env, FILE* fid)
  ...
  // fFidIsSeekable = FileIsSeekable(fFid); Fix in future
...
WAVAudioFileSource::~WAVAudioFileSource() {
  ...
  // CloseInputFile(fFid); Fix in future
}
...
void WAVAudioFileSource::doReadFromFile() {
  ...
  while (1) { // loop for 'trick play' only
    ...
    if (fScaleFactor != 1) {
      // SeekFile64(fFid, (fScaleFactor-1)*bytesPerSample, SEEK_CUR); Fix in future
