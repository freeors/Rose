...
#include "liveMedia/include/OnDemandServerMediaSubsession.hh"
...
::getStreamParameters(unsigned clientSessionId,
  ...
  if (fLastStreamToken != NULL && fReuseFirstSource) {
    ...
    if (clientRTPPort.num() != 0 || tcpSocketNum >= 0 || tcpNetSock != nullptr) { // Normal case: Create destinations
  ...
  Destinations* destinations;
  if (tcpSocketNum < 0 && tcpNetSock == nullptr) { // UDP
    destinations = new Destinations(destinationAddr, clientRTPPort, clientRTCPPort);
  } else { // TCP
    destinations = new Destinations(tcpSocketNum, tcpNetSock, rtpChannelId, rtcpChannelId);
  }
...
void OnDemandServerMediaSubsession::deleteStream(unsigned clientSessionId,
  ...
  if (streamState != NULL) {
    ...
    if (streamState->referenceCount() == 0) {
	  SDL_Log("OnDemandServerMediaSubsession::deleteStream, delete streamState");
      delete streamState;
      streamToken = NULL;
    }
  }
...
{
	SDL_Log("StreamState::~StreamState, E");
	reclaim();
	SDL_Log("StreamState::~StreamState, X");
}
...
::startPlaying(Destinations* dests, unsigned clientSessionId,
  ...
  if (dests->isTCP) {
    ...
    if (fRTPSink != NULL) {
      fRTPSink->addStreamSocket(dests->tcpSocketNum, dests->tcpNetSock, dests->rtpChannelId);
      RTPInterface
	::setServerRequestAlternativeByteHandler(fRTPSink->envir(), dests->tcpSocketNum, dests->tcpNetSock,
						 serverRequestAlternativeByteHandler, serverRequestAlternativeByteHandlerClientData);
        // So that we continue to handle RTSP commands from the client
    }
    if (fRTCPInstance != NULL) {
      fRTCPInstance->addStreamSocket(dests->tcpSocketNum, dests->tcpNetSock, dests->rtcpChannelId);
...
void StreamState::endPlaying(Destinations* dests, unsigned clientSessionId) {
  ...
  if (dests->isTCP) {
    if (fRTPSink != NULL) {
      fRTPSink->removeStreamSocket(dests->tcpSocketNum, dests->tcpNetSock, dests->rtpChannelId);
    }
    if (fRTCPInstance != NULL) {
      fRTCPInstance->removeStreamSocket(dests->tcpSocketNum, dests->tcpNetSock, dests->rtcpChannelId);
