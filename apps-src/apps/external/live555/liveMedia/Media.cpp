...
void MediaLookupTable::dump() const
{
	// const char* name;
	// Medium* m = (Medium*)(fTable->Lookup(name));
	BasicHashTable* table = (BasicHashTable*)fTable;
	table->dump("MediaLookupTable");
}

int tsessionid_lock::sessionid = nposm;
tsessionid_lock::tsessionid_lock(int id)
{
	CHECK(sessionid == nposm);
	sessionid = id;
}

tsessionid_lock::~tsessionid_lock()
{
	CHECK(sessionid != nposm);
	sessionid = nposm;
}