...
#include "liveMedia/include/Base64.hh"
#include "liveMedia/include/Locale.hh"
#include <groupsock/include/GroupsockHelper.hh>
#include "liveMedia/include/ourMD5.hh"
...
namespace live555 {
extern void OnWriteCompleted_chromium(int id, int rv);
}
...
static base::Thread* fls_socket_thread = nullptr;
static int thread_refcount = 0;

static base::Thread* thread_addref()
{
	if (thread_refcount == 0) {
		fls_socket_thread = new base::Thread("RtspSocketThread");
		...
		CHECK(fls_socket_thread->StartWithOptions(socket_thread_options));
	}

	thread_refcount ++;
	return fls_socket_thread;
}

static void thread_release()
{
	thread_refcount --;
	CHECK(thread_refcount >= 0);
	if (thread_refcount == 0) {
		delete fls_socket_thread;
		fls_socket_thread = nullptr;
	}
}

RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
		       int verbosityLevel, char const* applicationName,
		       portNumBits tunnelOverHTTPPortNum, int socketNumToServer, bool use_chromium)
  : Medium(env),
    ...
    , write_buf2_(new net::HttpConnection::QueuedWriteIOBuffer())
	, socket_thread_(thread_addref())
{
  SDL_Log("RTSPClient::RTSPClient()--- rtspURL: %s", rtspURL);
  setBaseURL(rtspURL);
  ...
  SDL_Log("---RTSPClient::RTSPClient() X, rtspURL: %s", rtspURL);
}

RTSPClient::~RTSPClient() {
  SDL_Log("RTSPClient::~RTSPClient()--- fBaseURL: %s", fBaseURL);
  reset();
  ...
  SDL_Log("---RTSPClient::~RTSPClient() X, fBaseURL: %s", fBaseURL);
}
...
int RTSPClient::grabSocket() {
  ...
  RTPInterface::clearServerRequestAlternativeByteHandler(envir(), fInputSocketNum, ctrl_socket_.get()); // in case we were receiving RTP-over-TCP
  fInputSocketNum = -1;
...
unsigned RTSPClient::sendRequest1(RequestRecord* request) {
  char* cmd = NULL;
  socket_op_result_ = net::ERR_FAILED;
  next_state_ = STATE_TRANSPORT_WRITE_COMPLETE;
  do {
	VALIDATE(ctrl_socket_.get() != nullptr, null_str);
    Boolean connectionIsPending = False;
    if (!fRequestsAwaitingConnection.isEmpty()) {
    ...
    // If requested (and we're not already doing it, or have done it), set up the special protocol for tunneling RTSP-over-HTTP:
    if (fTunnelOverHTTPPortNum != 0 && strcmp(request->commandName(), "GET") != 0 && fOutputSocket_ == ctrl_socket_.get()) {
      VALIDATE(ctrl_socket_.get() != nullptr, "not not support TunnelOverHTTPPort");
      if (!setupHTTPTunneling1()) break;
      fRequestsAwaitingHTTPTunneling.enqueue(request);
      return request->cseq();
    }
    ...
    int cmdSize = strlen(cmdFmt)
      ...
      + strlen(contentLengthHeader)
      + contentStrLen;
	if (write_buf_.get() == nullptr || cmdSize > write_buf_->size()) {
		write_buf_ = new net::IOBufferWithSize(cmdSize);
	}
	cmd = write_buf_->data();
    // cmd = new char[cmdSize];
    sprintf(cmd, cmdFmt,
	...
	net::NetworkTrafficAnnotationTag tag = MISSING_TRAFFIC_ANNOTATION;
    socket_op_result_ = ctrl_socket_->Write(write_buf_.get(), strlen(cmd), 
		envir().get_iocomplete(), tag);

    if (socket_op_result_ < 0 && socket_op_result_ != net::ERR_IO_PENDING) {
      char const* errFmt = "%s send() failed: ";
      unsigned const errLength = strlen(errFmt) + strlen(request->commandName());
      char* err = new char[errLength];
      sprintf(err, errFmt, request->commandName());
  ...
  // An error occurred, so call the response handler immediately (indicating the error):
  // delete[] cmd;
  handleRequestError1(request, socket_op_result_);
  delete request;
  return 0;
}
...
void RTSPClient::resetTCPSockets() {
  ...
  if (use_chromium_) {
    read_by_rtpinterface_ = false;
    VALIDATE(fInputSocketNum == fOutputSocketNum && fInputSocketNum == -1, null_str);
    ctrl_socket_.reset();
  }
...
int RTSPClient::openConnection1() {
  do {
    // Set up a connection to the server.  Begin by parsing the URL:
    ...
    if (ctrl_socket_.get() == nullptr) break;
    if (fOutputSocket_ == nullptr) fOutputSocket_ = ctrl_socket_.get();
    envir() << "Created new TCP socket " << ctrl_socket_.get() << " for connection\n";
      
    // Connect to the remote endpoint:
    fServerAddress = *(netAddressBits*)(destAddress.data());
    int connectResult = ctrl_socket_->Connect(envir().get_iocomplete());
    if (connectResult < 0 && connectResult != net::ERR_IO_PENDING) break;
    return connectResult;
  } while (0);
...
void RTSPClient::handleRequestError1(RequestRecord* request, int resultCode)
{
	if (request->handler() != NULL) (*request->handler())(this, resultCode, strDup(envir().getResultMsg()));
}
...
Boolean RTSPClient::handleSETUPResponse(MediaSubsession& subsession, char const* sessionParamsStr, char const* transportParamsStr,
  ...
  do {
	...
	subsession.rtpSource()->setStreamSocket(fInputSocketNum, ctrl_socket_.get(), subsession.rtpChannelId);
	...
	increaseReceiveBufferTo(envir(), fInputSocketNum, 50*1024);
      ...
      if (subsession.rtcpInstance() != NULL) subsession.rtcpInstance()->setStreamSocket(fInputSocketNum, ctrl_socket_.get(), subsession.rtcpChannelId);
      RTPInterface::setServerRequestAlternativeByteHandler(envir(), fInputSocketNum, ctrl_socket_.get(), handleAlternativeRequestByte, this);
      read_by_rtpinterface_ = true;
    } else {
      // Normal case.
...
void RTSPClient::handleAlternativeRequestByte1(u_int8_t requestByte) {
  if (requestByte == 0xFF) {
	...
	if (!use_chromium_) {
	  ...
	  tin_rtpinterface_iocomplete_lock lock(*this);
	  OnIOComplete(1);
	}
  }
}
...
net::CompletionOnceCallback RTSPClient_iocomplete(void* _client)
{
    RTSPClient* client = reinterpret_cast<RTSPClient*>(_client);
    return base::Bind(&RTSPClient::OnIOComplete, base::Unretained(client));
}

int RTSPClient::DoResolveHost()
{
  // DCHECK(host_resolver_.get() == nullptr);
  if (host_resolver_.get() == nullptr) {
	  host_resolver_ = net::HostResolver::CreateStandaloneResolver(nullptr);
  }
  next_state_ = STATE_RESOLVE_HOST_COMPLETE;

  DCHECK(envir().read_buf.get() == nullptr);
  // envir().read_buf = new net::IOBufferWithSize(1024);
  envir().read_buf = new net::IOBufferWithSize(100);

  // DCHECK(envir().iocomplete.is_null());
  // envir().iocomplete = base::Bind(&RTSPClient::OnIOComplete, base::Unretained(this));
  UsageEnvironment& env = envir();
  DCHECK(env.iocomplete_RTSPClient);
  DCHECK(env.iocomplete_arg0 == nullptr);
  env.iocomplete_arg0 = this;

  const GURL gurl(fBaseURL);
  net::HostPortPair host_port_pair(net::HostPortPair::FromURL(gurl));

  net::HostResolver::ResolveHostParameters parameters;
  parameters.initial_priority = net::DEFAULT_PRIORITY;
  resolve_request_ = host_resolver_->CreateRequest(host_port_pair,
                                            net::NetworkIsolationKey(),
                                            net_log_dns_, parameters);

  return resolve_request_->Start(base::BindOnce(&RTSPClient::OnIOComplete,
                                        base::Unretained(this)));
}

int RTSPClient::DoResolveHostComplete(int result) {
...
int RTSPClient::DoTransportConnect() {
  next_state_ = STATE_TRANSPORT_CONNECT_COMPLETE;
  return openConnection1();
}

int RTSPClient::DoTransportConnectComplete(int result)
{
	if (result == net::OK) {
		VALIDATE(describeResponseHandler_ != nullptr, "");

		if (fCurrentAuthenticator < describeAuthenticator_) fCurrentAuthenticator = *describeAuthenticator_;
		RequestRecord* request = new RequestRecord(++fCSeq, "DESCRIBE", describeResponseHandler_);
		// fRequestsAwaitingConnection.enqueue(request);

		sendRequest1(request);
		result = socket_op_result_;

		if (result > 0) {
			result = DoTransportWriteComplete(result);
		}
	} 
...
int RTSPClient::DoTransportWriteComplete(int result)
{
	if (result > 0) {
		...
		if (!read_by_rtpinterface_) {
			...
			result = ctrl_socket_->Read(envir().read_buf.get(), max_bytes,	envir().get_iocomplete());
		} else {
			...
			if (!in_rtpinterface_iocomplete_) {
				...
				envir().get_iocomplete().Run(SPECIAL_CALL_MAGIC);
			}
...
int RTSPClient::DoTransportReadComplete(int result)
{
	while (result > 0) {
		next_state_ = STATE_NONE;
		int bytesRead = result;
		memcpy((unsigned char*)&fResponseBuffer[fResponseBytesAlreadySeen], envir().read_buf->data(), bytesRead);

		if (!handleResponseBytes1(bytesRead)) {
			// maybe envir().read_buf too shorten, one response response multi-read.
			next_state_ = STATE_TRANSPORT_READ_COMPLETE;
			if (!read_by_rtpinterface_) {
				const int max_bytes = read_by_rtpinterface_? 1: SDL_min(envir().read_buf->size(), (int)fResponseBufferBytesLeft - 1);
				result = ctrl_socket_->Read(envir().read_buf.get(), max_bytes,	envir().get_iocomplete());
			} else {
				// let RTPInterface read this socket. net::ERR_IO_PENDING make DoLoop exit.
				result = net::ERR_IO_PENDING;
	...
	if (result <= 0 && result != net::ERR_IO_PENDING) {
		VALIDATE(result != net::OK, null_str);
		socket_io_fail();
	}
	return result;
}

void RTSPClient::Start_chromium(responseHandler* responseHandler, Authenticator* authenticator) {
  write_buf2_->set_max_buffer_size(4 * 1024 * 1024);
  describeResponseHandler_ = responseHandler;
  describeAuthenticator_ = authenticator;
  next_state_ = STATE_RESOLVE_HOST;
  DoLoop(net::OK);
}

int RTSPClient::sendData_chromium(net::StreamSocket* netSock, const uint8_t* data, int dataSize)
{
    VALIDATE(netSock == ctrl_socket_.get(), null_str);
    VALIDATE(data != nullptr && dataSize > 0, null_str);

    bool writing_in_progress = !write_buf2_->IsEmpty();
    if (write_buf2_->Append(std::string((const char*)data, dataSize)) && !writing_in_progress) {
        DoWriteLoop(TRAFFIC_ANNOTATION_FOR_TESTS);
    }
    return dataSize;
}

int RTSPClient::DoWriteLoop(net::NetworkTrafficAnnotationTag traffic_annotation) {
    net::HttpConnection::QueuedWriteIOBuffer* write_buf = write_buf2_.get();
    if (write_buf->GetSizeToWrite() == 0) {
        return net::OK;
    }
    int rv = ctrl_socket_->Write(
        write_buf, write_buf->GetSizeToWrite(),
        base::Bind(&live555::OnWriteCompleted_chromium, envir().id),
        traffic_annotation);
...
void RTSPClient::OnWriteCompleted(
    int client_id,
    int rv) {
  VALIDATE(client_id == envir().id, null_str);

  rv = HandleWriteResult(rv);
}

int RTSPClient::HandleWriteResult(int rv)
{
	if (rv < 0) {
		SDL_Log("RTSPClient::HandleWriteResult(rv: %i), call server_.Close()", rv);
		// server_.PostClose(*this);
		return rv;
	}

    // SDL_Log("RTSPClient::HandleWriteResult, [netSock(0x%x) + id(%i)] rv: %i", ctrl_socket_.get(), envir().id, rv);
    net::HttpConnection::QueuedWriteIOBuffer* write_buf = write_buf2_.get();
...
Boolean RTSPClient::handleResponseBytes1(int newBytesRead)
{
	do {
		if (newBytesRead >= 0 && (unsigned)newBytesRead < fResponseBufferBytesLeft) break; // data was read OK; process it below
		...
		if (newBytesRead > 0) { // The "RTSP response was truncated" error
			if ((request = fRequestsAwaitingResponse.dequeue()) != NULL) {
				handleRequestError1(request, net::ERR_FAILED);
				delete request;
			}
		} else {
			RequestQueue requestQueue(fRequestsAwaitingResponse);
	...
	do {
		...
		if (!endOfHeaders) return responseSuccess; // subsequent reads will be needed to get the complete response
		...
		do {
			...
			if (contentLength > numBodyBytes) {
				...
				delete[] headerDataCopy;
				if (foundRequest != NULL) fRequestsAwaitingResponse.putAtHead(foundRequest);// put our request record back; we need it again
				return responseSuccess; // We need to read more data
			}
			...
			if (foundRequest != NULL) {
				...
				if (needToResendCommand) {
					...
					(void)resendCommand(foundRequest);
					delete[] headerDataCopy;
					return responseSuccess; // without calling our response handler; the response to the resent command will do that
				}
			}
		...
		if (foundRequest != NULL && foundRequest->handler() != NULL) {
			...
			if (responseSuccess) {
				...
				// An error occurred parsing the response, so call the handler, indicating an error:
				handleRequestError1(foundRequest, net::ERR_FAILED);
			}
		}
		if (foundRequest) {
			const bool is_play_response = strcmp(foundRequest->commandName(), "PLAY") == 0;
			if (is_play_response && responseSuccess) {
				// if use chromium, media data maybe subseed play's response. but MUST NOT.
				VALIDATE(numExtraBytesAfterResponse == 0, null_str);
				envir().setup_finished = true;
			}
		}
		delete foundRequest;
		delete[] headerDataCopy;
