...
#include "liveMedia/include/ADTSAudioFileSource.hh"
...
ADTSAudioFileSource::createNew(UsageEnvironment& env, char const* fileName) {
  ...
  do {
    ...
    if (posix_fread(fid->fp, fixedHeader, sizeof(fixedHeader)) < sizeof fixedHeader) break;
...
ADTSAudioFileSource
::ADTSAudioFileSource(UsageEnvironment& env, tfile* fid, u_int8_t profile,
...
void ADTSAudioFileSource::doGetNextFrame() {
  ...
  if (posix_fread(fFid->fp, headers, sizeof headers) < sizeof headers) {
  ...
  int numBytesRead = posix_fread(fFid->fp, fTo, numBytesToRead);
