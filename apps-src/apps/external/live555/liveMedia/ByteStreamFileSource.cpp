...
ByteStreamFileSource::createNew(UsageEnvironment& env, char const* fileName,
  ...
  tfile* fid = OpenInputFile(env, fileName);
...
ByteStreamFileSource*
ByteStreamFileSource::createNew(UsageEnvironment& env, tfile* fid,
				unsigned preferredFrameSize,
				unsigned playTimePerFrame) {
  if (fid == NULL) return NULL;
...
ByteStreamFileSource::ByteStreamFileSource(UsageEnvironment& env, tfile* fid,
					   unsigned preferredFrameSize,
					   unsigned playTimePerFrame)
...
#ifdef READ_FROM_FILES_SYNCHRONOUSLY
  fFrameSize = posix_fread(fFid->fp, fTo, fMaxSize);
#else
  if (fFidIsSeekable) {
