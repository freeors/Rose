...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
  ...
  while (1) {
	...
	if (parseSDPAttribute_http(sdpLine)) continue;
  }
...
Boolean MediaSession::parseSDPAttribute_http(char const* sdpLine)
{
	const std::string prefix = "a=http:";
	if (SDL_strncmp(sdpLine, prefix.c_str(), prefix.size()) != 0) {
		return False;
	}
	int n = prefix.size();
	while (true) {
	...
	fHttpdUrl.assign(sdpLine + prefix.size(), n - prefix.size());
	return True;
}
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset, bool useChromium) {
  ...
  do {
    ...
    if (!createSourceObjects(useSpecialRTPoffset, useChromium)) break;
...
Boolean MediaSubsession::createSourceObjects(int useSpecialRTPoffset, bool useChromium) {
  do {
	...
	fReadSource = fRTPSource
	  = H264VideoRTPSource::createNew(env(), fRTPSocket,
					  fRTPPayloadFormat,
					  fRTPTimestampFrequency, useChromium);
