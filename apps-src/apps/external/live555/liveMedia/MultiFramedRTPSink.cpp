...
#include "liveMedia/include/MultiFramedRTPSink.hh"
...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  : RTPSink(env, rtpGS, rtpPayloadType, rtpTimestampFrequency,
	...
	, fIsSendNextCall(false)
	, fSendFrames(0)
	, fLastSendFrames(0)
	, startup_ms(0)
	, last_ms(0)
	, fIsLiveVideo(isLiveVideo)
	, fSessionid(tsessionid_lock::sessionid)
{
	// CHECK(fSessionid != nposm);
	SDL_Log("T:%u, MultiFramedRTPSink::MultiFramedRTPSink, E", SDL_GetTicks());
	setPacketSizes((RTP_PAYLOAD_PREFERRED_SIZE), (RTP_PAYLOAD_MAX_SIZE));
	SDL_Log("T:%u, MultiFramedRTPSink::MultiFramedRTPSink, X", SDL_GetTicks());
}

MultiFramedRTPSink::~MultiFramedRTPSink()
{
	delete fOutBuf;
	SDL_Log("T:%u, MultiFramedRTPSink::~MultiFramedRTPSink---, fSessionid: %i", SDL_GetTicks(), fSessionid);
}
...
void MultiFramedRTPSink::buildAndSendPacket(Boolean isFirstPacket, bool isSendNextCall) {
  nextTask() = NULL;
  fIsFirstPacket = isFirstPacket;
  fIsSendNextCall = isSendNextCall;
...
void MultiFramedRTPSink::sendPacketIfNecessary()
{
	...
	if (fOutBuf->curPacketSize() != RTP_PAYLOAD_MAX_SIZE) {
		fSendFrames ++;
		...
		if (now - last_ms >= 1000) {
			...
			SDL_Log("T:%u, [MultiFramedRTPSink]#%u, %u fps", SDL_GetTicks(), elapsed_second, fSendFrames - fLastSendFrames);
			fLastSendFrames = fSendFrames;
		}
	}
  ...
  if (fNoFramesLeft) {
	...
	if (!fls_use_chromium || inGetAuxSDPLine) {
		...
		base::ThreadTaskRunnerHandle::Get()->PostTask(FROM_HERE, base::BindOnce(&MultiFramedRTPSink::sendNext_chromium, fSessionid, base::Unretained(this)));
	}
  }
}

void MultiFramedRTPSink::sendNext_chromium(int sessionid, void* firstArg)
{
	if (!sessionid_is_valid_chromium(sessionid)) {
		return;
	}
	sendNext(firstArg);
}
...
void MultiFramedRTPSink::sendNext(void* firstArg) {
  ...
  sink->buildAndSendPacket(False, true);
}
