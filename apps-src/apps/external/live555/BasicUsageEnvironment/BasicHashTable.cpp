...
void BasicHashTable::dump(const char* scene) const
{
	SDL_Log("---dump(%s)---", scene);
	for (unsigned i = 0; i < fNumBuckets; ++i) {
		TableEntry* entry;

		for (entry = fBuckets[i]; entry != NULL; entry = entry->fNext) {
			SDL_Log("#%i, key(%p): %s", i, entry->key, entry->key);
		}
	}
	SDL_Log("--------------------------");
}
