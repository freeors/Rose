/* $Id: cursor.cpp 47836 2010-12-05 18:09:48Z mordante $ */
/*
...
{
	const surface nsurf = clone_surface(surf);
	if(nsurf == NULL) {
		return NULL;
	}
...
SDL_Cursor* cache[cursor::NUM_CURSORS] = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
const int shift_y[cursor::NUM_CURSORS] = {0, 0, 0, 0, 0, 0, 0, 20, 22, 0, 0, 0};
...
void draw()
{
	if (!game_config::os_kos && use_color_cursors() == false) {
		return;
	}
	...
	if (now - last_log_ticks >= 10000) {
		SDL_Log("cursor::draw, %u, have_focus: %s", now, have_focus? "true": "false");
		last_log_ticks = now;
	}
	...
	if (surf == NULL) {
		...
		SDL_Log("could not load color cursors. Falling back to hardware cursors");
		return;
	}
	...
	if (cursor_buf == NULL) {
		...
		if (cursor_buf == NULL) {
			SDL_Log("Could not allocate surface for mouse cursor");
			return;
		}
	}
...
*/
	...
	{
		surface_lock dst_lock(cursor_buf);
		SDL_RenderReadPixels(renderer, &area, get_screen_format().format, cursor_buf->pixels, 4 * area.w);
	}
	render_surface(renderer, surf, NULL, &area);
}
...
void undraw()
{
	...
	render_surface(get_renderer(), cursor_buf, NULL, &area);
}
