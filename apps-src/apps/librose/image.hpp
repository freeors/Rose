...
struct tblit
{
	...
	explicit tblit(const texture& _tex, int _x, int _y, int _width, int _height, const SDL_Rect& _clip = SDL_Rect())
		: type(BLITM_TEXTURE)
		, x(_x)
		, y(_y)
		, clip(_clip)
	...
	explicit tblit(const int type, const int x, const int y, const int width, const int height, const uint32_t color)
		...
		, blend_color(color)
	{
		VALIDATE(type == BLITM_RECT || type == BLITM_FRAME || type == BLITM_LINE, null_str);
	}
