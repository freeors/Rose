...
typedef bool (*fhero_confirm_carry_to)(hero& h1, hero& h2, int carry_to);
typedef bool (*fhero_check_valid)(const hero& h);
extern fhero_confirm_carry_to hero_confirm_carry_to;
extern fhero_check_valid hero_check_valid;
...
#if defined(_KINGDOM_EXE) || !defined(_WIN32)
	bool upgrade_leader(hero_map& heros);
	bool upgrade_member(hero_map& heros, hero& h);
	bool upgrade_internal(hero_map& heros, tmember* m, std::map<int, http::temployee>* employees);
	int calculate_total_point() const;
	void from_membership(hero_map& heros, const http::membership& m, int city = HEROS_INVALID_NUMBER);
	void from_local_membership(hero_map& heros, const http::membership& m, bool enhance);
	http::membership to_membership() const;

	void set_layout(const std::string& str);
	const std::string& layout() const { return layout_; }

	void layout_from_team(team& t);
	bool layout_to_team(unit_map& units, hero_map& heros, std::vector<team>& teams, game_state& state, team& t, bool mirror, bool conside_stratagem, bool wall);
