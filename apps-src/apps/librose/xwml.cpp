...
static uint32_t wml_config_to_fp(posix_file_t fp, const config &cfg, uint32_t *max_str_len, std::vector<std::string>& td, uint16_t deep, std::vector<std::set<std::string> >& msgids)
{
	...
	BOOST_FOREACH (const config::any_child &value, cfg.all_children_range()) {
		...
		*max_str_len = SDL_max(*max_str_len, value.key.size());
		...
		BOOST_FOREACH (const config::attribute &istrmap, value.cfg.attribute_range()) {
			...
			*max_str_len = SDL_max(*max_str_len, u32n);
...
static void generate_cfg_cpp(const std::string& fname, const std::vector<std::string>& tdomain, const std::vector<std::set<std::string> >& msgids, uint32_t max_str_len, const std::map<std::string, std::string>& owners, bool)
{
	// owners->second is owner. if it isn't app or applet, second is null_str.
	// owners example===
	// [0]<rose-lib, "">
	// [1]<kdesktop-lib, kdesktop>

	if (owners.empty()) {
		return;
	}
	// if destination file is at <res>/xwml, write cfg-cpp.
	const std::string res_xwml_path = game_config::path + "/xwml";
	bool owner_is_app = fname.find(res_xwml_path) == 0;

	// if owner_is_app is false, mean to applet. in this case, xwml_sub_dir is empty always.
	std::string xwml_sub_dir;
	if (owner_is_app) {
		xwml_sub_dir = extract_directory(fname.substr(res_xwml_path.size() + 1));
	}
	const std::string main_name = file_main_name(extract_file(fname));
	...
	for (std::vector<std::set<std::string> >::const_iterator it = msgids.begin(); it != msgids.end(); ++ it, at ++) {
		...
		const std::string& current_domain = tdomain[at];
		if (owners.find(current_domain) == owners.end()) {
			continue;
		}
		const std::string owner_app = owners.find(current_domain)->second;

		ss.str("");

		if (!owner_app.empty()) {
			// current_domain --> owner_app
			// kdesktop-lib       kdesktop
			// launcher-lib       launcher
			ss << game_config::path << "/";
			if (owner_is_app) {
				ss << game_config::generate_app_dir(owner_app);
			} else {
				ss << owner_app;
			}
			ss << "/po/cfg-cpp/" << current_domain;
			if (!xwml_sub_dir.empty()) {
				ss << "/" << xwml_sub_dir;
			}
		} else {
			VALIDATE(xwml_sub_dir.empty(), null_str);
			// current_domain
			// rose-lib
			ss << game_config::path << "/po/cfg-cpp/" << current_domain;
		}
		...
		for (std::set<std::string>::const_iterator it2 = item.begin(); it2 != item.end(); ++ it2) {
			std::string msgid = utils::replace_all(*it2, "\"", "\\\"");
			if (msgid.empty()) {
				continue;
			}
...
void wml_config_to_file(const std::string& fname, const config &cfg, uint32_t nfiles, uint32_t sum_size, uint32_t modified, const std::map<std::string, std::string>& owners)
{
	...
	if (!lock.valid()) {
		SDL_Log("------<xwml.cpp>::wml_config_to_file, cannot create %s for write\n", fname.c_str());
		return;
	}

	max_str_len = SDL_max(WMLBIN_MARK_CONFIG_LEN, WMLBIN_MARK_VALUE_LEN);
	...
	generate_cfg_cpp(fname, tdomain, msgids, max_str_len, owners, true);
}
...
bool wml_config_from_data(uint8_t *data, uint32_t datalen, uint8_t *namebuf, uint8_t *valbuf, std::vector<std::string> &tdomain, config &cfg)
{
	...
	while (rdpos < data + datalen) {
		...
		// SDL_Log("in while, rdpos: %p, pos: %u(0x%x)", rdpos, rdpos - data + 4, rdpos - data + 4);
		// read {[cfg]}{len}{name}
...
void wml_config_from_file(const std::string &fname, config &cfg, uint32_t* nfiles, uint32_t* sum_size, uint32_t* modified)
{
	...
	SDL_Log("<xwml.cpp>::wml_config_from_file------fname: %s\n", fname.c_str());

	cfg.clear();	// first clear. below action is add.

	tfile lock(fname, GENERIC_READ, OPEN_EXISTING);
	if (!lock.valid()) {
		SDL_Log("------<xwml.cpp>::wml_config_from_file, cannot create %s for read\n", fname.c_str());
		return;
	}
	...
	for (idx = 0; idx < tdcnt; idx ++) {
		...
		{
			// below statement will change translations path, may remark?
			int ii = 0;
...
// @deep: nesting deep. top level: 0
static uint32_t collect_msgid_from_cfg_deep(const config &cfg, uint32_t *max_str_len, std::vector<std::string>& td, uint16_t deep, std::vector<std::set<std::string> >& msgids)
{
	// Except for not storing file, logic is same as wml_config_to_fp.
	...
	BOOST_FOREACH (const config::any_child &value, cfg.all_children_range()) {
		...
		*max_str_len = SDL_max(*max_str_len, value.key.size());
		...
		bytes += collect_msgid_from_cfg_deep(value.cfg, max_str_len, td, deep + 1, msgids);
	}
...
{
	// Except for not storing file, logic is same as wml_config_to_file.
	...
	std::vector<std::set<std::string> > msgids;
	collect_msgid_from_cfg_deep(cfg, &max_str_len, tdomain, 0, msgids);

	// fname: c:/ddksample/apps-res/xwml/gui.bin
	generate_cfg_cpp(fname, tdomain, msgids, max_str_len, owners, owner_is_app);
}
...
void wml_building_rules_to_file(const std::string& fname, terrain_builder::building_rule* rules, uint32_t rules_size, uint32_t nfiles, uint32_t sum_size, uint32_t modified)
{
	...
	SDL_Log("<xwml.cpp>::wml_building_rules_to_file------fname: %s, will save %u rules\n", fname.c_str(), rules_size);

	posix_fopen(fname.c_str(), GENERIC_WRITE, CREATE_ALWAYS, fp);
	if (fp == INVALID_FILE) {
		SDL_Log("------<xwml.cpp>::wml_building_rules_to_file, cannot create %s for wrtie\n", fname.c_str());
		return;
	}
...
//					}
	...
	SDL_Log("------<xwml.cpp>::wml_building_rules_to_file, return\n");
	return;
}
...
terrain_builder::building_rule* wml_building_rules_from_file(const std::string& fname, uint32_t* rules_size_ptr)
{
	...
	if (fp == INVALID_FILE) {
		SDL_Log("------<xwml.cpp>::wml_building_rules_from_file, cannot create %s for read\n", fname.c_str());
		return NULL;
	}
	...
	SDL_Log("max_str_len: %u, fsize: %u, datalen: %u\n", max_str_len, (int)fsize, datalen);
...
*/
	...
	SDL_Log("------<xwml.cpp>::wml_building_rules_from_file, restore %u rules, return\n", rule_index);
	return rules;
}
