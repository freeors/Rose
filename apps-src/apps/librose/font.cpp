...
void update_7size(int default_size)
{
	int cfg_default_size = default_size;
	if (cfg_default_size == nposm) {
		// i think, win10 is 13
		cfg_default_size = game_config::mobile? 16: 13;
	}

	int quantizer;
	if (game_config::mobile || game_config::gui_as_mobile) {
		if (cfg_default_size <= 16) {
			quantizer = 2;
		} else if (cfg_default_size <= 24) {
			quantizer = 3;
		} else {
			quantizer = 4;
		}
	} else {
		if (cfg_default_size <= 13) {
			quantizer = 2;
		} else if (cfg_default_size <= 18) {
			quantizer = 3;
		} else {
			quantizer = 4;
		}
	}
	font::SIZE_SMALLEST = (cfg_default_size - quantizer * 3) * gui2::twidget::hdpi_scale;
	font::SIZE_SMALLER = (cfg_default_size - quantizer * 2) * gui2::twidget::hdpi_scale;
	font::SIZE_SMALL = (cfg_default_size - quantizer) * gui2::twidget::hdpi_scale;
	font::SIZE_DEFAULT = cfg_default_size * gui2::twidget::hdpi_scale;
	font::SIZE_LARGE = (cfg_default_size + quantizer) * gui2::twidget::hdpi_scale;
	font::SIZE_LARGER = (cfg_default_size + quantizer * 2) * gui2::twidget::hdpi_scale;
	font::SIZE_LARGEST = (cfg_default_size + quantizer * 3) * gui2::twidget::hdpi_scale;
}
...
static surface text_render(const std::string& text, int font_size, const SDL_Color& font_color, int style)
{
	...
	// It takes some time to perform text_surface::get_surfaces. in order to save, wesnoth use std::list<text_surface> cache.
	// I discard it, because it isn't safe for multi-thread.
	const std::vector<surface>& surfs = txt_surf.get_surfaces();
	...
	if (surfs.size() == 1) {
		...
		size_t width = txt_surf.width();
		size_t height = txt_surf.height();
...
surface floating_label::create_surface()
{
	if (surf_.null()) {
		...
		if (bgalpha_ != 0) {
			...
			foreground = adjust_surface_alpha(foreground, ftofxp(1.13));
