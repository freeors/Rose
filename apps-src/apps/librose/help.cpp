...
static int calculate_attention_min_chars(const int width, const int font_size) 
{
	...
	return attention_min_chars;
}

std::vector<std::string> split_in_width(const std::string &s, const int font_size, int style, const unsigned width)
{
	VALIDATE(!s.empty(), null_str);
	...
	try {
		...
		int attention_min_chars = calculate_attention_min_chars(width, font_size);
		const std::string& first_line = font::word_wrap_text(s, font_size, style, width, 1, attention_min_chars);
...
std::string get_first_word(const std::string& s, int font_size, int style, int max_width)
{
	// max_width maybe < font_size, for example only one "(".
	VALIDATE(!s.empty() && s[0] != '\n' && font_size > 0 && max_width > 0, null_str);

	utils::utf8_iterator ch(s);
	std::string current_word;
	const int min_may_cut_chars = 15; // can not use calculate_attention_min_chars. max_width is remaining_width, maybe small.
	const int attention_min_chars = min_may_cut_chars;

	// [see remark#23]
	wchar_t previous = 0;
	int chars = 0;
	size_t first_word_end = 0;
	for (; ch != utils::utf8_iterator::end(s); ++ch) {
		...
		first_word_end += ch.substr().second - ch.substr().first;
		previous = wch;

		if (wch == ' ') {
			VALIDATE(current_word.find_first_not_of(' ') == std::string::npos, null_str);
		} else {
			chars ++;
		}
		if (chars > attention_min_chars) {
			if (font::line_size(current_word, font_size, style).w > max_width) {
				size_t last_char_size = ch.substr().second - ch.substr().first;
				current_word.erase(current_word.size() - last_char_size);
				first_word_end -= last_char_size;
				break;
			}
		}
	}
...
void init_book(const config* _game_cfg, tmap* _map, bool _editor)
{
	game_cfg = _game_cfg;
	map = _map;
	editor = _editor;
...
std::string multi_digit_image_str(int integer)
{
	...
	for (std::vector<int>::const_reverse_iterator it = digits.rbegin(); it != digits.rend(); ++ it) {
		...
		strstr << ht::generate_img(img.str());
	}
	return strstr.str();
}
