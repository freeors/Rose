...
struct tdisable_up_result_swipe
{
	tdisable_up_result_swipe();
	~tdisable_up_result_swipe();
};
...
private:
	...
	void send_extra_mouse_up(const SDL_Event& event);
...
enum {REASON_NORMAL, REASON_CLOSE};
void set_visible(bool visible, int reason = REASON_NORMAL);
bool is_visible();

struct tkey
{
	tkey(const std::string& utf8, int ch);

	std::string utf8;
	int ch;
};
extern std::vector<tkey> keys;
