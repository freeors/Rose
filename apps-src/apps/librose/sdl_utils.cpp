...
surface clone_surface(const surface &surf)
{
	...
	if (result.get() == nullptr) {
		...
		err << " size: " << surf->w << "x" << surf->h;
		VALIDATE(result, err.str());
	}
...
surface create_neutral_surface(int w, int h, bool use_rle)
{
	if (w <= 0 || h <= 0) {
	...
	SDL_Surface* result = SDL_CreateRGBSurface(SDL_SWSURFACE, w, h,
			format.BitsPerPixel,
			format.Rmask,
			format.Gmask,
			format.Bmask,
...
texture create_neutral_texture(const int w, const int h, const int access)
{
	...
	if (result.get() != nullptr) {
		// if app is in background, SDL_CreateTexture will be nullptr(android).
...
texture create_alpha_texture(int width, int height, uint32_t color)
{
	...
	return SDL_CreateTextureFromSurface(get_renderer(), bg_surf);
}
...
tpoint calculate_adaption_ratio_size(const int outer_w, const int outer_h, const int inner_w, const int inner_h)
{
	if (outer_w <= 0 || outer_h <= 0 || inner_w <= 0 || inner_h <= 0) {
		SDL_Log("calculate_adaption_ratio_size(%i, %i, %i, %i), invalid parameter", outer_w, outer_h, inner_w, inner_h);
		VALIDATE(false, null_str);
	}
...
cv::Mat get_adaption_ratio_mat(const cv::Mat& src, const int outer_width, const int outer_height)
{
	...
	if (src.cols != center.x || src.rows != center.y) {
		cv::resize(src, center_mat, cv::Size(center.x, center.y));
	}
...
tpoint calculate_adaption_ratio_size_cut(const int outer_w, const int outer_h, const int inner_w, const int inner_h)
{
	if (outer_w <= 0 || outer_h <= 0 || inner_w <= 0 || inner_h <= 0) {
		std::stringstream err;
		err << "calculate_adaption_ratio_size_cut(" << outer_w << "," << outer_h << "," << inner_w << "," << inner_h << "), invalid parameter";
	...
	tpoint result(static_cast<int>(outer_w / ratio), static_cast<int>(outer_h / ratio));
	if (result.x > inner_w) {
		result.x = inner_w;
	}
	if (result.y > inner_h) {
...
uint32_t surf_calculate_most_color(const surface& surf)
{
	...
	const cv::Mat& src = mat_lock.mat;

	std::map<uint32_t, int> colors;
	uint32_t color;
	std::map<uint32_t, int>::iterator find_it;

	const int channels = src.channels();
	const int ncols = src.cols * channels;
	for (int row = 0; row < src.rows; row ++) {
		const uint32_t* data = src.ptr<uint32_t>(row);
		for (int col = 0; col < src.cols; col ++) {
			color = data[col];
			find_it = colors.find(color);
			if (find_it != colors.end()) {
				find_it->second ++;
			} else {
				colors.insert(std::make_pair(color, 1));
			}
		}
	}
	int result = 0;
	int max_pixels = 0;
	for (std::map<uint32_t, int>::const_iterator it = colors.begin(); it != colors.end(); ++ it) {
		if (it->second > max_pixels) {
			max_pixels = it->second;
			result = it->first;
		}
	}
	return result;
...
{
	VALIDATE(surf.get() != nullptr && roi.width > 0 && roi.height > 0, null_str);
	tsurface_2_mat_lock lock(surf);
	cv::Mat gray;

	cv::cvtColor(lock.mat(roi), gray, cv::COLOR_BGRA2GRAY);

	// use 3x3 Laplacian calulate
	// cv::Laplacian(gray, gray, CV_16S, 3);
	// cv::Laplacian(gray, gray, CV_8U, 3);
	cv::Mat laplacian_gray;
	cv::Laplacian(gray, laplacian_gray, gray.depth());

	double Iave = 0, Idelta = 0;
	for (int row = 0; row < laplacian_gray.rows; row ++) {
		const uint8_t* data = laplacian_gray.ptr<uint8_t>(row);
		for (int col = 0; col < laplacian_gray.cols; col ++) {
			int gray_value = data[col];
			Idelta = Idelta + (gray_value - Iave) * (gray_value - Iave);
		}
	}
	Idelta = Idelta / (laplacian_gray.cols * laplacian_gray.rows);
...
surface scale_surface(const surface& surf, int w, int h)
{
	...
	cv::Size czSize(w, h);
	cv::Mat dst_mat;
	cv::resize(mat_lock.mat, dst_mat, cv::Size(w, h));
...

	{
		...
		for (int row = 0; row < height; row ++) {
			for (int col = 0; col < width; col ++, pixel ++) {
				...
				hit = inside? point_in_rect(col, row, rect): !point_in_rect(col, row, rect);
...
surface cut_surface(const surface &surf, SDL_Rect const &r)
{
	...
	surface res = create_neutral_surface(r.w, r.h);
...
{
	// if returned mem is valid, free it by SDL_free.
	VALIDATE(surf.get(), null_str);
#if defined(__APPLE__) && TARGET_OS_IPHONE
	VALIDATE(format == img_png, "On iOS, now only support img_png.");
#else
	VALIDATE(format == img_png || format == img_jpg, "Now only support img_png or img_jpg.");
#endif
	uint8_t* mem = nullptr;
	int len = 0, ret;
	int channels = surf->format->BytesPerPixel > 1? surf->format->BytesPerPixel - 1: 1;
	while (!len) {
		const int size = surf->w * surf->h * channels;
		mem = (uint8_t*)SDL_malloc(size);
		SDL_RWops* dst = SDL_RWFromMem(mem, size);
		if (format == img_png) {
			ret = IMG_SavePNG_RW(surf, dst, 0);
		} else {
			// jpg
			surface surf2;
			if (has_alpha_le(surf, 254)) {
				surf2 = create_neutral_surface(surf->w, surf->h);
				fill_surface(surf2, 0xffffffff);
				sdl_blit(surf, nullptr, surf2, nullptr);
			} else {
				surf2 = surf;
			}
			int quality = 93; // normal is 100. I want reduce size, so use less quality.
			ret = IMG_SaveJPG_RW(surf2, dst, 0, quality);
		}
		len = dst->hidden.mem.here - dst->hidden.mem.base;
		SDL_RWclose(dst);
		if (ret != 0) {
			// if fail, reason is mem too small normaly.
			SDL_free(mem);
			
			// if channels is twice surf->format->BytesPerPixel, think fail.
			if (channels == surf->format->BytesPerPixel * 2) {
				mem = nullptr;
				len = 0;
				break;
			}
...
surface imread_mem(const void* data, int size)
{
	...
	SDL_RWops* src = SDL_RWFromMem((void*)data, size);
	surface surf = IMG_Load_RW(src, 0);
	SDL_RWclose(src);
	if (surf) {
...
{
	VALIDATE(long_size > 0 && short_size > 0, null_str);

	std::vector<SDL_Rect> rects;
	const int s1 = long_size;
	const int s2 = short_size;
	const SDL_Point O{rect.x, rect.y};
	// top
	rects.push_back(::create_rect(O.x, O.y, s1, s2));
	rects.push_back(::create_rect(O.x, O.y, s2, s1));
	rects.push_back(::create_rect(O.x + rect.w - s1, O.y, s1, s2));
	rects.push_back(::create_rect(O.x + rect.w - s2, O.y + s2, s2, s1));

	// bottom
	rects.push_back(::create_rect(O.x, O.y + rect.h - s1, s2, s1));
	rects.push_back(::create_rect(O.x, O.y + rect.h - s2, s1, s2));
	rects.push_back(::create_rect(O.x + rect.w - s2, O.y + rect.h - s1, s2, s1));
	rects.push_back(::create_rect(O.x + rect.w - s1, O.y + rect.h - s2, s1, s2));

	for (std::vector<SDL_Rect>::const_iterator it = rects.begin(); it != rects.end(); ++ it) {
		SDL_FillRect(target, &*it, color);
	}
}
...
double calculate_line_angle(bool image_view, const SDL_Point& O, const SDL_Point& A, const SDL_Point& B, bool& clockwise)
{
	...
	// reference to  https://blog.csdn.net/u012041523/article/details/25151063
	double angle_line = (dx1*dx2 + dy1 * dy2) / sqrt((dx1*dx1 + dy1 * dy1)*(dx2*dx2 + dy2 * dy2) + 1e-10);
	double angle = acos(angle_line) * 180 / CV_PI;

	// how to judge direction of ration, reference to https://blog.csdn.net/hy3316597/article/details/52732963
	// positive is counterclockwise, negative is clockwise.
	// When judging direction, need O as the origin, so need to use DELTA value.
	int val = dx1 * dy2 - dy1 * dx2;
	// int val = A.x * B.y - A.y * B.x;
	clockwise = val < 0;
	if (image_view) {
		// on image view: for y, top to bottom is increase.
		// else on math view, from top to bottom, y is decrease.
		clockwise = !clockwise;
	}
...
{
	VALIDATE(src.channels() == 1, null_str);
	VALIDATE(rotationAngle != 0, null_str);

	// reference https://blog.csdn.net/bigFatCat_Tom/article/details/94719156

	double radian = rotationAngle * CV_PI / 180.; // 弧度
	double a = sin(radian), b = cos(radian);
	int width = src.cols;
	int height = src.rows;
	int width_rotate = int(height * fabs(a) + width * fabs(b));
	int height_rotate = int(width * fabs(a) + height * fabs(b));
	// cv::Mat drawingRotation = cv::Mat::zeros(cv::Size(width_rotate, height_rotate), CV_8UC1);
	// cv::Mat drawingRotation = cv::Mat::zeros(cv::Size(width * 2, height * 2), CV_8UC1);
	// cv::Mat drawingRotation = cv::Mat::zeros(cv::Size(src.cols, src.rows), CV_8UC1);

	cv::Point2f center2;
	center2.x = float(src.cols / 2.0 - 0.5);
	center2.y = float(src.rows / 2.0 - 0.5);

	cv::Mat M1 = cv::getRotationMatrix2D(center2, rotationAngle, 1.0); //求得旋转矩阵
	// cv::Mat M1 = cv::getRotationMatrix2D(cv::Point(pointAdjust[0].x, pointAdjust[0].y), rotationAngle, 1.0); //求得旋转矩阵
	// cv::warpAffine(src, drawingRotation, M1, drawingRotation.size());

	cv::Point2f srcPoints1[3];
	cv::Point2f dstPoints1[3];
 
	srcPoints1[0] = cv::Point2i(0, 0);
	srcPoints1[1] = cv::Point2i(0, src.rows);
	srcPoints1[2] = cv::Point2i(src.cols, 0);
 
	dstPoints1[0] = cv::Point2i((width_rotate - width)/2 , (height_rotate - height)/2);
	dstPoints1[1] = cv::Point2i((width_rotate - width)/2 , src.rows + (height_rotate - height)/2);
	dstPoints1[2] = cv::Point2i(src.cols + (width_rotate - width)/2, (height_rotate - height)/2);
 
	cv::Mat M2 = getAffineTransform(srcPoints1, dstPoints1);
	
	M1.at<double>(0, 2) = M1.at<double>(0, 2) + M2.at<double>(0, 2);
	M1.at<double>(1, 2) = M1.at<double>(1, 2) + M2.at<double>(1, 2);
 
	// Over test, set up fill color does not have effect.
	// for example, test 255(white), but after warpAffine, ther are changed to 0(black).
	cv::Mat drawingRotation(width_rotate, height_rotate, CV_8UC1, cv::Scalar(0));

	cv::warpAffine(src, drawingRotation, M1, cv::Size(width_rotate, height_rotate));

	return drawingRotation;
}

SDL_Rect enlarge_rect(const SDL_Rect& src, int left, int right, int top, int bottom, int max_width, int max_height)
{
	VALIDATE(src.w >= 0 && src.h >= 0 && left >= 0 && right >= 0 && top >= 0 && bottom >= 0 && max_width > 0 && max_height > 0, null_str);
	int min_x = src.x - left;
	int max_xplush1 = src.x + src.w + right;
	int min_y = src.y - top;
	int max_yplush1 = src.y + src.h + bottom;

	if (min_x < 0) {
		min_x = 0;
	}
	if (max_xplush1 > max_width) {
		max_xplush1 = max_width;
	}
	if (min_y < 0) {
		min_y = 0;
	}
	if (max_yplush1 > max_height) {
		max_yplush1 = max_height;
	}
	return SDL_Rect{min_x, min_y, max_xplush1 - min_x, max_yplush1 - min_y};
}
