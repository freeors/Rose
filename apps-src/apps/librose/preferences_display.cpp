...
void set_fullscreen(CVideo& video, const bool ison)
{
	...
	tpoint size = preferences::landscape_size(video);
	if (video.isFullScreen() != ison) {
		const int flags = ison? SDL_WINDOW_FULLSCREEN: 0;
		tpoint orientation_size = gui2::twidget::orientation_swap_size(size.x, size.y);

		video.setMode(orientation_size.x, orientation_size.y, flags);
...
bool set_resolution(CVideo& video, const unsigned width, const unsigned height, bool resize_only, bool scene)
{
	// Make sure resolutions are always divisible by 4
	// res.first &= ~3;
	// res.second &= ~3;

	// from lock-screen to wake, maybe result resize_screen same size.
	SDL_Rect rect = frameTexture_rect();
	SDL_Log("set_resolution(%i, %i, %s), original size: %i x %i", width, height, scene? "scene": "dialog", rect.w, rect.h);

	const tpoint landscape_size = gui2::twidget::orientation_swap_size(width, height);
	VALIDATE(landscape_size.x >= game_config::min_allowed_width() && landscape_size.y >= game_config::min_allowed_height(), null_str);

	if (!resize_only) {
		const int flags = fullscreen()? SDL_WINDOW_FULLSCREEN : 0;
		video.setMode(width, height, flags);
	} else {
		VALIDATE(rect.w != width || rect.h != height, null_str);
		video.create_frameTexture(width, height, true);
	}
	if (scene) {
		display::require_change_resolution = true;
	}
	...
	preferences::set_str('x' + postfix, lexical_cast<std::string>(landscape_size.x / gui2::twidget::hdpi_scale));
	preferences::set_str('y' + postfix, lexical_cast<std::string>(landscape_size.y / gui2::twidget::hdpi_scale));
...
bool show_resolution_dialog(CVideo& video)
{
	...
	for (int n = 0; n < modes; n ++) {
		...
		if (mode.w >= game_config::min_allowed_width() && mode.h >= game_config::min_allowed_height()) {
	...
	set_resolution(video, normal_size.x, normal_size.y, false);
	return true;
}
