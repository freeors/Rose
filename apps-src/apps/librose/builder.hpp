/* $Id: builder.hpp 47608 2010-11-21 01:56:29Z shadowmaster $ */
/*
 ...
 * tmap object. On construction, it parses the configuration and extracts
...
{
public:
	enum SELECTOR {SELECTOR_MAP, SELECTOR_UNIT};
	 ...
	 * @param map			A properly-initialized tmap object representing
	 ...
	 *						and '.png' suffix
	 */
	terrain_builder(const std::string& id, const tmap* map);
	~terrain_builder();
	...
	const tmap& map() const { return *map_; }
	 ...
	 */
	const imagelist *get_terrain_at(const map_location &loc, const std::string &tod, bool background);

	SDL_Rect get_terrain_at(const map_location &loc);
	...
	struct rule_image {
		...
		rule_image(int layer, int x, int y, bool global_image = false, int cx = -1, int cy = -1) :
			layer(layer),
			basex(x),
			basey(y),
			variants(),
...
public:
	 ...
	 */
	void add_rule(building_rule* rules, building_rule &rule);
	 ...
	 */
	void add_rotated_rules(building_rule* rules, building_rule& tpl,
	 ...
	 * from the tmap and the building_rules_.
	 */
	void build_terrains();

	/**
	 * A pointer to the tmap class used in the current level.
	 */
	const tmap* map_;
	...
	static terrain_builder::building_rule* building_rules_;
