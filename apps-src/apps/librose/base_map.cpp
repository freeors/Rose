...
base_map::base_map(base_controller& controller, const tmap& gmap, bool consistent) 
...
base_unit* base_map::find_base_unit(const map_location& loc) const
{
	...
	if (!u) {
		u = coor_map_[index(loc.x, loc.y)].base;
	}
	return u;
}
...
{
	VALIDATE(!map_ && !map_size_ && !map_vsize_ && !coor_map_, "Error state!");
	map_size_ = w * h;
	map_ = (base_unit**)malloc(map_size_ * sizeof(base_unit*));
...
// u must be existed in map_.
void base_map::sort_map(base_unit& u)
{
	VALIDATE(u.map_index_ != UNIT_NO_INDEX, null_str);
	...
	// if u require from middle to last, i will equal to map_vsize_
...
void base_map::insert(const map_location loc, base_unit* u)
{
	...
	if (!loc.valid()) {
		err << "Trying to add " << u->name().str() << " at an invalid location; Discarding.";
		delete u;
		VALIDATE(false, err.str());
	}
	bool base = u->base();

	if ((base && coor_map_[index(loc.x, loc.y)].base) || (!base && coor_map_[index(loc.x, loc.y)].overlay)) {
...
void base_map::expand_coor_map(int w)
{
	...
	map_size_ = w * h_;
	base_unit** tmp = (base_unit**)malloc(map_size_ * sizeof(base_unit*));
	memset(tmp, 0, map_size_ * sizeof(base_unit*));
	if (map_vsize_) {
...
map_location base_map::conflict_calculate_loc(const base_unit& u)
{
	...
	map_location loc = disp.map_2_loc(rect.x, rect.y);
	int pitch = w_ * loc.y;
...
base_unit* base_map::find_base_unit(const int xscreen, const int yscreen) const
{
	const display& disp = get_controller().get_display();
	const SDL_Rect& max_rect = disp.main_map_view_rect();
	if (!point_in_rect(xscreen, yscreen, max_rect)) {
		return nullptr;
	}
	if (consistent_) {
		map_location loc = disp.screen_2_loc(xscreen, yscreen);
		return find_base_unit(loc);

	} else {
		int xmap = xscreen, ymap = yscreen;
		disp.screen_2_map(xmap, ymap);

		for (int i = map_vsize_ - 1; i >= 0; i --) {
			base_unit* u = map_[i];
			if (!u->hidden_) {
				const SDL_Rect& rect = u->get_rect();
...
bool base_map::erase2(base_unit* u, bool delete_unit)
{
	...
	for (std::set<map_location>::const_iterator itor = touch_locs.begin(); itor != touch_locs.end(); ++ itor) {
		...
		if (base) {
			coor_map_[index(loc.x, loc.y)].base = nullptr;
		} else {
			coor_map_[index(loc.x, loc.y)].overlay = nullptr;
		}
