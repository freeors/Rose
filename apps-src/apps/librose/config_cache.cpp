...
void get_partial_windows_cfg(const std::string& path, config& cfg)
{
...
bool teditor_::cfgs_2_cfg(const BIN_TYPE type, const std::string& name, const std::string& app, bool write_file, uint32_t nfiles, uint32_t sum_size, uint32_t modified, const std::map<std::string, std::string>& app_domains)
{
	...
	try {
		...
		} else if (type == APLT_DATA) {
			VALIDATE(name == std::string(BASENAME_APLT), null_str);
			const std::string bundleid = app;

			VALIDATE(aplts_config_.count(bundleid) != 0, null_str);

			get_partial_windows_cfg(working_dir_ + "/" + bundleid + "/gui", tmpcfg);

			if (write_file) {
				const std::string xwml_path = working_dir_ + "/" + bundleid + "/xwml";
				SDL_MakeDirectory(xwml_path.c_str());
				wml_config_to_file(xwml_path + "/" + BASENAME_APLT, tmpcfg, nfiles, sum_size, modified, app_domains);
			}
...
void teditor_::reload_extendable_cfg()
{
	...
	// why remark? reference to http://www.libsdl.cn/bbs/forum.php?mod=viewthread&tid=126&pid=436&page=1&extra=#pid436 #3 floor
...
void teditor_::get_wml2bin_desc_from_wml(const std::vector<BIN_TYPE>& system_bin_types)
{
	...
	// expand: applet
	size_t aplt_bin_index = 0;
	for (std::set<std::string>::const_iterator it = aplts_config_.begin(); it != aplts_config_.end(); ++ it) {
		const std::string& aplt = *it;
		bin_types.push_back(APLT_DATA);
	}

	// expand <data>/campaigns, and form [campaign].bin
	...
	for (std::vector<BIN_TYPE>::const_iterator itor = bin_types.begin(); itor != bin_types.end(); ++ itor) {
		...
		// and home of rose is cpu-sensitive.
		const bool use_should_method = false;

		bool skip_media_dir = true;
		if (type == TB_DAT) {
			const std::string& id = tbs[tb_index]["id"].str();
			short_paths = generate_tb_short_paths(id, tbs[tb_index]);
			skip_media_dir = false;

			data_tree_checksum(short_paths, dir_checksum, skip_media_dir);
			...
			if (use_should_method) {
				...
				if (!app_cfg[BINKEY_MACROS].empty()) {
					short_paths.push_back(working_dir_ + "/" + app_cfg[BINKEY_MACROS]);
				}
				short_paths.push_back(working_dir_ + "/" + app_cfg[BINKEY_PATH] + "/" + bin.id);
			} else {
				if (!bin.macros.empty()) {
			...
			std::set<std::string>::const_iterator hit_it = aplts_config_.begin();
			std::advance(hit_it, aplt_bin_index ++);
			const std::string& lua_bundleid = *hit_it;

			short_paths.push_back(lua_bundleid + "/gui");
			use_cfg_checksum = false;

			desc.bin_name = BASENAME_APLT;
			desc.app = lua_bundleid;

			bin_to_path = working_dir_ + "/" + lua_bundleid + "/xwml";

		} else if (type == GUI) {
			std::set<std::string> apps = apps_in_res(); 
			for (std::set<std::string>::const_iterator it = apps.begin(); it != apps.end(); ++ it) {
				const std::string& app = *it;
				if (use_should_method) {
					short_paths.push_back(working_dir_ + "/" + game_config::generate_app_dir(app) + "/gui");
				} else {
					short_paths.push_back(game_config::generate_app_dir(app) + "/gui");
				}
			}
			if (use_should_method) {
