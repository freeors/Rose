	...
	, special_separator_('\0')
{
	...
	for (size_t i = 0; (i < s); ++i) {
		nums_[i] = utils::to_int(components[i]);
	}
}
...
std::string version_info::str(bool with_special) const
{
	...
	if (with_special && !special_.empty()) {
...
bool version_info::is_rose_recommended() const
{
	// for example: 1.1.1-20190803
	if (nums_.size() != 3) {
		return false;
	}
	if (special_separator_ != '-') {
		return false;
	}
	...
	if (!utils::isinteger(special_)) {
		return false;
	}
	return special_.find("20") == 0;
}
...
tdomain::tdomain(const std::string& id)
{
	if (!id.empty()) {
		reset(id);
	}
...
void tdomain::reset(const std::string& id)
{
	if (id == id_) {
		return;
	}
	id_ = id;
	vnode_ = utils::split(id, '.');
	if (!valid()) {
		vnode_.clear();
	}
}