...
std::string hotkey_item::get_name() const
{
	...
	if (type_ == BY_CHARACTER) {
		...
		if (cmd_)
			str << "cmd+";
		str << SDL_GetKeyName(SDL_Keycode(keycode_));
	}
	return str.str();
}
...
{
	const std::string keyname = SDL_GetKeyName(SDL_Keycode(keycode));
	...
	if (character < 64 && ctrl) {
		if (shift)
			...
			character += 96;
		// Mapped to character $lexical_cast<std::string>(character)
	}
...
void save_hotkeys(config& cfg)
{
	...
	for(std::map<int, hotkey_item>::iterator it = hotkeys.begin(); it != hotkeys.end(); ++ it) {
		...
		if (key.get_type() == hotkey_item::BY_KEYCODE) {
			item["key"] = SDL_GetKeyName(SDL_Keycode(key.get_keycode()));
