...
void ocr_controller::recognize()
{
	...
	gui2::tprogress_default_slot slot(std::bind(&ocr_controller::did_recognize, this, _1), null_str);
...
bool ocr_controller::did_recognize(gui2::tprogress_& progress)
{
	...
	for (int at = 0; at < size; at ++) {
		...
		for (std::map<trect, int>::const_iterator it2 = line.chars.begin(); it2 != line.chars.end(); ++ it2, char_at ++) {
			...
			cv::Mat tmp;
			cv::threshold(gray(clip_rect), tmp, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
...
*/
			cv::Mat dest;
			cv::resize(tmp, dest, cv::Size(tflite_params_.width, tflite_params_.height));
