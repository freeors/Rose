/* $Id: map.hpp 47608 2010-11-21 01:56:29Z shadowmaster $ */
/*
...
#ifndef LIBROSE_MAP_H_INCLUDED
#define LIBROSE_MAP_H_INCLUDED
...
public:
	 ...
	 */
	tmap(const std::string& data); // throw(incorrect_map_format_error)
	...
	void overlay(const tmap& m, const config& rules, int x=0, int y=0, bool border=false);
	...
	/** [terrain_type] blocks in game_config. */
	static config terrain_types;
	...
	// support data is surface
	bool data_is_terrain() const { return !tex_.get(); }
	const texture tex() const { return tex_; }
	const surface surf() const { return surf_; }
