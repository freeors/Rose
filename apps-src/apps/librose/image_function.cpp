/* $Id: image_function.cpp 46940 2010-10-07 18:08:52Z alink $ */
/*
...
surface crop_function::operator()(const surface& src) const
{
	...
	if(area.x < 0) {
		// start X coordinate of SECTION function is negative - truncating to zero
		area.x = 0;
	}
	if(area.y < 0) {
		// start Y coordinate of SECTION function is negative - truncating to zero
		area.y = 0;
	}
...
{
	surface nsrc = clone_surface(src);
	surface nsurf = clone_surface(surf_);
...
surface scale_function::operator()(const surface& src) const
{
	...
	if(w <= 0) {
		if(w < 0) {
			// width of SCALE is negative - resetting to original width
		}
		w = old_w;
	}
	if(h <= 0) {
		if(h < 0) {
			// height of SCALE is negative - resetting to original height
		}
		h = old_h;
	}
