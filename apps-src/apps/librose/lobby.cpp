...
bool valid_logfile(tfile& lock, int* index_offset, int* index_size)
{
	...
	if (header->fourcc != SDL_FOURCC('L', 'O', 'G', '0')) {
		return false;
	}
	if (header->reserve0) {
		return false;
...
void verify_logfile_data(const std::string& ansifile)
{
	...
	for (std::set<thistory_log>::const_iterator it = history2_logs.begin(); it != history2_logs.end(); ++ it) {
		...
		do {
			...
			VALIDATE(!msg.empty() && utils::is_utf8str(msg.c_str(), msg.size()), "msg string!");
...
void save_temp_logfile()
{
	...
	header.fourcc = SDL_FOURCC('L', 'O', 'G', '0');
...
bool tlobby::thttp_sock::network_connect_dialog(bool quiet)
{
	...
	// block style.
	while (state_ != s_ready && !progress->task_cancelled()) {
		progress->show_slice();
	}
...
bool tlobby::thttp_sock::network_receive_dialog(int hidden_ms)
{
	...
	// block style.
	while ((!response_size_ || response_size_ != raw_data_vsize_) && !progress->task_cancelled()) {
		progress->show_slice();
	}
