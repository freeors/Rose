/* $Id: map_label.cpp 46186 2010-09-01 21:12:38Z silene $ */
/*
...
/*
	...
	if(!tmp_color.empty()) {
		...
		if(!temp_rgb.empty()) {
			color = uint32_to_color(temp_rgb[0]);
		}
	}
	color_ = color;
*/
...
void terrain_label::draw()
{
	...
	const int xloc = (parent_->disp().loc_2_screen_x(loc_) +
			parent_->disp().loc_2_screen_x(loc_nextx)*2)/3;
	const int yloc = parent_->disp().loc_2_screen_y(loc_nexty) - font::SIZE_DEFAULT;
	...
	flabel.set_clip_rect(parent_->disp().main_map_widget_rect());
