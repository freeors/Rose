...
typedef void (*fget_mix_data)(Uint8* stream, int len, void*);
class thook_get_mix_data
{
public:
	static fget_mix_data current_hook;
	static void* current_contex;

	thook_get_mix_data(fget_mix_data hook, void* contex);
	~thook_get_mix_data();

private:
	fget_mix_data original_;
	void* original_contex_;
};
