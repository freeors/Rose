...
public:
	...
	enum BIN_TYPE {BIN_MIN = 0, MAIN_DATA = BIN_MIN, GUI, LANGUAGE, BIN_SYSTEM_MAX = LANGUAGE, 
		TB_DAT, SCENARIO_DATA, APLT_DATA, EXTENDABLE};
	struct wml2bin_desc {
		wml2bin_desc();
		std::string bin_name;
		std::string app;
...
protected:
	...
	config tbs_config_;
	std::set<std::string> aplts_config_;
	config_cache& cache_;
