...
{
	tcallback_lock lock(false, std::bind(&ttask::tsubtask_copy::did_fail_rollback, this, _1));
	...
	if (is_directory(src_path)) {
		for (std::vector<tres>::const_iterator it = copy_res_.begin(); it != copy_res_.end(); ++ it) {
			...
			if (r.type == res_file || r.type == res_dir) {
				...
				fok = copy_root_files(src, dst, &files, std::bind(&ttask::app_filter, &task_, *this, _1, false));
				if (!has_resolved) {
