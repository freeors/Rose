/* $Id: terrain_translation.cpp 47608 2010-11-21 01:56:29Z shadowmaster $ */
/*
   Copyright (C) 2006 - 2010 by Mark de Wever <koraq@xs4all.nl>
...
const t_terrain IMG_MAP = string_to_number_("_img^_img");
	...
	while(offset < str.length()) {
		...
		if(starting_position != -1) {
			if(starting_positions.find(starting_position) != starting_positions.end()) {
				...
				// Starting position $starting_position is redefined.
		...
		if(pos_separator == std::string::npos || utils::isnewline(str[pos_separator])) {
			...
			if(y == 0) {
				...
				if((x + 1) != width ) {
					// Map not a rectangle error occurred at line offset $y position offset $x
					throw error("Map not a rectangle.");
				}
				if (y > max_map_size()) {
					// Map size exceeds limit (y > " $max_map_size())
			...
			offset = pos_separator + 1;
			if (x > max_map_size()) {
				// Map size exceeds limit (x > $max_map_size())
