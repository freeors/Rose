...
hex_display::hex_display(base_controller& controller, CVideo& video, const tmap* map, size_t num_reports, int initial_zoom)
	: display(game_config::tile_hex, controller, video, map, num_reports, initial_zoom)
{
}

const SDL_Rect& hex_display::main_map_rect() const
{
...
const map_location hex_display::map_2_loc(int x, int y) const
{
	// adjust for the border
	x -= static_cast<int>(border_.size * hex_width());
	y -= static_cast<int>(border_.size * zoom());
	...
	if(nearest_hex != NULL) {
		...
		const int centerx = (loc_2_screen_x(res) - main_map_view_rect().x + xpos_) + zoom()/2 - hex_width();
		const int centery = (loc_2_screen_y(res) - main_map_view_rect().y + ypos_) + zoom()/2 - zoom();
...
int hex_display::loc_2_screen_x(const map_location& loc) const
{
	return static_cast<int>(main_map_view_rect().x + (loc.x + border_.size) * hex_width() - xpos_);
}

int hex_display::loc_2_screen_y(const map_location& loc) const
{
	return static_cast<int>(main_map_view_rect().y + (loc.y + border_.size) * zoom_ - ypos_ + (is_odd(loc.x) ? zoom_/2 : 0));
}

void hex_display::loc_2_screen(const map_location& loc, int& x, int& y) const
{
	const SDL_Rect& rect = main_map_view_rect();
	x = static_cast<int>(rect.x + (loc.x + border_.size) * hex_width() - xpos_);
	y = static_cast<int>(rect.y + (loc.y + border_.size) * zoom_ - ypos_ + (is_odd(loc.x) ? zoom_/2 : 0));
}
...
void hex_display::draw_border(const map_location& loc, const int xpos, const int ypos)
{
	...
	if(loc.x == -1 && loc.y == -1) { // top left corner
		...
		if(loc.x%2 == 0) {
			drawing_buffer_add(LAYER_BORDER, loc,
				border_.corner_image_top_right_odd, image::SCALED_TO_ZOOM, xpos, ypos + zoom_/2, 0, 0);
		} else {
			drawing_buffer_add(LAYER_BORDER, loc,
		...
		if(loc.x%2 == 1) {
			...
			drawing_buffer_add(LAYER_BORDER, loc,
				border_.corner_image_bottom_right_odd, image::SCALED_TO_ZOOM, xpos, ypos, 0, 0);
		}
	...
	// Now handle the sides:
		...
		if(loc.x%2 == 1) {
			drawing_buffer_add(LAYER_BORDER, loc,
				border_.border_image_top_even, image::SCALED_TO_ZOOM, xpos, ypos, 0, 0);
		} else {
			drawing_buffer_add(LAYER_BORDER, loc,
		...
		if(loc.x%2 == 1) {
			drawing_buffer_add(LAYER_BORDER, loc,
				border_.border_image_bottom_even, image::SCALED_TO_ZOOM, xpos, ypos, 0, 0);
		} else {
			drawing_buffer_add(LAYER_BORDER, loc,
...
{
	return - (border_.size + 0.25) * zoom() - (map_out_rect.h - map_rect.h) / 2;
}