	...
	for(walker w(*this); !w.eos(); w.next()) {
		...
		if(w.translatable()) {
			if(w.countable()) {
				...
				translated_value_ +=
					dsngettext(w.textdomain().c_str(), part.c_str(), plural.c_str(), w.count());
			} else {
				translated_value_ +=
					dsgettext(w.textdomain().c_str(), part.c_str());
			}
		} else {
...
std::vector<t_string_base::trans_str> t_string_base::valuex() const
{
	std::vector<trans_str>	t;
	trans_str				ti;
