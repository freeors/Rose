...
private:
        ...
        void display_float(const map_location& location, const std::string& text) const{
                display::get_singleton()->float_label(location,text,255,0,0);
        }
};
...
public:
	...
	variant execute(const formula_callable& variables
		...
		if(offset < 0) {
			offset += result.size();
			if(offset < 0) {
				// [substring] Offset '$args()[1]->evaluate(variables, fdb).as_int()' results in a negative start in string '$result' and is reset at the beginning of the string.

				offset = 0;
			}
		} else {
			if(static_cast<size_t>(offset) >= result.size()) {
				// [substring] Offset '$offset' is larger than the size of '$result' and results in an empty string.
		...
		if(args().size() > 2) {
			...
			if(size < 0) {
				// [substring] Size is negative an empty string is returned.
...
variant formula_function_expression::execute(const formula_callable& variables, formula_debugger *fdb) const
{
	...
	if(precondition_) {
		if(!precondition_->evaluate(callable,fdb).as_bool()) {
			// FAILED function precondition for function '$formula_->str()' with arguments: 
			for(size_t n = 0; n != arg_names_.size(); ++n) {
				//  arg " << (n+1) : $args()[n]->evaluate(variables,fdb).to_debug_string()
			}
		}
	}
