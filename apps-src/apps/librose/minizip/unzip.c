...
local int unzlocal_getByte(SDL_RWops* filestream, int *pi)
{
    unsigned char c;
...
*/
local int unzlocal_getShort(voidpf filestream, uLong *pX)
{
    uLong x ;
    int i;
    int err;
...
local int unzlocal_getLong(voidpf filestream, uLong *pX)
{
    uLong x ;
    int i;
    int err;
...
/*
  Locate the Central directory of a zipfile (at the end, just before
    the global comment)
*/
local uLong unzlocal_SearchCentralDir(SDL_RWops* filestream)
{
    unsigned char* buf;
    uLong uSizeFile;
    uLong uBackRead;
    ...
    ZSEEK(filestream,0,ZLIB_FILEFUNC_SEEK_END);
    ...
    while (uBackRead<uMaxBack)
    {
        ...
        ZSEEK(filestream,uReadPos,ZLIB_FILEFUNC_SEEK_SET);
...
unzFile unzOpen2(const char *path)
{
    ...
    us.filestream = SDL_RWFromFile(path, "rb");
    if (us.filestream == NULL) {
		SDL_Log("unzOpen2, open %s fail", path);
        return NULL;
	}
    ...
    ZSEEK(us.filestream, central_pos,ZLIB_FILEFUNC_SEEK_SET);
    ...
    uLong commentBufferSize;
{
    ...
    if ((err==UNZ_OK) && (extraField!=NULL))
    {
        ...
        if (lSeek!=0) {
            ZSEEK(s->filestream,lSeek,ZLIB_FILEFUNC_SEEK_CUR);
            lSeek=0;
		}
        if ((file_info.size_file_extra>0) && (extraFieldBufferSize>0))
            if (ZREAD(s->filestream,extraField,uSizeRead)!=uSizeRead)
                err=UNZ_ERRNO;
    ...
    if ((err==UNZ_OK) && (szComment!=NULL))
    {
        ...
        if ((file_info.size_file_comment>0) && (commentBufferSize>0))
            if (ZREAD(s->filestream,szComment,uSizeRead)!=uSizeRead)
                err=UNZ_ERRNO;
...
/*
  ...
  return UNZ_OK if there is no problem.
*/
int rose_unzGetCurrentFileInfo(unzFile file,
    unz_file_info *pfile_info,
    char *szFileName,
    uLong fileNameBufferSize,
    void *extraField,
...
int unzLocateFile(unzFile file, const char *szFileName, int iCaseSensitivity)
{
    ...
    while (err == UNZ_OK)
    {
        ...
        err = rose_unzGetCurrentFileInfo(file,NULL,
    ...
    uInt  *psize_local_extrafield;
{
    ...
    *psize_local_extrafield = 0;
    ...
    ZSEEK(s->filestream,s->cur_file_info_internal.offset_curfile +
                                s->byte_before_the_zipfile,ZLIB_FILEFUNC_SEEK_SET);
...
#    ifndef NOUNCRYPT
    if (password != NULL)
    {
        ...
        ZSEEK(s->filestream,
                  ...
                  SEEK_SET);
        if(ZREAD(s->filestream,source, 12)<12)
            return UNZ_INTERNALERROR;
    ...
    unsigned len)
{
    ...
    while (pfile_in_zip_read_info->stream.avail_out>0)
    {
            ...
            (pfile_in_zip_read_info->rest_read_compressed>0))
        {
            ...
            if (uReadThis == 0)
                return UNZ_EOF;
            ZSEEK(pfile_in_zip_read_info->filestream,
                      pfile_in_zip_read_info->pos_in_zipfile +
                         pfile_in_zip_read_info->byte_before_the_zipfile,
                         ZLIB_FILEFUNC_SEEK_SET);
            if (ZREAD(pfile_in_zip_read_info->filestream,
...
int unzGetGlobalComment(unzFile file, char *szComment, uLong uSizeBuf)
{
    ...
    ZSEEK(s->filestream,s->central_pos+22,ZLIB_FILEFUNC_SEEK_SET);

    if (uReadThis>0)
    {
      *szComment='\0';
      if (ZREAD(s->filestream,szComment,uReadThis)!=uReadThis)
        return UNZ_ERRNO;
    }
