...
*/
#ifndef LIBROSE_CONFIG_H_INCLUDED
#define LIBROSE_CONFIG_H_INCLUDED
...
{
	extern const bool gui_as_mobile;
	extern const bool mobile;
	extern const int os;
	extern const bool os_kos;
	extern std::string sn;
	// SDL_GetNotchSize. x => statusbar_height, y => corner_height
	extern int statusbar_height;
	extern int navigation_height;
	// for example iPhone, bottom corner height. this value is SDL_GetNotchSize(window).y.
	// why not use SDL_GetNotchSize(window).y directly? --SDL_GetNotchSize is function call.
	// if want to use corner_height, must be after first SDL_CreateWindow.
	extern int corner_height;
	extern bool fullscreen;
	extern std::string app;
	extern std::string app_channel;
	extern std::string app_dir; // app-<kdesktop>
	extern std::string app_path; // game_config::path + /app-<kdesktop>
	extern int longpress_time;
	extern int min_longblock_threshold_s;
	...
	extern version_info rose_version;
	extern version_info version;
	...
	struct tbbs_server {
		...
		void split_furl(const std::string& furl, tbbs_server* special);
		std::string generate_furl();
	...
	extern int equation_of_time;

	extern std::string shroud_prefix;
	extern std::string fog_prefix;

	void init(const std::string& app, const std::string& channel);
	std::string extract_app_from_app_dir(const std::string& dir);
	std::string generate_app_dir(const std::string& app);
	std::string get_app_msgstr(const std::string& app);
