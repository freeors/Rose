...
public:
	...
	std::vector<trans_str> valuex() const;
private:
	std::string value_;
...
public:
	...
	std::vector<t_string_base::trans_str> valuex() const { return get().valuex(); }
...
inline std::ostream& operator<<(std::ostream& os, const t_string& str) { return os << str.str(); }
