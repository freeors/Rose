...
class preprocessor_data: preprocessor
{
	...
	bool in_good_get() const { return current_at_ < in_len_; }
	int in_get()
	{ 
		if (current_at_ >= in_len_) {
			return EOF;
		}
		return in_[current_at_ ++]; 
	}
...
{
	throw twml_exception("don't compare tokens with characters");
}
	...
	end_()
{
	if (is_directory(name)) {
		...
		get_files_in_dir(name, &files_, NULL, true, true, true);
	} else {
		increment_preprocessor_progress(name, true);

		tfile lock(name, GENERIC_READ, OPEN_EXISTING);
		int fsize = lock.valid()? posix_fsize(lock.fp): 0;
		if (fsize) {
			char* in = (char*)malloc(fsize);
			posix_fread(lock.fp, in, fsize);

			new preprocessor_data(t, in, fsize, "", get_short_wml_path(name),
				1, extract_directory(name) + "/", t.textdomain_, NULL);
		}
		if (preprocessor_checksum && lock.valid()) {
			SDL_dirent st;
			SDL_GetStat(name.c_str(), &st);
			if (st.mtime > preprocessor_checksum->modified) {
				preprocessor_checksum->modified = st.mtime;
			}
			preprocessor_checksum->sum_size += fsize;
			preprocessor_checksum->nfiles ++;
		}
	}
	pos_ = files_.begin();
	end_ = files_.end();
...
preprocessor_data::preprocessor_data(preprocessor_streambuf &t,
	char* in, int in_len, std::string const &history, std::string const &name, int linenum,
...
std::string preprocessor_data::read_word()
{
	...
	for(;;) {
		int c = in_peek();
		if (c == EOF || c == preprocessor_streambuf::traits_type::eof() || utils::portable_isspace(c)) {
			return res;
		}
		in_get();
	...
	if (c == '\376') {
		...
		if (command == "define") {
			...
			if (!skipping_) {
				...
				// defining macro $symbol ($location get_location(target_.location_))
			}
		} else if (command == "ifdef") {
			skip_spaces();
			std::string const &symbol = read_word();
			bool found = target_.defines_->count(symbol) != 0;
			// testing for macro 'symbol': (found ? "defined" : "not defined");
			...
			if (!skipping_) {
				...
				// undefine macro $symbol ($location " << get_location(target_.location_))
			}
			...
			if (!skipping_) {
				...
				// #warning: $message at $linenum_ ' ' $get_location(target_.location_)
			} else {
				// Skipped a warning
			}
		} else
		...
		if (c == '(') {
			...
			else if ((macro = target_.defines_->find(symbol)) != target_.defines_->end())
			{
				...
				int in_len = val.value.size();
				char* in = (char*)malloc(in_len);
				memcpy(in, val.value.c_str(), in_len);
				...
				pop_token();
				const std::string dir = extract_directory(val.location.substr(0, val.location.find(' '))) + "/";
				if (!slowpath_) {
					// substituting macro $symbol
					new preprocessor_data(target_, in, in_len, val.location, "",
					...
					{	
						std::istream in2(buf);
						new preprocessor_data(*buf, in, in_len, val.location, "",
						                      val.linenum, dir, val.textdomain, defines);
						res << in2.rdbuf(); 
					}
					delete buf;
				...
				// Macro definition not found for $symbol, attempting to open as file.
				pop_token();
				std::string prefix;
