...
static bool up_can_result_swipe = true;
tdisable_up_result_swipe::tdisable_up_result_swipe()
{
	VALIDATE(up_can_result_swipe, null_str);
	up_can_result_swipe = false;
}

tdisable_up_result_swipe::~tdisable_up_result_swipe()
{
	VALIDATE(!up_can_result_swipe, null_str);
	up_can_result_swipe = true;
}
...
tevent_dispatcher::tevent_dispatcher(const bool scene, const bool auto_join) 
	...
	, mousebuttondown_pending_(false)
{
	if (auto_join) {
...
void tevent_dispatcher::handle_event(const SDL_Event& event)
{
	...
	case SDL_FINGERDOWN:
		...
		// SDL_Log("%i, SDL_FINGERDOWN, (%i, %i)", now, x, y);
	...
	case SDL_FINGERMOTION:
		{
			...
			// SDL_Log("%i, SDL_FINGERMOTION, (%i, %i), delta(%i, %i)", now, x, y, dx, dy);
	...
	case SDL_FINGERUP:
		// x = event.tfinger.x * screen_width2;
		// y = event.tfinger.y * screen_height2;
		// SDL_Log("%i, SDL_FINGERUP, (%i, %i)", now, x, y);
	...
	case SDL_MOUSEBUTTONDOWN:
		...
		// SDL_Log("(%s)SDL_MOUSEBUTTONDOWN[#%i], (%i, %i), last_motion(%i, %i)", scene_? "scene": "gui2", event.button.button, event.button.x, event.button.y, last_motion_coordinate_.x, last_motion_coordinate_.y);

		if (event.button.button == SDL_BUTTON_LEFT) {
			send_extra_mouse_up(event);
			mousebuttondown_pending_ = true;
		}
	...
	case SDL_MOUSEBUTTONUP:
		if (event.button.button == SDL_BUTTON_LEFT) {
			// below handle maybe block, set it at once.
			mousebuttondown_pending_ = false;
		}
		if (up_can_result_swipe && fingers_.size() == 1) {
			...
			if (!finger.motions.empty()) {
				// use last motion's coordinate. signal_handler_sdl_wheel use it to call signal_handler_sdl_mouse_motion.
				x2 = finger.motions.back().x;
				y2 = finger.motions.back().y;
			}
		...
		// SDL_Log("(%s)SDL_MOUSEBUTTONUP[#%i], (%i, %i)", scene_? "scene": "gui2", event.button.button, event.button.x, event.button.y);
		{
			...
			if (!multi_gestures2) {
				...
				// SDL_Log("(%s)SDL_MOUSEBUTTONUP, multi_gestures2 is true, send multi_gestures_up", scene_? "scene": "gui2");
	...
	case SDL_MOUSEMOTION:
		...
		// SDL_Log("SDL_MOUSEMOTION, xy(%i, %i), ref(%i, %i)", event.motion.x, event.motion.y, event.motion.xrel, event.motion.yrel);
		if (!multi_gestures()) {
	...
	case SDL_MOUSEWHEEL:
		// I think mobile device cannot send it. but rdp maybe result it?
		// VALIDATE(!game_config::mobile, null_str);

		// SDL_Log("SDL_MOUSEWHEEL, wheel(%i, %i), which(%i), direction: %i", event.wheel.x, event.wheel.y, event.wheel.which, event.wheel.direction);
...
{
	// SDL_Log("(%s)mouse_leave_window", scene_? "scene": "gui2");
...
void tevent_dispatcher::send_extra_mouse_up(const SDL_Event& event)
{
	VALIDATE(event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_LEFT && event.button.state == SDL_PRESSED, null_str);

	if (!mousebuttondown_pending_) {
		return;
	}
	SDL_Log("(%s)now pending MOUSEBUTTONUP event, require send extra mouse up", scene_? "scene": "gui2");
...
void tevent_dispatcher::send_extra_mouse_motion(const int x, const int y)
{
	...
	SDL_Log("(%s)last_motion(%i, %i) isn't desire coordiante(%i, %i), require send extra mouse motion", scene_? "scene": "gui2", last_motion_coordinate_.x, last_motion_coordinate_.y, x, y);
...
void tevent_dispatcher::send_sdl_event(const SDL_Event& e)
{
	SDL_Log("(%s)tevent_dispatcher::send_sdl_event, e.type: 0x%x", scene_? "scene": "gui2", e.type);
	handle_event(e);
}
...
static uint32_t last_input_ticks = 0;
int no_input_seconds()
{
	uint32_t now = SDL_GetTicks();
	return now >= last_input_ticks? (now - last_input_ticks) / 1000: 0;
}
...
void dump_events(const std::vector<SDL_Event>& events)
{
	...
	for (std::map<int, int>::const_iterator it = dump.begin(); it != dump.end(); ++ it) {
		...
		SDL_snprintf(buf, sizeof(buf), "(type: 0x%x, count: %i)", it->first, it->second);
		ss << buf;
	}
...
/*
	if (!instance->foreground() && instance->silent_background()) {
		while (SDL_PollEvent(&temp_event)) {}
		instance->pump(true);
		// instance->check_ip_slice(true);
		// In silent state, there are few things to do, giving a delay and let other threads more cpu.
		SDL_Delay(100);
		return;
	}
*/
	...
	while (SDL_PollEvent(&temp_event)) {
		...
		if (!begin_ignoring && temp_event.type == SDL_WINDOWEVENT) {
			...
			// SDL_Log("events::pump, [1] ignore event: 0x%x, begin_ignoring: %i", temp_event.type, begin_ignoring);
			continue;
		}
	...
	if (events.size() > 10) {
		SDL_Log("------waring!! events.size(): %u, last_event: %x", events.size(), events.back().type);
		dump_events(events);
	}

	if (!keyboard::keys.empty()) {
		SDL_Event e;
		for (std::vector<keyboard::tkey>::const_iterator it = keyboard::keys.begin(); it != keyboard::keys.end(); ++ it) {
			const keyboard::tkey& key = *it;
			memset(&e, 0, sizeof(e));
			if (!key.utf8.empty()) {
				e.type = SDL_TEXTINPUT;
				e.text.windowID = 1;
				strcpy(e.text.text, key.utf8.c_str());
				
			} else {
				e.type = SDL_KEYDOWN;
				e.key.windowID = 1;
				e.key.state = 1;
				e.key.keysym.sym = key.ch;
			}
			events.push_back(e);
		}
		keyboard::keys.clear();
	...
	for (int i = 1; i < begin_ignoring; ++i) {
		if (ev_it->type >= INPUT_MASK_MIN && ev_it->type <= INPUT_MASK_MAX) {
			...
			// SDL_Log("events::pump, [2] ignore event: 0x%x, begin_ignoring: %i", ev_it->type, begin_ignoring);
	...
	for (ev_it = events.begin(); ev_it != ev_end; ++ev_it){
		...
		if (event.type >= INPUT_MASK_MIN && event.type <= INPUT_MASK_MAX) {
		...
		switch (event.type) {
			...
			case SDL_WINDOWEVENT:
				if (event.window.event == SDL_WINDOWEVENT_ENTER || event.window.event == SDL_WINDOWEVENT_FOCUS_GAINED) {
					...
					// don't use (event.window.event == SDL_WINDOWEVENT_RESIZED).
					// ---why? some resize method cann't trigger SDL_WINDOWEVENT_RESIZED, for example SDL_SetWindowSize.
					SDL_Log("events::pump, SDL_WINDOWEVENT[SDL_WINDOWEVENT_SIZE_CHANGED], (%i x %i)", event.window.data1, event.window.data2);
					if (game_config::os == os_windows) {
						// both android and ios must not conside this event.
						instance->handle_resize_screen(event.window.data1, event.window.data2);
					}
				}
...
void set_visible(bool visible, int reason)
{
	if (preferences::use_rose_keyboard()) {
...
/*
			// if don't show both soft/rose keyboard, will not show key in rose's widget.
			if (!SDL_CanScreenKeyboardShown()) {
				// for anndroid, current is using a physical keyboard.
				// config.keyboard == Configuration.KEYBOARD_QWERTY && config.hardKeyboardHidden != Configuration.HARDKEYBOARDHIDDEN_YES
				return;
			}
*/
			windows.back()->show_keyboard(reason);
			
		} else {
			if (!keyboard_visible) {
				return;
			}
			VALIDATE(!windows.empty(), null_str);
			windows.back()->hide_keyboard(reason);
		}
		keyboard_visible = visible;

	} else if (game_config::mobile) {
		if (visible) {
			SDL_StartTextInput();
		} else {
			SDL_StopTextInput();
...
tkey::tkey(const std::string& utf8, int ch)
	: utf8(utf8)
	, ch(ch)
{
	if (!utf8.empty()) {
		VALIDATE(ch == nposm && utf8.size() < SDL_TEXTINPUTEVENT_TEXT_SIZE - 1, null_str);
	} else {
...
void input_internal(const std::string& utf8, int ch)
{
	const int max_buffed_keys = 5;
	if (keys.size() >= max_buffed_keys) {
		keys.erase(keys.begin());
	}

	keys.push_back(keyboard::tkey(utf8, ch));
}

void input(const std::string& utf8)
{
	input_internal(utf8, nposm);
}

void input(int ch)
{
	input_internal(null_str, ch);
}

} //end keyboard namespace
