...
void base_unit::calculate_draw_locs()
{
	...
	if (units_.consistent()) {
		...
		map_location lt = disp.map_2_loc(rect_.x, rect_.y);
		map_location rb = disp.map_2_loc(rect_.x + rect_.w - 1, rect_.y + rect_.h - 1);
...
{
	VALIDATE(!SDL_RectEmpty(&rect), "rect must not be empty!");
...
void base_unit::draw_unit(display& disp)
{
	if (refreshing_) {
		return;
	}
	VALIDATE(loc_.valid(), null_str);
	...
	int ysrc;
	if (consistent()) {
		disp.loc_2_screen(loc_, xsrc, ysrc);

	} else {
		xsrc = rect_.x;
		ysrc = rect_.y;
		disp.map_2_screen(xsrc, ysrc);
	}

	app_draw_unit(xsrc, ysrc);
}
