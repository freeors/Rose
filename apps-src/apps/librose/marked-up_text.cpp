...
std::string word_wrap_text(const std::string& unwrapped_text, int font_sz, int style, int max_width, int max_lines, const int attention_min_chars)
{
	...
	while (1) {
		...
		if (current_word == "\n") {
			...
			// maybe appear word_width > max_width.
			// for example, it is chinese-english unwrapped_text, font_sz = 20.
			// first is english, word_width is 13, so word_width < max_width.
			// second is chinese, word_width is 24, so word_width > max_width.
