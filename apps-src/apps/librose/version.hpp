/* $Id: version.hpp 52533 2012-01-07 02:35:17Z shadowmaster $ */
/*
...
public:
	...
	bool is_rose_recommended() const;
	...
	std::string str(bool with_special) const; /**<
...
{
public:
	tdomain(const std::string& id);

	const std::string& id() const { return id_; }
	const std::string& node(int at) const 
	{
		return vnode_[at];
	}
	bool valid() const 
	{
		return vnode_.size() == 3;
	}
	int size() const { return id_.size(); }

	void reset(const std::string& id);

private:
	std::string id_;
	std::vector<std::string> vnode_;
};
