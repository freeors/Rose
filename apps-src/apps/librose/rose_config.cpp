...
namespace game_config
{
	...
	int corner_height = nposm;
	bool fullscreen = false; // must set false, same as SDL_Activity.java's onCreate
	std::string app;
	std::string app_channel;
	std::string app_dir; // app-<kdesktop>
	std::string app_path; // game_config::path + /app-<kdesktop>
	int longpress_time = nposm;
	int min_longblock_threshold_s = 30; // unit: second
	int min_width = MIN_WIN_WIDTH;
	int min_height = MIN_WIN_HEIGHT;
	int base_income = 2;
	...
	std::string game_title,
			...
			checked_menu = "buttons/checkbox_pressed.png",
			unchecked_menu = "buttons/checkbox.png",
			wml_menu = "buttons/WML_custom.png",
			level,
			ellipsis,
			missing;
	} //images
	...
	namespace terrain {
		...
		short_grid_top = "grid_top.png",
		short_grid_bottom = "grid_bottom.png",
		mouseover,
		selected,
		editor_brush,
		unreachable,
...
{
	// return: east(positive), west(negative)

	time_t utc = time(nullptr);
	struct tm* tm = gmtime(&utc);

	struct tm tm_clone;
	memcpy(&tm_clone, tm, sizeof(struct tm));
	
	// 1. time(nullptr) ---> time_t |utc domain
	// 2. gmtime(&utc) ---> tm | utc domain
	// 3. mktime ---> think tm as local tm, result is 'fake' utc. 
	// utc(local) - fake_utc(utc)
	time_t fake_utc = mktime(&tm_clone);
	return utc - fake_utc;
}

void init(const std::string& _app, const std::string& channel)
{
	VALIDATE(!_app.empty() && _app.size() <= MAXLEN_APP, null_str);
	app = _app;
	app_channel = channel;
	...
	gui2::SCROLLBAR_BEST_SIZE = 3 * gui2::twidget::hdpi_scale;
	gui2::SCROLLBAR2_BEST_SIZE = 8 * gui2::twidget::hdpi_scale;

	//
	// status bar
	//
	// Only here set statusbar_height. in gui2, to the widget when the use of the fullscreen. 
	// gui/widgets/helper.cpp
	//   variable.add("statusbar_height", variant(game_config::fullscreen? 0: game_config::statusbar_height / twidget::hdpi_scale));
	// value of SDL_GetStatusBarHeight() returned is include hdpi_scale.
	// Note: If current is in fullscreen mode, SDL_GetStatusBarHeight() maybe return 0.
	if (game_config::os == os_ios || game_config::os == os_android) {
		statusbar_height = SDL_GetStatusBarHeight(nullptr);
	} else {
		VALIDATE(statusbar_height == 0, null_str);;
	}
	navigation_height = SDL_GetNavigationHeight();
	SDL_Log("twidget::hdpi_scale: %i statusbar_height: %i navigation_height: %i", gui2::twidget::hdpi_scale, statusbar_height, navigation_height);

	// white font. android only support wihte font.
	SDL_SetStatusBarColor(SDL_TRUE);
...
std::string get_app_msgstr(const std::string& _app)
{
	// if _app is empty, think want get game_config::app's msgstr.
	const std::string app2 = _app.empty()? app: _app;
	const std::string app_lib_mo = app2 + "-lib";
	return dgettext(app_lib_mo.c_str(), app2.c_str());
}

int min_allowed_width()
{
	return min_width * gui2::twidget::hdpi_scale;
}

int min_allowed_height()
{
	return min_height * gui2::twidget::hdpi_scale;
}
...
void load_config(const config* cfg)
{
	...
	rose_version = v["version"].str();
	VALIDATE(rose_version.is_rose_recommended(), null_str);
...
void tbbs_server::from_cfg(const config& cfg)
{
	if (!cfg.empty()) {
		game_config::bbs_server.name = cfg["name"].str();
		// address = www.leagor.com:80
		...
		// url = /bbs or /
		game_config::bbs_server.url = cfg["url"].str();
...
void tbbs_server::split_furl(const std::string& furl, tbbs_server* special)
{
	tbbs_server& result = special? *special: *this;
	std::string host_port;

	int pos = furl.find_first_of("/");
	if	(pos == std::string::npos) {
		host_port = furl;
		result.url = "/";
	} else {
		host_port = furl.substr(0, pos);
		result.url = furl.substr(pos);
	}

	// host_port = www.leagor.com:80
	pos = host_port.find_first_of(":");
	if	(pos == std::string::npos) {
		result.host = host_port;
		result.port = 80;
	} else {
		result.host = host_port.substr(0, pos);
		result.port = utils::to_int(host_port.substr(pos + 1));
		if (!result.port) {
			result.port = 80;
		}
	}
...
std::string tbbs_server::generate_furl()
{
	std::stringstream ss;
	ss << host;
	if (port != 80) {
