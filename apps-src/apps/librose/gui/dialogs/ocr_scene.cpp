...
void tocr_scene::signal_handler_longpress_name(bool& halt, const tpoint& coordinate, ttoggle_panel& row)
{
	...
	window_->start_drag(coordinate, std::bind(&ocr_controller::did_drag_mouse_motion, &controller_, _1, _2, std::ref(*window_)),
		std::bind(&ocr_controller::did_drag_mouse_leave, &controller_, _1, _2, _3));
}
