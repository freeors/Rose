...
void tphoto_selector::pre_show()
{
	...
	navigation.set_did_item_changed(std::bind(&tphoto_selector::did_navigation_item_changed, this, _2));
	navigation.select_item(0);

	treport& photos = find_widget<treport>(window_, "photos", false);
	photos.set_did_item_pre_change(std::bind(&tphoto_selector::did_photos_item_pre_change, this, _3));
	photos.set_did_item_changed(std::bind(&tphoto_selector::did_photos_item_changed, this, _2));
