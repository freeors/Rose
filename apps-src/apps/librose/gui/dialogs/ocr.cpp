...
void tocr::pre_show()
{
	...
	paper_->set_did_draw(std::bind(&tocr::did_draw_paper, this, _1, _2, _3));
	paper_->set_did_mouse_leave(std::bind(&tocr::did_mouse_leave_paper, this, _1, _2, _3));
	paper_->set_did_mouse_motion(std::bind(&tocr::did_mouse_motion_paper, this, _1, _2, _3));
...
int locate_character(const cv::Mat& src, const SDL_Rect& base_rect, bool right, SDL_Rect& result_rect, int& threshold_level, bool verbose, int step)
{
	...
	if (SDL_RectEmpty(&base_rect) || threshold_level == 0 || threshold_level == 255) {
		threshold_level = (int)cv::threshold(src, first_cut_mat, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);

	} else {
		threshold_level = (int)cv::threshold(src, first_cut_mat, threshold_level, 255, cv::THRESH_BINARY);
	}
...
static void generate_threshold_demo_png()
{
	...
	for (std::vector<std::pair<cv::Rect, int> >::iterator it = clips.begin(); it != clips.end(); ++ it) {
		...
		if (it->second == nposm) {
			threshold = cv::threshold(gray(clip), out, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
			it->second = threshold;
		} else {
			int distance = std::distance(clips.begin(), it);
			VALIDATE(it->second < distance, null_str);

			threshold = cv::threshold(gray(clip), out, clips[it->second].second, 255, cv::THRESH_BINARY);
		}
			...
			if (!calibrating) {
				...
				if (space) {
					...
					if (!line.chars.empty()) {
						...
						accumulate_rect.w += SDL_max(result_rect.w, min_accumulate_width);
						valid_chars ++;
...
void tocr::did_draw_slice(int id, SDL_Renderer* renderer, trtc_client::VideoRenderer** locals, int locals_count, trtc_client::VideoRenderer** remotes, int remotes_count, const SDL_Rect& draw_rect)
{
	VALIDATE(!target_.get(), null_str);
	...
	if (!blended_tex_pixels_) {
		int pitch = 0;
		SDL_LockTexture(vsink.cv_tex_.get(), NULL, (void**)&blended_tex_pixels_, &pitch);
	}

	if (vsink.new_frame && !current_surf_.first) {
		const int max_value = SDL_max(frame.cols, frame.rows);
		if (max_value <= 640 || vsink.frames >= next_recognize_frame_) {
	...
	if (camera_surf_.get()) {
		...
		SDL_RenderCopy(renderer, vsink.cv_tex_.get(), nullptr, &dst);
		
	} else {
		SDL_RenderCopy(renderer, vsink.tex_.get(), nullptr, &dst);
	}
