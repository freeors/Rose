...
tchat_::tchat_(const std::string& widget_id, int chat_page)
	: trtc_client(0, *this, *this, tpoint(nposm, nposm), false, std::vector<trtsp_settings>(), nposm, false, true)
...
void tchat_::enter_inputing(bool enter)
{
	...
	input_scale_->set_best_size_1th(nposm, input_scale_->get_width_is_max(), height, input_scale_->get_height_is_max());
}
...
void tchat_::generate_channel_tree(tlobby_channel& channel)
{
	...
	for (std::vector<tlobby_user*>::const_iterator it = channel.users.begin(); it != channel.users.end(); ++ it) {
		...
		toggle.set_did_state_changed(std::bind(&tchat_::channel2_toggled, this, _1));
...
*/
	...
	person_tree_->set_did_node_changed(std::bind(&tchat_::contact_channel_toggled, this, _1, _2));

	notify_list_->enable_select(false);
	notify_list_->set_did_row_changed(std::bind(&tchat_::notify_did_click, this, _1, _2));
#ifdef _WIN32
	notify_list_->set_did_row_double_click(std::bind(&tchat_::notify_did_double_click, this, _1, _2));
#endif
	chanlist_->set_did_row_changed(std::bind(&tchat_::find_chan_toggled, this, std::ref(window), _1));
	...
	
	{
		int ii = 0;
		ttext_box* text_box = &find_widget<ttext_box>(window_, "_chat_my_nick", false);
		text_box->set_label("ancientcc");
	}

	tscroll_text_box& input = find_widget<tscroll_text_box>(window_, ctrlid_input, false);
	ttext_box& tb = find_widget<ttext_box>(input.content_grid(), "_text_box", false);
	input_tb_ = &tb;
	connect_signal_pre_key_press(tb, std::bind(&tchat_::signal_handler_sdl_key_down, this, _3, _4, _5, _6, _7));
	...
	toolbar_->set_did_item_click(std::bind(&tchat_::did_item_click_report, this, _1, _2));
	reload_toolbar(window);

	catalog_ = find_widget<treport>(&window, ctrlid_contact_bar, false, true);
	catalog_->set_did_item_changed(std::bind(&tchat_::did_item_changed_report, this, _1, _2));
...
void tchat_::did_draw_slice(int id, SDL_Renderer* renderer, trtc_client::VideoRenderer** locals, int locals_count, trtc_client::VideoRenderer** remotes, int remotes_count, const SDL_Rect& draw_rect)
{
	surface text_surf;

	if (remotes_count > 0) {
		trtc_client::VideoRenderer& vsink = *remotes[0];

		ttexture_2_mat_lock mat_lock(vsink.tex_);
		cv::Mat& frame = mat_lock.mat;
		ttexture_2_mat_lock cv_mat_lock(vsink.cv_tex_);

		SDL_RenderCopy(renderer, vsink.tex_.get(), NULL, &draw_rect);
	...
	if (locals_count > 0) {
		...
		SDL_RenderCopy(renderer, vsink.tex_.get(), NULL, &dst);
...
void tchat_::did_draw_vrenderer(ttrack& widget, const SDL_Rect& widget_rect, const bool bg_drawn, bool force)
{
	...
	if (message_handler_.send_helper.deconstructed()) {
		return;
	}

	SDL_Renderer* renderer = get_renderer();

	VideoRenderer* local_sink = vrenderer(false, 0);
	VideoRenderer* remote_sink = vrenderer(true, 0);
	...
	if (require_render_remote || require_render_local) {
		draw_slice(renderer, widget_rect);
	}
}
...
void tchat_::did_drag_coordinate(ttrack& widget, const tpoint& first, const tpoint& last)
{
	...
	vrenderer_track_->immediate_draw();
}
...
void tchat_::switch_to_video(twindow& window)
{
	...
	if (!vrenderer_track_) {
		...
		track->set_did_draw(std::bind(&tchat_::did_draw_vrenderer, this, _1, _2, _3, false));
		track->set_did_mouse_leave(std::bind(&tchat_::did_control_drag_detect, this, _1, _2, _3));
		track->set_did_mouse_motion(std::bind(&tchat_::did_drag_coordinate, this, _1, _2, _3));
...
void tchat_::process_message(const std::string& chan, const std::string& from, const std::string& text)
{
	...
	chat_logs::add(receive_id, !chan.empty(), user, utils::is_utf8str(text.c_str(), text.size())? text: err_encode_str);
...
void tchat_::process_chanlist(const std::string& chan, int users, const std::string& topic)
{
	...
	if (list_chans_.size() <= 100) {
		...
		list_item_item.insert(std::make_pair("topic", utils::is_utf8str(topic.c_str(), topic.size())? topic: err_encode_str));
