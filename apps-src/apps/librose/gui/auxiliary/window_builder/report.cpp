...
tbuilder_report::tbuilder_report(const config& cfg)
	...
	, multi_line(cfg["multi_line"].to_bool())
	, toggle(cfg["toggle"].to_bool())
	, unit_definition(cfg["unit_definition"].str())
	, unit_width(cfg["unit_width"])
	, unit_height(cfg["unit_height"])
	, gap(cfg["gap"].to_int())
	, segment_switch(cfg["segment_switch"].to_bool())
	, fixed_cols(cfg["fixed_cols"].to_int())
	, multi_select(cfg["multi_select"].to_bool())
{
	if (unit_definition.empty()) {
		unit_definition = "default";
	}
}

twidget* tbuilder_report::build() const
{
	treport* widget = new treport(multi_line, toggle, unit_definition, unit_width, unit_height, gap, segment_switch, fixed_cols, multi_select);
