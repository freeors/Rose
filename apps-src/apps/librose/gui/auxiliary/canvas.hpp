...
public:
	...
	enum {none_shape, line_shape, rect_shape, circle_shape, image_shape, text_shape, blit_shape, anim_shape, commit_shape};
	/**
	...
	public:
		 ...
		 *                        for these formulas.
		 */
		virtual void draw(texture& canvas, const int canvas_width, const int canvas_height, const game_logic::map_formula_callable& variables, std::vector<SDL_Rect>& drawn_rects) = 0;
	 ...
	 */
	void draw(const tcontrol& widget, const SDL_Rect& rect, bool force, const std::vector<int>& post_anims);
	 ...
	 */
	void blit(const tcontrol& widget, texture& surf, SDL_Rect rect, bool force, const std::vector<int>& post_anims);

	/**
	 * Sets the config.
	 *
	 * @param cfg                 The config object with the data to draw, see
	 *                            /wiki/GUICanvasWML for
	 *                            more information.
	 */
	...
	texture& canvas_tex() { return canvas_; }
	const texture& canvas_tex() const { return canvas_; }
	...
	texture get_canvas_tex(tcontrol& widget, const std::vector<int>& post_anims);
