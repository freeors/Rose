...
/*
	SDL_Log("==>capture_mouse, E, mouse_focus_(%p)(%s), widget(%p)(%s)", 
		mouse_focus_, !mouse_focus_->id().empty()? mouse_focus_->id().c_str(): "<nil>",
		widget, widget && !widget->id().empty()? widget->id().c_str(): "<nil>");
*/
	...
	// The above mouse_leave() will set mouse_captured_ is false, 
	// so this set must be after mouse_leave().
...
/*
	SDL_Log("signal_handler_sdl_mouse_motion(%s), E, (%i, %i), mouse_captured_[%s, mouse_focus_[%p(%s), parent[(%s)", 
		owner_.id().c_str(),
...
*/
	...
	if (is_focus_lost_event(coordinate) || is_mouse_leave_window_event(coordinate)) {
		// SDL_Log("------signal_handler_sdl_mouse_motion, magic event(%i, %i), mouse_focus_: %p", coordinate.x, coordinate.y, mouse_focus_);
	...
	VALIDATE(!is_focus_lost_event(coordinate) && !is_mouse_leave_window_event(coordinate), null_str);
		...
		, const tpoint& coordinate2)
{
	...
	// SDL_Log("[dbg]call signal_handler_sdl_mouse_motion(%i, %i), signal_handler_sdl_wheel", coordinate.x, coordinate.y);
...
{
	// SDL_Log("mouse_enter: %s(0x%p), (%i, %i, %i, %i)", mouse_over->id().empty()? "<nil>": mouse_over->id().c_str(), mouse_over, mouse_over->get_x(), mouse_over->get_y(), mouse_over->get_width(), mouse_over->get_height());
...
{
	// SDL_Log("mouse_leave: %s(0x%p), mouse_captured_: %s, (%i, %i, %i, %i)", mouse_focus_->id().empty()? "<nil>": mouse_focus_->id().c_str(), mouse_focus_, 
	//	mouse_captured_? "true": "false", mouse_focus_->get_x(), mouse_focus_->get_y(), mouse_focus_->get_width(), mouse_focus_->get_height());
    VALIDATE(mouse_focus_, null_str);

	{
		std::string err_msg = _("Must not popup new window during MOUSE_LEAVE. replace with app_OnMessage.");
		twidget::tdisable_popup_window_lock lock(err_msg);
		owner_.fire(event::MOUSE_LEAVE, *mouse_focus_, coordinate, coordinate2);
		VALIDATE(mouse_focus_, err_msg);
	}
		
	owner_.fire2(NOTIFY_REMOVE_TOOLTIP, *mouse_focus_);

	mouse_focus_ = nullptr;
	// When mouse_captured_ is true, mouse_focus_ must not be nullptr. 
	// Since mouse_focus_ has been set nullptr, requiring to set mouse_captured_ to be false.
...
void tdistributor::start_hover_timer(twidget* widget, const tpoint& coordinate)
{
	...
	hover_timer_.reset(50, *static_cast<twindow*>(&owner_), std::bind(&tdistributor::show_tooltip, this));
...
/*
	SDL_Log("[#%i]signal_handler_sdl_button_down(%s)------(%i, %i), mouse_focus_: %p(captured: %s), mouse_click_: %p, window_id(%s)",
		times, distributor_.owner_.id().c_str(), coordinate.x, coordinate.y,
		distributor_.mouse_focus_, distributor_.mouse_captured_? "true": "false",
		distributor_.mouse_click_, distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
*/
	...
	if (!mouse_over) {
		// SDL_Log("------[#%i]signal_handler_sdl_button_down[#%i], mouse_over = nullptr, window_id(%s)", times, button_, distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
		return;
	}
	...
	// SDL_Log("------[#%i]signal_handler_sdl_button_down[#%i], window_id(%s)", times, button_, distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
}
		...
		, const tpoint& coordinate)
{
	const int times = times_;
/*
	SDL_Log("[#%i]signal_handler_sdl_button_up[#%i](%s)------mouse_focus_: %p(%s), window_id(%s)",
		times, button_, distributor_.owner_.id().c_str(),
...
*/
	...
	handled = true;
/*
	SDL_Log("------[#%i]signal_handler_sdl_button_up[#%i], mouse_click_: %p, window_id(%s)", times, button_, distributor_.mouse_click_,
		distributor_.owner_.id().empty()? "nill": distributor_.owner_.id().c_str());
*/
}
...
void tdistributor::keyboard_capture(twidget* widget)
{
	...
	if (keyboard_focus_) {
		owner_.fire(event::LOSE_KEYBOARD_FOCUS, *keyboard_focus_, nposm, widget);
	}
...
{
	bool handled = static_cast<twindow*>(&owner_)->signal_handler_sdl_key_down_pre(key);
	if (handled) {
		return;
	}
...
void tdistributor::new_window_connected()
{
	if (mouse_focus_) {
...
{
	// SDL_Log("tdistributor::post_mouse_up[%s]---", owner_.id().empty()? "nill": owner_.id().c_str());
