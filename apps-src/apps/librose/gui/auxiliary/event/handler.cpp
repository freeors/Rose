...
tpoint revise_screen_size(const int width, const int height)
{
	...
	if (landscape_size.x < game_config::min_allowed_width()) {
		landscape_size.x = game_config::min_allowed_width();
	}
	if (landscape_size.y < game_config::min_allowed_height()) {
		landscape_size.y = game_config::min_allowed_height();
	}
...
class thandler: public tevent_dispatcher
{
	...
	friend void gui2::absolute_draw_float_widgets();
	friend void gui2::absolute_draw_all(bool need_layout);
	friend void gui2::invalidate_layout_all();
	friend std::vector<twindow*> gui2::connectd_window();
	friend void gui2::pump_timers();
public:
	thandler();
...
private:
	...
	/** Fires a draw event. */
	void draw_internal(bool scene, twindow& window, bool with_float_widgets);
	void draw();
...
void thandler::handle_mouse_motion(const SDL_MouseMotionEvent& motion)
{
	...
	if (!is_mouse_leave_window_event(motion)) {
		// SDL_Log("[dbg]call signal_handler_sdl_mouse_motion(%i, %i), thandler::handle_mouse_motion, 1", motion.x, motion.y);
		handled = window.signal_handler_sdl_mouse_motion(tpoint(motion.x, motion.y));
	}
	if (!handled) {
		// SDL_Log("[dbg]call signal_handler_sdl_mouse_motion(%i, %i), thandler::handle_mouse_motion, 2", motion.x, motion.y);
		...
		if (event.window.event == SDL_WINDOWEVENT_LEAVE) {
			...
			// if (mouse_flags & SDL_BUTTON(SDL_BUTTON_LEFT)) {
				...
				if (twindow::eat_left_up) {
					SDL_Log("SDL_WINDOWEVENT_LEAVE, eat_left_up(%i) isn't 0, evalue to 0.", twindow::eat_left_up);
...
void thandler::connect(twindow* window)
{
	...
	if (dispatchers_.empty()) {
		...
		dispatchers_.back()->new_window_connected();
	}
...
void thandler::draw_internal(bool scene, twindow& window, bool with_float_widgets)
{
	if (!window.draw()) {
		return;
	}
...
void thandler::draw()
{
	...
	if (instance->invalidate_layout(true)) {
		window->invalidate_layout(nullptr);
	}
	draw_internal(in_scene, *window, true);
/*
	if (!window->draw()) {
		return;
	}
...
void thandler::draw_float_widgets()
{
	if (dispatchers_.empty()) {
		return;
	}
...
{
	for (std::vector<twindow*>::reverse_iterator ritor = dispatchers_.rbegin(); ritor != dispatchers_.rend(); ++ritor) {
...
{
	event::handler->draw_float_widgets();
}

void absolute_draw_all(bool need_layout)
{
	int at = 0;
	const bool in_scene = event::handler->dispatchers_.front()->is_scene();
	for (std::vector<twindow*>::const_iterator it = event::handler->dispatchers_.begin(); it != event::handler->dispatchers_.end(); ++ it, at ++) {
		twindow* window = *it;
		if (need_layout) {
			window->invalidate_layout(nullptr);
		}
		event::handler->draw_internal(in_scene, *window, at == event::handler->dispatchers_.size() - 1);
	}
}

void invalidate_layout_all()
{
	int at = 0;
	const bool in_scene = event::handler->dispatchers_.front()->is_scene();
	for (std::vector<twindow*>::const_iterator it = event::handler->dispatchers_.begin(); it != event::handler->dispatchers_.end(); ++ it, at ++) {
...
void send_sdl_event(const SDL_Event& e)
{
	if (event::handler == nullptr) {
		return;
	}
	event::handler->send_sdl_event(e);
}
...
void pump_timers()
{
	...
	event::handler->dispatchers_.back()->pump_timers();
}
