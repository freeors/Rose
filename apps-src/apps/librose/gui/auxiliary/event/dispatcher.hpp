...
*/
	...
	bool fire(const tevent event, twidget& target, const int val = 0, twidget* extra_widget = nullptr);
	 ...
	 */
	bool fire(const tevent event, twidget& target, const tpoint& coordinate, const tpoint& coordinate2);
...
inline void connect_signal_mouse_left_click(
		  tdispatcher& dispatcher
		, const tsignal_function& signal, const tdispatcher::tposition position = tdispatcher::back_child)
{
	dispatcher.connect_signal<LEFT_BUTTON_CLICK>(signal, position);
}
...
/** Connects a signal handler for getting a notification upon modification. */
inline void disconnect_signal_notify_modified(
		  tdispatcher& dispatcher
		, const tsignal_notification_function& signal)
{
	dispatcher.disconnect_signal<event::NOTIFY_MODIFIED>(signal);
}
