...
{
public:
	ttrigger(const int val, twidget* extra_widget)
		: val_(val)
		, extra_widget_(extra_widget)
	{}
	...
	void operator()(tsignal_function functor
			...
			, bool& halt)
	{
		functor(dispatcher, event, handled, halt, val_, extra_widget_);
	}

private:
	int val_;
	twidget* extra_widget_;
};

bool tdispatcher::fire(const tevent event, twidget& target, const int val, twidget* extra_widget)
{
...
{
public:
	ttrigger_mouse(const tpoint& coordinate, const tpoint& coordinate2)
	...
	void operator()(tsignal_mouse_function functor
			...
			, bool& halt)
	{
		functor(dispatcher, event, handled, halt, coordinate_, coordinate2_);
	}
		...
		, const tpoint& coordinate2)
{
	...
	return fire_event<tsignal_mouse_function>(event
			, dynamic_cast<twidget*>(this)
			, &target
			, ttrigger_mouse(coordinate, coordinate2));
}
