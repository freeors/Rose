...
public:
	...
	/** Determine whether the class contains a formula or a value. */
	bool has_formula2() const { return formula2_; }

	/** Determine whether the class contains a formula. */
	bool has_formula() const { return !formula_.empty(); }

	bool has_multi_or_noninteger_formula() const 
	{
		if (formula_.empty()) {
			return false;
		}
		// if true, has_formula() must be true.
		// in formulas_, first char must be '(', but ')' maybe not exist.
		int len = (int)formula_.size();
		if (len < 2) {
			// only exist '(', think is true.
			return true;
		}
		return !utils::isinteger(formula_.substr(1, len -2)); 
	}
...
private:
	 ...
	 *
	 * If the string is empty or begin width '(', there's no formula.
	 */
	std::string formula_;

	/**
	 * Contains the formuale or value for the variable.
	 *
	 * If the string is empty, there's no formula2.
	 */
	bool formula2_;
		...
		const game_logic::map_formula_callable& variables) const
{
	if(has_formula()) {
		...
		// Formula: execute 'formula_' result 'result'.
		return result;
	} else {
		return value_;
	}
