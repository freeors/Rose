 ...
 *
 */
twindow* build(tdialog& dlg, CVideo &video, const twindow_builder::tresolution *definition, const unsigned explicit_x, const unsigned explicit_y)
{
	...
	twindow* window = new twindow(dlg, video
			, definition->x
			, definition->y
			, definition->width
			, definition->height
	...
	if (get_keyboard_widget() == nullptr) {
		tfloat_widget* vertical = nullptr;
		tfloat_widget* horizontal = nullptr;

		for (std::vector<twindow_builder::tfloat_widget>::const_iterator it = definition->float_widgets.begin(); it != definition->float_widgets.end(); ++ it) {
			const twindow_builder::tfloat_widget& builder = *it;
			twidget* widget2 = builder.widget->build();
			if (widget2->id() == settings::vertical_scrollbar_id) {
				tcontrol& widget = *dynamic_cast<tcontrol*>(widget2);
				vertical = new tfloat_widget(builder, widget);
				widget.set_parent(nullptr);
				widget.set_visible(twidget::INVISIBLE);
				widget.set_float_widget();

			} else if (widget2->id() == settings::horizontal_scrollbar_id) {
				tcontrol& widget = *dynamic_cast<tcontrol*>(widget2);
				horizontal = new tfloat_widget(builder, widget);
				widget.set_parent(nullptr);
				widget.set_visible(twidget::INVISIBLE);
				widget.set_float_widget();
			}
		}
		gui.create_keyboard_widget(vertical, horizontal);
	}
...
twindow* build(tdialog& dlg, CVideo &video, const std::string &type, const unsigned explicit_x, const unsigned explicit_y)
{
	...
	twindow *window = build(dlg, video, &*definition, explicit_x, explicit_y);
	window->set_id(type);
	return window;
}
...
tbuilder_widget_ptr create_builder_widget(const config& cfg)
{
	...
	const std::map<std::string, std::function<tbuilder_widget_ptr(config)> >& widgets = builder_widget_lookup();
	std::map<std::string, std::function<tbuilder_widget_ptr(config)> >::const_iterator it = widgets.find(item.key);
...
{
	std::map<std::string, std::function<tbuilder_widget_ptr(config)> >::const_iterator it =
