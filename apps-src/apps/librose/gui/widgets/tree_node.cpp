		...
		BOOST_FOREACH(const tnode_definition& node_definition, node_definitions_) {
			if (node_definition.id == id) {
				...
				if (icon_) {
					...
					icon_->set_did_state_changed(std::bind(&ttree_node::did_icon_state_changed, this, _1));
...
void ttree_node::sort_children(const std::function<bool (const ttree_node&, const ttree_node&)>& did_compare) 
{
	if (children_.size() < 2) {
		return;
	}
