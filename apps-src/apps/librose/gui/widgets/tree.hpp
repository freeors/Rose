...
public:
	...
	void clear() override { root_node_->erase_children(); }
	...
	void set_did_fold_changed(const std::function<void(ttree&, ttree_node&, const bool)>& did)
	{
		did_fold_changed_ = did;
	}
	void set_did_node_changed(const std::function<void(ttree& view, ttree_node& node)>& did)
	{
		did_node_changed_ = did;
	}
	void set_did_double_click(const std::function<void(ttree& view, ttree_node& node)>& did)
	{
		did_double_click_ = did;
	}
	void set_did_right_button_up(const std::function<void(ttree& view, ttree_node&, const tpoint&)>& did)
	{
	...
	void adjust_offset(int& x_offset, int& y_offset) override;
...
private:
	...
	std::function<void (ttree&, ttree_node&, const bool)> did_fold_changed_;
	std::function<void (ttree&, ttree_node&)> did_node_changed_;
	// only leaf can trigger did_double_click_.
	std::function<void (ttree&, ttree_node&)> did_double_click_;
	std::function<void (ttree&, ttree_node&, const tpoint&)> did_right_button_up_;
