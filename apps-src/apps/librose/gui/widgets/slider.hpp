...
public:
	...
	/** Inherited from tinteger_selector_. */
	int get_value() const
		{ return minimum_value_ + get_item_position(); }
	...
	void set_did_value_change(const std::function<void (tslider&, int)>& did)
	{ 
		did_value_changed_ = did; 
	}
protected:
...
private:
	...
	std::function<void (tslider&, int)> did_value_changed_;
