...
public:
	...
	tfloat_widget(const twindow_builder::tfloat_widget& builder, tcontrol& widget)
		: w(builder.w)
		, h(builder.h)
		, x(builder.x)
		, y(builder.y)
...
public:
	...
	int get_maximum_text_width_4_camvas() const;
	...
	bool get_width_is_max() const { return width_is_max_; }
	bool get_height_is_max() const { return height_is_max_; }

	void set_sound(const std::string& sound) { sound_ = sound; }
	tpoint best_size_from_builder() const;

	int calculate_best_height_2th(const twindow& window) const;
...
public:
	 ...
	 */
	void set_definition(const std::string& definition, const std::string& min_text_width, const std::string& min_text_height);

	/***** ***** ***** setters / getters for members ***** ****** *****/
	const std::string& label() const { return label_; }
	virtual void set_label(const std::string& label);

	virtual void set_border(const std::string& border) { set_canvas_variable("border", variant(border)); }
	...
	void set_best_size_1th(const int width, bool width_is_max, const int height, bool height_is_max);
	void set_best_size_2th(const std::string& width, bool width_is_max, const std::string& height, bool height_is_max);
	void set_mtwusc() override;
...
protected:
	 ...
	 */
	tpoint get_best_text_size(const int maximum_width, const int font_size) const;
...
{
public:
	enum {type_text_box2 = 1};
	tbase_tpl_widget(twindow& window, twidget& widget);
	virtual ~tbase_tpl_widget() {}

	virtual tpanel* panel() const { return nullptr; }
	virtual tcontrol* image() const { return nullptr; }
	virtual ttext_box* text_box() const { return nullptr; }
	virtual tbutton* button() const { return nullptr; }

	virtual void set_did_text_changed(const std::function<void (ttext_box& widget) >& did) {}
