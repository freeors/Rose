...
public:
	...
	void deselect_item(int at);
	void deselect_item(twidget& widget);
	void clear_selected_items();

	void tabbar_set_callback_show(const std::function<void (treport&, tcontrol&)>& callback) 
	{ 
		callback_show_ = callback; 
	}
	// think first change must be allowed, and don't trigger did_item_pre_change_
	void set_did_item_pre_change(const std::function<bool (treport&, ttoggle_button& from, ttoggle_button& to)>& did)
	{
		did_item_pre_change_ = did;
	}
	void set_did_item_changed(const std::function<void (treport&, ttoggle_button&)>& did)
	{
		did_item_changed_ = did;
	}
	void set_did_item_click(const std::function<void (treport&, tbutton&)>& did)
	{
		did_item_click_ = did;
	}
	...
	bool multi_select() { return multi_select_; }
	const std::set<tcontrol*>& selected_items() const { return selected_items_; }

	const tgrid::tchild* child_begin() const;

private:
	void tabbar_init(bool toggle, const std::string& definition, bool multi_select, bool segment_switch);
	void multiline_init(bool toggle, const std::string& definition, bool multi_select, int fixed_cols);
...
private:
	...
	// valid only multiline report. can select multiple items simultaneous.
	bool multi_select_;
	...
	std::function<void (treport&, tcontrol& widget)> callback_show_;
	std::function<bool (treport&, ttoggle_button& from, ttoggle_button& to)> did_item_pre_change_;
	std::function<void (treport&, ttoggle_button& widget)> did_item_changed_;
	std::function<void (treport&, tbutton& widget)> did_item_click_;

	tpanel* boddy_;
	// valid when multi_select_ = true.
	std::set<tcontrol*> selected_items_;
