...
void tscroll_container::clear_text_maximum_width()
{
	content_grid_->clear_text_maximum_width();
}
...
tpoint tscroll_container::calculate_best_size() const
{
	...
	// the widget's best size derived from tcontainer_ include margin. 
		...
		const unsigned items, const unsigned visible_items)
{
	...
	if (&scrollbar != dummy_horizontal_scrollbar_ && &scrollbar != dummy_vertical_scrollbar_) {
		if (scrollbar.float_widget()) {
			...
			if (scrollbar.id() == settings::vertical_scrollbar_id) {
				vertical_scrollbar2_->set_visible(visible);
			} else {
				VALIDATE(scrollbar.id() == settings::horizontal_scrollbar_id, null_str);
				horizontal_scrollbar2_->set_visible(visible);
			}
		} else {
			const bool scrollbar_needed = items > visible_items;
...
void tscroll_container::finalize_setup(const tbuilder_grid_const_ptr& grid_builder)
{
	...
	vertical_drag_spacer_->set_definition("default", null_str, null_str);
	...
	pullrefresh_track_->set_definition("default", null_str, null_str);
	pullrefresh_track_->set_parent(this);

	/***** Setup vertical scrollbar *****/
	if (twindow::init_instance != nullptr) {
		vertical_scrollbar2_ = twindow::init_instance->find_float_widget(vertical_id);
	} else {
		vertical_scrollbar2_ = gui.keyboard_vertical_scrollbar;
	}
	...
	vertical_scrollbar_->set_definition("default", null_str, null_str);
	...
	if (twindow::init_instance != nullptr) {
		...
		horizontal_scrollbar2_ = gui.keyboard_horizontal_scrollbar;
	}
	VALIDATE(horizontal_scrollbar2_, null_str);

	dummy_horizontal_scrollbar_ = horizontal_scrollbar_ = new thorizontal_scrollbar();
	horizontal_scrollbar_->set_definition("default", null_str, null_str);
...

	{
		...
		content_grid_->connect_signal<event::MOUSE_LEAVE>(std::bind(
...
bool tscroll_container::can_invalidate_layout(const twidget& widget) const
{
	// if this is dirty, requier invalidte layout its parent.
	return &widget != this;
}
...
bool tscroll_container::scroll(const bool vertical, const bool up, const int level, const bool first)
{
	...
	if (!wheel && first) {
		...
		scroll_timer_.reset(200, *get_window(), std::bind(&tscroll_container::scroll_timer_handler, this, vertical, up, level));
	}
	if (vertical) {
		if (dummy_vertical_scrollbar_ != vertical_scrollbar_) {
...
void tscroll_container::signal_handler_mouse_enter(const tpoint& coordinate, bool pre_child)
{
	...
	if (vertical_scrollbar_ == dummy_vertical_scrollbar_) {
		...
		if (vertical_scrollbar2_->widget.get()->parent() != window) {
			...
			window->reset_scrollbar(*vertical_scrollbar2_);
		}
		...
		vertical_scrollbar.set_did_modified(std::bind(&tscroll_container::vertical_scrollbar_moved, this));
		...
		horizontal_scrollbar.set_did_modified(std::bind(&tscroll_container::horizontal_scrollbar_moved, this));
...
{
/*
	SDL_Log("tscroll_container::signal_handler_left_button_down, coordinate(%i, %i), id: %s, first_coordinate_(%i, %i)\n", 
...
/*
	SDL_Log("tscroll_container::signal_handler_left_button_up, coordinate(%i, %i), id: %s, first_coordinate_(%i, %i)\n", 
		coordinate.x, coordinate.y,
		id().empty()? "<nil>": id().c_str(), first_coordinate_.x, first_coordinate_.y);
*/
	twindow* window = get_window();
	...
	if (vertical_scrollbar_ == vertical_scrollbar2_->widget.get()) {
		...
		if ((game_config::mobile && is_up_result_leave(coordinate2)) || !point_in_rect(coordinate.x, coordinate.y, content_->get_rect())) {
			...
			if (!scroll_timer_.valid()) {
				...
				scroll_timer_.reset(200, *window, std::bind(&tscroll_container::scroll_timer_handler, this, false, false, 0));
			}
		}
	}
...
void tscroll_container::did_default_pullrefresh_drag1(ttrack& widget, const SDL_Rect& rect)
{
	...
	surface surf = image::get_image("misc/arrow_down.png");
...
void tscroll_container::did_default_pullrefresh_drag2(ttrack& widget, const SDL_Rect& rect)
{
	...
	surface surf = image::get_image("misc/arrow_up.png");
...
void tscroll_container::set_did_pullrefresh(const int refresh_height, const std::function<void (ttrack&, const SDL_Rect&)>& did_refresh,
		const int min_drag2_height, const std::function<void (ttrack&, const SDL_Rect&)>& did_drag1,const std::function<void (ttrack&, const SDL_Rect&)>& did_drag2)
{
	...
	if (did_drag1) {
		...
		did_pullrefresh_drag1_ = std::bind(&tscroll_container::did_default_pullrefresh_drag1, this, _1, _2);
		did_pullrefresh_drag2_ = std::bind(&tscroll_container::did_default_pullrefresh_drag2, this, _1, _2);
...
void tscroll_container::release_mouse_focus()
{
	// when is dragging the mouse/finger, the listbox's row structure changed, for example insert/erase row.
	twindow* window = get_window();
/*
	const twidget* current_mouse_click = window->mouse_click_widget();
	if (current_mouse_click != nullptr && current_mouse_click->me_or_parent_is_widget(*this)) {
		// since will move scroll_container, must not trigger mouse_click event.
		SDL_Log("%s release_mouse_foucs, [1/3] call window->clear_mouse_click()", id_.c_str());
		window->clear_mouse_click();
	}

	const twidget* current_mouse_focus = window->mouse_captured_widget();
	if (current_mouse_focus != nullptr && current_mouse_focus->me_or_parent_is_widget(*this)) {
		SDL_Log("%s release_mouse_foucs, [2/3] call window->mouse_capture(nullptr)", id_.c_str());
		window->mouse_capture(nullptr);
	}
*/
	if (!is_null_coordinate(first_coordinate_)) {
		tpoint coordinate(0, 0);
		set_mouse_leave_window_event(coordinate);
		SDL_Log("%s release_mouse_foucs, [3/3] call set_first_coordinate_null(coordinate)", id_.c_str());
		set_first_coordinate_null(coordinate);
	}
}
