...
struct tedit_box_param 
{
	enum {hide_cancel = nposm, show_cancel = 0};
	tedit_box_param(const std::string& title, const std::string& prefix, const std::string& placeholder, const std::string& initial, const std::string& remark, const std::string& reset, const std::string& ok, size_t max_chars, int cancel_strategy = show_cancel) 
		: title(title)
		, prefix(prefix)
	...
	{}
	bool valid() const { return !title.empty() || !placeholder.empty(); }

	std::string title;
	std::string prefix;
	std::string placeholder;
	std::string initial;
	// if remark isn't empty, will visiable "remark" label, and set this value.
	std::string remark;
	// when "reset" clicked, will set this value to text box.
	std::string reset;
	// if ok is null_str, should hide ok button.
	std::string ok;
	// if don't limit the count of characters, use nposm.
	size_t max_chars;
	// nposm(hide_cancel): don't show cancel button.
	// 0(show_cancel): show cancel button, and don't use countdown.
	// >0(show_cancel + N): show cancel button, use count down, cancel_strategy is total second.
	int cancel_strategy;
	std::function<bool (const std::string& label) > did_text_changed;
	std::string result;
};
...
{
public:
	ttext_box(bool multi_line, bool desensitize, bool atom_markup);
	virtual ~ttext_box();
	...
	void set_did_text_changed(const std::function<void (ttext_box& textboxt) >& did)
	{
		did_text_changed_ = did;
	}

	void set_did_extra_menu(const std::function<void (ttext_box& widget, std::vector<tfloat_widget*>&)>& did)
	{
		did_extra_menu_ = did;
	}
	...
	tedit_box_param& edit_box_param() { return edit_box_param_; }
	void set_edit_box_param(const std::string& title, const std::string& placeholder)
	{
		edit_box_param_.title = title;
		edit_box_param_.placeholder = placeholder;
	}
	...
	void set_did_cursor_moved(const std::function<void (ttext_box& textboxt) >& did)
	{
		did_cursor_moved_ = did;
	}
	void set_did_desensitize_str(const std::function<std::string (const ttext_box&, const std::string&)>& did)
	{
		did_desensitize_str_ = did;
	}
	int cursor_height() const;
	int text_y_offset() const { return text_y_offset_; }

	void set_desensitize() 
	{ 
		desensitize_ = true;
		cipher_ = true;
	}
...
public:
	 ...
	 * Alt                        Ignored.
	 */
	bool handle_key_left_arrow(SDL_Keymod modifier);
	 ...
	 * Alt                        Ignored.
	 */
	bool handle_key_right_arrow(SDL_Keymod modifier);
...
private:
	...
	std::string to_password_str(const std::string& str) const;
...
private:
	tedit_box_param edit_box_param_;
	...
	std::function<void (ttext_box& widget, std::vector<tfloat_widget*>&)> did_extra_menu_;
	std::vector<tfloat_widget*> extra_menu_;

	std::unique_ptr<tout_of_chain_widget_lock> out_of_chain_widget_lock_;

	struct tcursor_bytes_pos_lock
	{
	public:
		tcursor_bytes_pos_lock(ttext_box& widget, int pos)
			: widget_(widget)
		{
			VALIDATE(widget.cursor_bytes_pos_ == nposm, null_str);
			widget.cursor_bytes_pos_ = pos;
		}
		~tcursor_bytes_pos_lock()
		{
			widget_.cursor_bytes_pos_ = nposm;
		}

	private:
		ttext_box& widget_;
	};
	int cursor_bytes_pos_;
...
private:
	...
	void signal_handler_left_button_click(const event::tevent event, bool& handled, bool& halt);
	...
	std::function<void (ttext_box&)> did_cursor_moved_;

	std::function<std::string (const ttext_box&, const std::string&)> did_desensitize_str_;
	...
	void signal_handler_lose_keyboard_focus(const event::tevent event, twidget* extra_widget);
