...
treport::treport(bool multi_line, bool toggle, const std::string& unit_definition, const std::string& unit_w, const std::string& unit_h, int gap, bool segment_switch, int fixed_cols, bool multi_select)
	...
	, internal_set_visible_(false)
{
	...
	if (multi_line_) {
		multiline_init(toggle, unit_definition, multi_select, fixed_cols);
	} else {
		tabbar_init(toggle, unit_definition, multi_select, segment_switch);
	}
}
...
void treport::erase_item(int at)
{
	...
	if (toggle_ && auto_select && !multi_select_) {
...
tpoint treport::tgrid2::calculate_best_size() const
{
	...
	if (!report_.multi_line_) {
		...
		if (valids != 0) {
			result.x += (valids - 1) * report_.gap_;
		}
		result.y = max_height;
		return result;
	}
...
void treport::tabbar_init(bool toggle, const std::string& definition, bool multi_select, bool segment_switch)
{
	...
	tformula_blit blit("misc/arrow_left.png", null_str, null_str, str_cast(unit_size_.x), str_cast(unit_size_.y));
	...
	blit = tformula_blit("misc/arrow_right.png", null_str, null_str, str_cast(unit_size_.x), str_cast(unit_size_.y));
...
void treport::multiline_init(bool toggle, const std::string& definition, bool multi_select, int fixed_cols)
{
...
{
	ttoggle_button* from = cursel();
	// if no selected item existed, previous maybe nullptr.
	
	if (did_item_pre_change_ && from != nullptr) {
		if (!did_item_pre_change_(*this, *from, widget)) {
			return false;
		}
	}
	return true;
...
{
	const bool selected = widget.get_value();
	if (multi_select_) {
		if (selected) {
			// unselect --> selected
			VALIDATE(selected_items_.count(&widget) == 0, null_str);
			selected_items_.insert(&widget);
		} else {
			// selected --> unselect
			VALIDATE(selected_items_.count(&widget) != 0, null_str);
			selected_items_.erase(selected_items_.find(&widget));
		}
...
tcontrol& treport::create_child(const std::string& id, const std::string& label)
{
	...
	if (toggle_) {
		...
		widget2->set_did_state_pre_change(std::bind(&treport::did_pre_change, this, _1));
		widget2->set_did_state_changed(std::bind(&treport::did_changed, this, _1));
		widget = widget2;
...
{
	if (multi_line_ || previous_->parent() != nullptr) {
		return content_grid_->children_vsize() - front_childs_ - back_childs_;
	} else {
		VALIDATE(content_grid_->children_vsize() == 0, null_str);
		return 0;
	}
}
...
void treport::deselect_item(int at)
{
	VALIDATE(toggle_, null_str);
	const tgrid::tchild* children = content_grid()->children();
	int childs = content_grid()->children_vsize();
...
// to toggle_button's report, must exist one selected in any time.
void treport::deselect_item(twidget& widget)
{
	VALIDATE(toggle_, null_str);
	...
	ttoggle_button* widget2 = dynamic_cast<ttoggle_button*>(&widget);
	if (widget2->get_value()) {
		widget2->set_value(false);
	}
/*
	// now, I don't know how to wirte. fixed in future.
	if (boddy_) {
		boddy_->set_hole_variable(widget2->get_x(), widget2->get_x() + widget2->get_width());
	}
*/
...
void treport::clear_selected_items()
{
	while (!selected_items_.empty()) {
		deselect_item(**selected_items_.begin());
	}
}
...
void treport::tgrid2::erase_child(int at)
{
	...
	if (report_.multi_select_ && !report_.selected_items_.empty()) {
		std::set<tcontrol*>::iterator it = report_.selected_items_.find(static_cast<tcontrol*>(children_[at].widget_));
		if (it != report_.selected_items_.end()) {
			report_.selected_items_.erase(it);
		}
	}
...
void treport::tgrid2::erase_children()
{
	...
	if (report_.multi_select_) {
		report_.selected_items_.clear();
	}
