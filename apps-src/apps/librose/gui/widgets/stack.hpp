...
public:
	...
	void set_did_can_drag(const std::function<bool (tstack&, int x, int y)>& did) { did_can_drag_ = did; }
	void set_did_calculate_reserve(const std::function<void (tstack&, int& max_top_movable, int& reserve_height, int& threshold)>& did) { did_calculate_reserve_ = did; }
	void set_did_mouse_event(const std::function<void (tstack&, const tmouse_event event, const tpoint&, const tpoint&)>& did) { did_mouse_event_ = did; }
	bool upward() const { return upward_; }
	bool dragging() const { return !is_null_coordinate(first_coordinate_); }

	// pip
	void set_did_pip_top_grid_rect(const tgrid& grid, const std::function<SDL_Rect (tstack&, tgrid&, int width, int height)>& did);
	void pip_relayout();
...
private:
	...
	std::function<bool (tstack&, int x, int y)> did_can_drag_;
	std::function<void (tstack&, int& max_top_movable, int& reserve_height, int& threshold)> did_calculate_reserve_;
	std::function<void (tstack&, const tmouse_event event, const tpoint&, const tpoint&)> did_mouse_event_;

	std::map<const tgrid*, std::function<SDL_Rect (tstack&, tgrid&, int width, int height)> > did_pip_top_grid_rects_;
	std::map<const tgrid*, uint64_t> pip_width_verifier_;
};
