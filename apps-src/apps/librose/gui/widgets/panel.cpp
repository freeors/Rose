...
void tpanel::layout_children()
{
	if (need_layout_) {
		place(get_origin(), get_size());
		need_layout_ = false;

		// since place container_ again, set it dirty.
		set_dirty();
...
bool tpanel::can_invalidate_layout(const twidget& widget) const
{
	// if this is dirty, requier invalidte layout its parent.
	return can_invalidate_layout_ && &widget != this;
}

void tpanel::invalidate_layout(twidget* widget)
{
	need_layout_ = true;
}
