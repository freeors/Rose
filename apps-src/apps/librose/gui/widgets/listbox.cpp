...
ttoggle_panel& tlistbox::insert_row(const std::map<std::string, std::string>& data, int at)
{
	...
	if (at != nposm) {
		...
		if (rows != 0) {
			...
			// rows == 0 && at == 0, tune at to nposm;
			at = nposm;
		}
	}
	ttoggle_panel* widget = dynamic_cast<ttoggle_panel*>(list_builder_->widgets[0]->build());
	widget->set_did_mouse_enter_leave(std::bind(&tlistbox::did_focus_changed, this, _1, _2));
	widget->set_did_state_pre_change(std::bind(&tlistbox::did_pre_change, this, _1));
	widget->set_did_state_changed(std::bind(&tlistbox::did_changed, this, _1));
	widget->set_did_double_click(std::bind(&tlistbox::did_double_click, this, _1));
	widget->set_did_click(std::bind(&tlistbox::did_click, this, _1, _2));

	// Until here, the parent of the widget is null.
	// So don't worry about below's set_child_members call twindow::invalidate_layout.
...
void tlistbox::gc_insert_or_erase_row(const int row, const bool insert)
{
	...
	} else if (gc_next_precise_at_ > 0 && row == gc_next_precise_at_) {
...
{
public:
	tsort_func(const std::function<bool (const ttoggle_panel& widget, const ttoggle_panel&)>& did_compare)
...
private:
	std::function<bool (const ttoggle_panel& widget, const ttoggle_panel&)> did_compare_;
};

void tlistbox::sort(const std::function<bool (const ttoggle_panel& widget, const ttoggle_panel&)>& did_compare)
{
	tgrid::tchild* children = list_grid_->children();
	const int rows = list_grid_->children_vsize();
	...
	if (widget) {
		...
		if (tmp == left_drag_grid_) {
			...
			if (gc_first_at_ != nposm && at >= gc_first_at_ && at <= gc_last_at_) {
				...
				// must calculate this row's best_size.
				// 1)widget maybe not in linked_group. 2)linked_group is width, but maybe changed in height.
				const std::string& group = widget->linked_group();
				bool force_recalculate = group.empty();
				if (!force_recalculate) {
					std::map<std::string, tlinked_size>::const_iterator it = linked_size_.find(group);
				...
				if (force_recalculate) {
					panel.gc_width_ = nposm;
					panel.gc_height_ = nposm;
				}
...
int tlistbox::cookie_2_at(size_t cookie, bool validate) const
{
	int rows2 = rows();
	for (int at = 0; at < rows2; at ++) {
		const ttoggle_panel& row = row_panel(at);
		if (row.cookie_ == cookie) {
			return row.at_;
		}
	}
	VALIDATE(!validate, null_str);
	return nposm;
}
...
bool tlistbox::select_row(const int at)
{
	...
	if (multiselect_) {
		VALIDATE(desire_widget != nullptr, null_str);
		if (desire_widget->get_value()) {
			return false;
		} else {
			did_click(*desire_widget, desire_widget->get_value());
		}
		return true;
	}
...
void tlistbox::enable_multiselect(const bool enable)
{
	VALIDATE(!selectable_, null_str);
	if (enable == multiselect_) {
		return;
	}
	multiselect_ = enable;
	if (enable) {
		VALIDATE(multiselected_rows_.empty(), null_str);
	} else {
		for (std::set<int>::const_iterator it = multiselected_rows_.begin(); it != multiselected_rows_.end(); ++ it) {
			ttoggle_panel& row = row_panel(*it);
			VALIDATE(row.get_value(), null_str);
			row.set_value(false);
			VALIDATE(!row.get_value(), null_str);
		}
		multiselected_rows_.clear();
	}
}
...
bool tlistbox::select_internal(twidget* widget, const bool selected)
{
	...
	if (widget) {
		desire_panel = static_cast<ttoggle_panel*>(widget);
	...
	if (desire_panel) {
		...
		if (changed) {
			if (selectable_) {
				...
				// non-select mode and non-multiselect_
				if (selected) {
	...
	if (!multiselect_ && changed && (cursel_ || (!selectable_ && desire_panel)) && did_row_changed_) {
		// Why is it still necessary to call did_row_changed_ when selectable_ == false? 
		// --For example gui2::tmenu, once click item, requires call did_row_changed_.
		int origin_at = desire_panel->at();
		...
		bool row_struct_changed = list_grid_->children_vsize() != origin_rows;
		if (!row_struct_changed) {
			ttoggle_panel& now_panel = row_panel(origin_at);
			if (&now_panel != desire_panel || now_panel.at() != origin_at) {
				row_struct_changed = true;
			}
		}
		// if did_row_changed_ insert/erase row, call select_internal(nullptr) only is no useful, 
		// because it's parent ttoggle_panel::set_value require desire_panel is valid.
		VALIDATE(!row_struct_changed, "Must not insert/erase row when did_row_changed_");
	}
...
ttoggle_panel* tlistbox::visible_area_point_2_row(int x, int y) const
{
	VALIDATE(content_ != nullptr, null_str);
	VALIDATE(visible_ == twidget::VISIBLE, null_str);

	SDL_Rect widget_rect = content_->get_rect();
	if (!point_in_rect(x, y, widget_rect)) {
		return nullptr;
	}
	// FIX FUTURE:
	//  When perfecting garbage collection, the search algorithm here must be optimized by "Two-way method".
	const int rows = list_grid_->children_vsize();
	tgrid::tchild* children = list_grid_->children();
	...
	for (int row = 0; row < rows; row ++) {
		const tgrid::tchild& child = children[row];

		if (child.widget_->get_visible() != VISIBLE) {
			continue;
		}

		widget_rect = child.widget_->get_rect();
		if (point_in_rect(x, y, widget_rect)) {
			return static_cast<ttoggle_panel*>(child.widget_); 
		}
	}
	return nullptr;
}
...
void tlistbox::did_click(ttoggle_panel& widget, bool pre_value)
{
	if (!multiselect_) {
		return;
	}
	bool desire_value = !pre_value;
	if (widget.get_value() != desire_value) {
		widget.set_value(desire_value);
		VALIDATE(widget.get_value() == desire_value, null_str);
	}
	if (desire_value) {
		VALIDATE(multiselected_rows_.count(widget.at()) == 0, null_str);
		multiselected_rows_.insert(widget.at());
	} else {
		std::set<int>::iterator it = multiselected_rows_.find(widget.at());
		VALIDATE(it != multiselected_rows_.end(), null_str);
		multiselected_rows_.erase(it);
	}
}
...
void tlistbox::layout_init2()
{
	...
	std::set<int> get_best_size_rows;
	{
		...
		for (int row = gc_first_at_; row <= gc_last_at_; ++ row) {
			...
			} else if (widget->gc_width_ == nposm) {
				// reference to tlistbox::invalidate_layout(...)
...
void tlistbox::tgrid3::clear_text_maximum_width()
{
	VALIDATE(false, null_str);
}
...
void tlistbox::tgrid3::impl_draw_children(texture& frame_buffer, int x_offset, int y_offset)
{
	...
	for (int row = listbox_.gc_first_at_; row <= listbox_.gc_last_at_; row ++) {
		...
		if (listbox_.left_drag_grid_ && listbox_.drag_at_ == row) {
			...
			if (listbox_.left_drag_grid_->get_width() > 0 && listbox_.left_drag_grid_->get_height() > 0) {
			...
			// 1. left drag this list's row.
			// 2. popup progress-bar window (network task)
			// 3. tdialog::show will call this windows's absolute_draw().
			// ====> it may result to listbox_.left_drag_spacer_->get_width() != 0.
...
void tlistbox::tgrid3::dirty_under_rect(const SDL_Rect& clip)
{
	...
	if (listbox_.gc_first_at_ == -1) {
		// BUG! I've been in, but haven't found the reason, and need to fix it in the future.
		return;
	}
...
void tlistbox::click_left_drag_widget(const std::string& id)
{
	...
	std::map<std::string, std::function<void (tlistbox&, ttoggle_panel&)> >::const_iterator it = left_drag_widget_click_.find(id);
...
tbutton& tlistbox::set_did_left_drag_widget_click(const std::string& id, const std::function<void (tlistbox&, ttoggle_panel&)>& did)
{
	...
	std::map<std::string, std::function<void (tlistbox&, ttoggle_panel&)> >::iterator it = left_drag_widget_click_.find(id);
...
void tlistbox::mini_mouse_down(const tpoint& first)
{
	...
	if (drag_at_ == nposm && longpress_at_ == nposm) {
		...
		if (use_drag_) {
			...
			longpress_timer_.reset(100, *window, std::bind(&tlistbox::longpress_timer_handler, this));
		}
...
{ 
	...
	if (list_builder_->widgets.size() >= 2) {
		...
		left_drag_spacer_->set_definition("default", null_str, null_str);
		left_drag_spacer_->set_visible(twidget::INVISIBLE);
