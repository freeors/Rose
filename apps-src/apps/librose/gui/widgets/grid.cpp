...
{
	const tpoint current_best = from.get_size();
	VALIDATE(from.parent() == this && current_best.x > 0 && from.get_visible() != twidget::INVISIBLE, null_str);

	const tchild* from_child = nullptr;
	tpoint cell_orig(x_, y_);
	tpoint cell_size(nposm, nposm);
	for (unsigned row = 0; row < rows_; ++row) {
		for (unsigned col = 0; col < cols_; ++col) {
			const tchild& cell = child(row, col);
			if (cell.widget_ == &from) {
				from_child = &cell;
				cell_size = tpoint(col_width_[col], row_height_[row]);
				break;
			}
			cell_orig.x += col_width_[col];
		}
		if (from_child != nullptr) {
			break;
		}
		cell_orig.y += row_height_[row];
		cell_orig.x = x_;
	}

	VALIDATE(cell_size.x != nposm && cell_size.y != nposm && from_child != nullptr, null_str);

	bool consider_width = !from.mtwusc();
	bool consider_height = true;

	if (consider_width) {
		if ((from_child->flags_ & HORIZONTAL_MASK) == HORIZONTAL_ALIGN_EDGE) {
			consider_width = false;
		}
	}

	if (consider_height) {
		if ((from_child->flags_ & VERTICAL_MASK) == VERTICAL_ALIGN_EDGE) {
	...
	const tpoint cfg_size = from.best_size_from_builder();
	if (consider_width && cfg_size.x != nposm) {
		consider_width = false;
	}
	if (consider_height && cfg_size.y != nposm) {
		consider_height = false;
	}
	...
	// const tpoint border_size = cell_border_space(*from_child);
	// const tpoint new_cell_size = new_best_size + border_size;
	// const int current_cell_width = col_width_[from_at % cols_];
	// const int current_cell_height = row_height_[from_at / cols_];

	// maybe exit width equal to height font, so calulcate width's jitter use font size.
	const int max_jitter = from.get_text_font_size() * 1 / 3;
	const int hjitter = new_best_size.x - current_best.x;
	if (consider_width) {
		// cannot use new_cell_size.x > current_cell_width,
		// if isn't edge align, w_ is shoten, more will result tow line.
		if (new_best_size.x > current_best.x || posix_abs(hjitter) > max_jitter) {
			return true;
		}
	}

	const int vjitter = new_best_size.y - current_best.y;
	// same as width reason, new_cell_size.y > current_cell_height.
	if (consider_height) {
		if (new_best_size.y > current_best.y || posix_abs(vjitter) > max_jitter) {
			return true;
		}
	}

	if (from.mtwusc() && (new_best_size.x > current_best.x || posix_abs(hjitter) > max_jitter)) {
		// post mtwusc. don't trigger invalid window, only re-place it.
		if (current_best.x > new_best_size.x) {
			from_child->widget_->set_dirty_width(current_best.x); 
		}
		const uint32_t hflags = from_child->flags_ & HORIZONTAL_MASK;
		// if (hflags == HORIZONTAL_ALIGN_CENTER || hflags == HORIZONTAL_ALIGN_RIGHT) {
			cell_place(*from_child, cell_orig, cell_size);
		// }
	}
...
void tgrid::clear_text_maximum_width()
{
	// Inherited.
	twidget::clear_text_maximum_width();
	...
	for (int n = 0; n < children_vsize_; n ++) {
		...
		if (child.widget_->get_visible() != twidget::INVISIBLE) {
			child.widget_->clear_text_maximum_width();
		}
	}
}
...
static std::unique_ptr<bool> calculate_col_grow_visible(const tgrid::tchild* children, int children_vsize, const std::vector<unsigned>& col_grow_factor, unsigned& w_size)
{
	const int cols = col_grow_factor.size();
	std::unique_ptr<bool> col_grow_visible(new bool[cols]);
	bool* ptr = col_grow_visible.get();
	memset(ptr, 0, cols * sizeof(bool));
	...
	for (int at = 0; at < children_vsize; ) {
		...
		at ++;
		if (at % cols == 0) {
			// there maybe many rows in this grid.
...
static std::unique_ptr<bool> calculate_row_grow_visible(const tgrid::tchild* children, int children_vsize, const std::vector<unsigned>& row_grow_factor, unsigned& h_size, int cols)
{
	const int rows = row_grow_factor.size();
	std::unique_ptr<bool> row_grow_visible(new bool[rows]);
	bool* ptr = row_grow_visible.get();
	memset(ptr, 0, rows * sizeof(bool));
	...
	for (int at = 0; at < children_vsize; ) {
		...
		ptr[row] = true;
		h_size += row_grow_factor[row];
		at = (row + 1) * cols;
	}

	return row_grow_visible;
}
...
void tgrid::place(const tpoint& origin, const tpoint& size)
{
	...
	if (best_size.x <= size.x && best_size.y <= size.y) {
		...
		if (size.x > best_size.x) {
			// one example of restrict_width_factor
			// -----
			// [rows_, cols]: [7, 1], col at = 5 is mwusc, and col_grow_factor_[5] is 1 ==> 
			// restrict_width_factor: (5, 1)
...
/*
							...
							col_width_[at % cols_] = new_best_width;
*/
			...
			// both w and w_size is size that beside restrict widget.	
			const unsigned w = size.x - best_size.x;
			unsigned w_size = 0;
			// make sure invisible's col hasn't extra width.
			std::unique_ptr<bool> col_grow_visible = calculate_col_grow_visible(children_, children_vsize_, col_grow_factor_, w_size);
			const bool* col_grow_visible_ptr = col_grow_visible.get();

			// use temp col_grow_factor, don't touch col_grow_factor_. dirty col_grow_factor_ maybe result chaos.
			std::vector<unsigned> col_grow_factor = col_grow_factor_;

			if (restrict_width_factor.first != nposm) {
				// restrict widget's factor must be > 0.
				w_size -= restrict_width_factor.second;
				// if w_size is 0, indicate all expand size is belone to restrict widget.
				if (w_size == 0) {
					VALIDATE(w == 0, null_str);
				}
				...
				if (w_size == 0) {
					// If all sizes are 0 reset them to 1
					int i = 0;
					BOOST_FOREACH(unsigned& val, col_grow_factor) {
						if (col_grow_visible_ptr[i]) {
							val = 1;
							w_size ++;
						}
						i ++;
			...
			if (w_size != 0) {
				...
				for (unsigned i = 0; i < cols_; ++i) {
					if (restrict_width_factor.first != nposm && (restrict_width_factor.first % cols_) == i) {
						continue;
					}
					if (col_grow_visible_ptr[i]) {
						col_width_[i] += w_normal * col_grow_factor[i];
					}
		...
		if (size.y > best_size.y) {
			...
			unsigned h_size = 0;
			std::unique_ptr<bool> row_grow_visible = calculate_row_grow_visible(children_, children_vsize_, row_grow_factor_, h_size, cols_);
			const bool* row_grow_visible_ptr = row_grow_visible.get();

			// use temp row_grow_factor, don't touch row_grow_factor_. dirty row_grow_factor_ maybe result chaos.
			std::vector<unsigned> row_grow_factor = row_grow_factor_;
			// extra height(h) will be divided amount(h_size) units in (rows_) rows.
			if (h_size == 0) {
				// If all sizes are 0 reset them to 1
				int i = 0;
				BOOST_FOREACH(unsigned& val, row_grow_factor) {
					if (row_grow_visible_ptr[i]) {
						val = 1;
						h_size ++;
					}
					i ++;
			...
			if (h_size != 0) {
				...
				for (unsigned i = 0; i < rows_; ++i) {
					if (row_grow_visible_ptr[i]) {
						row_height_[i] += h_normal * row_grow_factor[i];
	...
	symbols["id"] = ht::generate_format(tmp.str(), 0xffff0000);
	tmp.str("");
	if (!parent()->id().empty()) {
		tmp << parent()->id();
	} else {
...
void cell_place(const tgrid::tchild& cell, tpoint origin, tpoint size)
{
	if (cell.widget_->get_visible() == twidget::INVISIBLE) {
		return;
	}
