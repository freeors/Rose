...
public:
	...
	private:
		...
		void clear_text_maximum_width() override;
		...
		twidget* find(const std::string& id, const bool must_be_active) override { return nullptr; }

		/** Inherited from tcontrol.*/
		const twidget* find(const std::string& id, const bool must_be_active) const override { return nullptr; }
	...
	void enable_multiselect(const bool enable);
	...
	ttoggle_panel* visible_area_point_2_row(int x, int y) const;
	int cookie_2_at(size_t cookie, bool validate) const;
	...
	/** Sort all items. */
	void sort(const std::function<bool (const ttoggle_panel&, const ttoggle_panel&)>& did_compare);
	...
	void set_did_row_focus_changed(const std::function<void (tlistbox&, ttoggle_panel&, const bool)>& did)
	{
		did_row_focus_changed_ = did;
	}
	void set_did_row_pre_change(const std::function<bool (tlistbox&, ttoggle_panel&)>& did)
	{
		did_row_pre_change_ = did;
	}
	void set_did_row_changed(const std::function<void (tlistbox&, ttoggle_panel&)>& did)
	{
		did_row_changed_ = did;
	}
	void set_did_row_double_click(const std::function<void (tlistbox&, ttoggle_panel&)>& did)
	{
		did_row_double_click_ = did;
	}
	void set_did_row_right_click(const std::function<void (tlistbox&, ttoggle_panel&)>& did)
	{
		did_row_right_click_ = did;
	}

	void set_did_allocated_gc(const std::function<void (tlistbox&, ttoggle_panel&)>& did)
	{
		did_allocated_gc_ = did;
	}

	void set_did_more_rows_gc(const std::function<void (tlistbox&)>& did)
	{
		did_more_rows_gc_ = did;
	}

	void set_did_longpress_menu(const std::function<void (tlistbox&, ttoggle_panel&, std::vector<tfloat_widget*>&)>& did)
	{
		did_longpress_memu_ = did;
	}

	void set_did_can_drag(const std::function<bool (tlistbox&, ttoggle_panel&)>& did)
	{
		did_can_drag_ = did;
	}
	...
	tbutton& set_did_left_drag_widget_click(const std::string& id, const std::function<void (tlistbox&, ttoggle_panel&)>& did);

	// in general, used in longpress menu's click handler.
	int last_longpress_at() const { return last_longpress_at_; }

	const std::set<int>& multiselected_rows() const { return multiselected_rows_; }
...
protected:
	...
	void did_click(ttoggle_panel& widget, bool pre_value);
...
private:
	...
	bool multiselect_;
	std::set<int> multiselected_rows_;
	...
	std::map<std::string, std::function<void (tlistbox&, ttoggle_panel&)> > left_drag_widget_click_;
	...
	std::function<void (tlistbox&, ttoggle_panel&, const bool)> did_row_focus_changed_;
	std::function<bool (tlistbox&, ttoggle_panel&)> did_row_pre_change_;
	std::function<void (tlistbox&, ttoggle_panel&)> did_row_changed_;
	std::function<void (tlistbox&, ttoggle_panel&)> did_row_double_click_;
	std::function<void (tlistbox&, ttoggle_panel&)> did_row_right_click_;

	std::function<void (tlistbox&, ttoggle_panel&, std::vector<tfloat_widget*>&)> did_longpress_memu_;
	std::vector<tfloat_widget*> longpress_menu_;

	std::function<bool (tlistbox&, ttoggle_panel&)> did_can_drag_;

	std::function<void (tlistbox&, ttoggle_panel&)> did_allocated_gc_;
	std::function<void (tlistbox&)> did_more_rows_gc_;
