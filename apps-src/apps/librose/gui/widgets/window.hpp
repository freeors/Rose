...
class twindow
	...
	, public ttransition
{
	friend twindow* build(tdialog&, CVideo&, const twindow_builder::tresolution*, const unsigned, const unsigned);
...
public:
	...
	static const std::string menu_dlg_id;
	 ...
	 * return value:
	 *  false: throw exception during draw(), caller require halt draw process.
	 *  true: normal return.
	 */
	bool draw();
	...
	bool can_invalidate_layout(const twidget& widget) const override { return true; }
	/**
	...
	void start_drag(const tpoint& coordinate, const std::function<bool (const int, const int)>& did_mouse_motion, 
		const std::function<void (const int, const int, bool)>& did_mouse_leave);
	void stop_drag();
	bool is_drag_surface_visible() const { return float_drag_->widget->get_visible() == twidget::VISIBLE; }
	void set_drag_surface_visible(bool visible);
	bool draging() const { return draging_; }
	const SDL_Surface* curr_drag_surf() const { return curr_drag_surf_; }
	...
	void set_click_dismiss_except(const std::function<bool (const int x, const int y)>& callback)
	{
		did_click_dismiss_except_ = callback;
	}

	void set_dismiss_outof_window(const bool value)
	{
		dismiss_outof_window_ = value;
	}

	void set_leave_dismiss(const bool leave_dismiss, const std::function<bool (const int x, const int y)>& callback = NULL)
	{
	...
	void undraw_float_widgets(const tfloat_widget* desired = nullptr);
	...
	tfloat_widget& float_drag() const { return *float_drag_; }
	tfloat_widget& float_track() const { return *float_track_; }
	tfloat_widget& float_button() const { return *float_button_.widget; }

	void reset_scrollbar(tfloat_widget& vertical_scrollbar);
	void clear_texture() override;
	void dirty_under_rect(const SDL_Rect& clip) override;

	void register_tpl_widget(const twidget& widget, tbase_tpl_widget* tpl_widget);


	bool is_scene() const { return scene_; }
	class tunique_event_widget_lock
	{
	public:
		tunique_event_widget_lock(twindow& window, twidget& widget)
			: window_(window)
		{
			VALIDATE(window.unique_event_widget_ == nullptr && widget.float_widget(), null_str);
			window.unique_event_widget_ = &widget;

			VALIDATE(!events::progress_running, null_str);
			events::progress_running = true;
		}
		~tunique_event_widget_lock()
		{
			VALIDATE(window_.unique_event_widget_ != nullptr, null_str);
			window_.unique_event_widget_ = nullptr;
			events::progress_running = false;
		}

	private:
		twindow& window_;
	};
	twidget* unique_event_widget() const { return unique_event_widget_; }
	...
	void set_did_click_edit_button(const std::function<void (twindow&, const int)>& did)
	{
		did_click_edit_button_ = did;
	}
	void set_float_button(const std::string& label, const std::string& icon, int font_size, int color_tpl, const tpoint& size, const std::function<void (twindow&)>& did_click, bool visible);
	
	void pump_timers() const;
	void set_trigger_quiet_mouse_leave(bool value) { trigger_quiet_mouse_leave_ = value; }
	bool trigger_quiet_mouse_leave() const { return trigger_quiet_mouse_leave_; }
...
private:
	...
	struct tfloat_button {
		tfloat_button(const std::string& prefix)
			: prefix(prefix)
			, widget(nullptr)
		{};
		const std::string prefix;
		tfloat_widget* widget;
		std::function<void (twindow&)> did_click;
	};

	// float button
	void click_float_button(tfloat_button& button);
	void signal_handler_longpress_float_button(tfloat_button& button2, bool& halt, const tpoint& coordinate);
	bool did_drag_float_button_motion(tfloat_button& button2, const int x, const int y);
	void did_drag_float_button_leave(tfloat_button& button2, const int x, const int y, bool up_result);

	// float edit button
	void click_edit_button(const int id);
	void remove_timer_during_destructor();

	void release_tpl_widgets();
...
private:
	...
	bool draging_;
	const SDL_Surface* curr_drag_surf_;
	std::function<bool (const int x, const int y)> did_drag_mouse_motion_;
	std::function<void (const int x, const int y, bool)> did_drag_mouse_leave_;
	 ...
	 */
	bool click_dismiss_;
	std::function<bool (const int x, const int y)> did_click_dismiss_except_;
	bool dismiss_outof_window_;

	/**
	 * once mouse leave this window, close it.
	 */
	bool leave_dismiss_;
	std::function<bool (const int x, const int y)> did_leave_dismiss_;
	...
	bool trigger_quiet_mouse_leave_;
	...
	{
	public:
		tdrawing_lock(twindow& window)
			: window_(window)
		{
			VALIDATE(!window.drawing_, null_str);

			window.drawing_ = true;
		}
		~tdrawing_lock()
		{
			VALIDATE(window_.drawing_, null_str);
			window_.drawing_ = false;
		}
	...
	int tooltip_at_;
	tfloat_widget* float_drag_;
	tfloat_widget* float_track_;

	tfloat_button float_button_;
	twidget* keyboard_widget_;

	bool scene_;
	twidget* unique_event_widget_;

	std::function<void (twindow&, const int)> did_click_edit_button_;
	std::map<const twidget*, tbase_tpl_widget*> tpl_widgets_;
...
public:
	...
	void show_keyboard(int reason);
	void hide_keyboard(int reason);
	void draw_keyboard(bool need_layout);
	...
	bool signal_handler_sdl_key_down_pre(const SDL_Keycode key);
private:
