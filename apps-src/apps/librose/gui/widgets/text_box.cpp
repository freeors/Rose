...
ttext_box::ttext_box(bool multi_line, bool desensitize, bool atom_markup)
	: tcontrol(COUNT)
	, edit_box_param_(null_str, null_str, null_str, null_str, null_str, null_str, null_str, nposm)
	, atom_markup_(atom_markup)
	, multi_line_(multi_line)
	, desensitize_(desensitize)
	, cipher_(desensitize)
	, password_char_(UCS2_to_UTF8(0x25cf))
	, state_(ENABLED)
	, selection_start_(::create_rect(0, 0, 0, 0))
	, selection_end_(::create_rect(nposm, nposm, 0, 0))
	, maximum_length_(nposm)
	...
	, xpos_(0)
	, cursor_bytes_pos_(nposm)
{
	VALIDATE(!desensitize_ || !multi_line_, null_str);
	...
	connect_signal<event::SDL_TEXT_INPUT>(std::bind(
	...
	connect_signal<event::LOSE_KEYBOARD_FOCUS>(std::bind(
			&ttext_box::signal_handler_lose_keyboard_focus, this, _2, _6));
	...
	connect_signal<event::LEFT_BUTTON_DOUBLE_CLICK>(std::bind(&ttext_box
				::signal_handler_left_button_double_click, this, _2, _4));

	connect_signal<event::LEFT_BUTTON_CLICK>(std::bind(
				&ttext_box::signal_handler_left_button_click, this, _2, _3, _4));

	connect_signal<event::RIGHT_BUTTON_UP>(std::bind(&ttext_box::signal_handler_right_button_click, this, _4, _5));
...
{
	VALIDATE(desensitize_, null_str);
...
void ttext_box::set_cipher(const bool cipher)
{
	...
	std::string before;
	if (selection_start_.x != nposm && integrate_ != nullptr) {
...
void ttext_box::show_edit_button(const bool show)
{
	...
	if (show) {
		...
		window->set_did_click_edit_button(std::bind(&ttext_box::did_edit_click, this, _1, _2));
		...
		window->set_did_click_edit_button(NULL);
...
void ttext_box::insert_str(const std::string& str)
{
	...
	if (maximum_length_ != nposm && integrate_->at_end(selection_start_.x, selection_start_.y)) {
	...
	std::unique_ptr<tcursor_bytes_pos_lock> cursor_lock;
	std::string new_text;
	SDL_Rect new_start;
	if (cipher_) {
		SDL_Rect start, end;
		...
		const int insert_chars = utils::utf8str_len(str2);
		const std::string new_desensitize_str = to_password_str(new_text);
		str2 = utils::utf8str_substr(new_desensitize_str, insert_at, insert_chars);
/*
		// new content of before's count chars maybe changed because this insert.
		// see delete_char or delete_selection.
		const std::string new_desensitize_str = to_password_str(new_text);
		cursor_lock.reset(new tcursor_bytes_pos_lock(*this, utils::utf8str_bytes(new_desensitize_str, insert_at + insert_chars)));
*/
	}
...
void ttext_box::calculate_integrate(const int maximum_width)
{
	...
	if (maximum_width > 0) {
		...
		if (label_.empty()) {
			...
			integrate_label = to_password_str(label_);
	...
	if (integrate_) {
		if (src_pos_ == -1) {
			if (cursor_bytes_pos_ == nposm) {
				selection_start_ = integrate_->editable_at(selection_start_.x, selection_start_.y);
			} else {
				selection_start_ = integrate_->calculate_cursor(cursor_bytes_pos_);
			}
		} else {
			selection_start_ = integrate_->calculate_cursor(src_pos_);
			src_pos_ = -1;
...
void ttext_box::set_state(const tstate state)
{
	...
	if (!cursor_timer_.valid() && state == FOCUSSED) {
		cursor_timer_.reset(500, *get_window(), std::bind(&ttext_box::cursor_timer_handler, this));
	}
...
bool ttext_box::handle_key_left_arrow(SDL_Keymod modifier)
{
...
bool ttext_box::handle_key_right_arrow(SDL_Keymod modifier)
{
	/** @todo implement the ctrl key. */
...
{
	if (key == '\r') {
		// all printable keycode(ascii, chinses, etc) are handled by signal_handler_sdl_text_input.
		handled = true;
...
// widgets/textbox.cpp:516. Would be nice if somebody on a MAC would test it.
	...
	switch(key) {
		...
		case SDLK_LEFT :
			handled = true;
			handle_key_left_arrow(modifier);
			break;

		case SDLK_RIGHT :
			handled = true;
			handle_key_right_arrow(modifier);
			break;
...
void ttext_box::signal_handler_lose_keyboard_focus(const event::tevent event, twidget* extra_widget)
{
	set_state(ENABLED);
	...
	// extra_widiget is the widget that will has keyboard focus.
	// if (!keyboard::is_visible() && (extra_widget == nullptr || !extra_widget->is_text_box())) {
	// it is impossible that use extra_widget is nullptr or not. 
	// why? click one button, it send extra_widget = nullptr always.
	if (extra_widget == nullptr || !extra_widget->is_text_box()) {
		keyboard::set_visible(false);
	}
}
...
std::string ttext_box::to_password_str(const std::string& str) const
{
	VALIDATE(cipher_, null_str);
	// str maybe not label_.
	if (str.empty()) {
		return str;
	}
	std::string ret;
	const int chars = utils::utf8str_len(str);
	if (did_desensitize_str_ != NULL) {
		ret = did_desensitize_str_(*this, str);
		VALIDATE(utils::utf8str_len(ret) == chars, null_str);
...
void ttext_box::delete_char(const bool backspace)
{
	...
	if (cipher_) {
		...
		std::string before_cursor_cipher = before;
		...
		if (backspace) {
			...
			delete_at --;
			before_cursor_cipher = utils::utf8str_erase(before_cursor_cipher, delete_at, 1);
		} else if (delete_at == utils::utf8str_len(label_)) {
			return;
		}
		new_text = utils::utf8str_erase(label_, delete_at, 1);
		VALIDATE(new_text != label_, null_str);

		// new content of before's count chars maybe changed because this delete.
		// 123***789 + delete 9 --> 123**678. result 123**678 isn't orignal's 123***78
		const std::string new_desensitize_str = to_password_str(new_text);
		cursor_lock.reset(new tcursor_bytes_pos_lock(*this, utils::utf8str_bytes(new_desensitize_str, delete_at)));
...
void ttext_box::signal_handler_left_button_click(const event::tevent event, bool& handled, bool& halt)
{
	bool popup_editbox = false;

	if (tedit_box::instance == nullptr && tprogress_::instance == nullptr && edit_box_param_.valid()) {
		twindow* window = get_window();
		// if (game_config::mobile && y_ > window->get_height() / 3) {
		if (y_ > window->get_height() / 3) {
			VALIDATE(edit_box_param_.ok.empty(), null_str);
			if (edit_box_param_.placeholder.empty()) {
				edit_box_param_.placeholder = placeholder_;
			}
			edit_box_param_.initial = label_;
			edit_box_param_.max_chars = maximum_length_;

			{
				gui2::tedit_box dlg(edit_box_param_);
				dlg.show(nposm, window->get_height() / 4);
			}
			set_label(edit_box_param_.result);
			halt = true;
