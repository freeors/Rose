...
bool tstack::can_invalidate_layout(const twidget& widget) const
{
	// in invalidate_layout, get_best_size and calculate_best_size_bh is Time-consuming, not don't use.
	return false;

	// if this is dirty, requier invalidte layout its parent.
	// return mode_ == radio && &widget != this;
}

void tstack::invalidate_layout(twidget* widget)
{
	VALIDATE(mode_ == radio && widget != nullptr, null_str);
	...
	VALIDATE(tmp != nullptr, null_str);

	tgrid* desire_visible_grid = static_cast<tgrid*>(tmp);
	// below logic likes set_radio_layer
...
{ 
	...
	if (mode_ == pip) {
		// from topest grid to lowerest.
		for (int n = childs - 1; n >= 0; n --) {
			tgrid* grid = dynamic_cast<tgrid*>(children[n].widget_);
			if (grid->get_visible() != twidget::VISIBLE) {
				continue;
			}
			const SDL_Rect rect = grid->get_rect();
...
tpoint tstack::tgrid2::calculate_best_size() const
{
	...
	if (stack_.mode_ == pip || stack_.mode_ == vertical) {
		...
		for (int i = 0; i < childs; ++ i) {
			const tgrid* grid = dynamic_cast<tgrid*>(children_[i].widget_);
			// must caluclate only get_visible() != INVISIBLE layer.
			// why reference to stack section in "Rose-Programming-Guide.docx".
			if (grid->get_visible() == INVISIBLE) {
				continue;
			}
			const tpoint best_size = grid->get_best_size();
...
tpoint tstack::tgrid2::calculate_best_size_bh(const int width)
{
	...
	if (stack_.mode_ == pip) {
		for (int i = 0; i < childs; ++ i) {
			...
			int width2 = width;
			if (stack_.did_pip_top_grid_rects_.count(grid) != 0) {
				SDL_Rect rect = stack_.did_pip_top_grid_rects_.find(grid)->second(stack_, *grid, width, nposm);
				width2 = rect.w;

				const uint64_t value = posix_mku64(width2, width);
				if (stack_.pip_width_verifier_.count(grid) != 0) {
					stack_.pip_width_verifier_[grid] = value;
				} else {
					stack_.pip_width_verifier_.insert(std::make_pair(grid, value));
				}
			}

			const tpoint best_size = grid->calculate_best_size_bh(width2);
			if (best_size.x > result.x) {
				result.x = best_size.x;
			}
...
void tstack::tgrid2::place(const tpoint& origin, const tpoint& size)
{
	...
	if (stack_.mode_ == pip) {
		...
		for (int i = 0; i < childs; ++ i) {
			...
			tpoint size2 = size;
			if (stack_.did_pip_top_grid_rects_.count(grid) != 0) {
				SDL_Rect rc = stack_.did_pip_top_grid_rects_.find(grid)->second(stack_, *grid, size.x, size.y);
				origin2 = origin + tpoint(rc.x, rc.y);
				size2 = tpoint(rc.w, rc.h);
				VALIDATE(size2.y >= std::accumulate(row_height_.begin(), row_height_.end(), 0), null_str);

				std::map<const tgrid*, uint64_t>::const_iterator verifier_it = stack_.pip_width_verifier_.find(grid);
				const uint64_t value = posix_mku64(size2.x, size.x);
				VALIDATE(verifier_it->second == value, null_str);
			}
...
void tstack::set_radio_layer(int layer)
{
	...
	twindow* window = get_window();
	std::unique_ptr<tdisable_invalidate_layout_lock> lock(new tdisable_invalidate_layout_lock);

	tgrid* desire_visible_grid = nullptr;
	for (int n = 0; n < childs; ++ n) {
		tgrid& grid = *(dynamic_cast<tgrid*>(children[n].widget_));
	...
	if (desire_visible_grid != nullptr) {
		...
		if (!widget_in_keyboard(*this)) {
			// if keyboard focus is in previous grid, change this grid force. 
			window->keyboard_capture(desire_visible_grid);
		}
	}
}

void tstack::set_did_pip_top_grid_rect(const tgrid& grid, const std::function<SDL_Rect (tstack&, tgrid&, int width, int height)>& did)
{
	VALIDATE(mode_ == pip && grid_.children_vsize() > 1, null_str);
	VALIDATE(&grid != layer(0), null_str);

	std::map<const tgrid*, std::function<SDL_Rect (tstack&, tgrid&, int width, int height)> >::iterator it = did_pip_top_grid_rects_.find(&grid);
	if (did != NULL) {
		VALIDATE(it == did_pip_top_grid_rects_.end(), null_str);
		did_pip_top_grid_rects_.insert(std::make_pair(&grid, did));
	} else {
		VALIDATE(it != did_pip_top_grid_rects_.end(), null_str);
		did_pip_top_grid_rects_.erase(it);
	}
}

void tstack::pip_relayout()
{
	VALIDATE(mode_ == pip && grid_.children_vsize() > 1, null_str);
	// Now for simplicity, let's re-layout entire window. 
	// In the future, it should be limited to re-layout tstack itself
	twindow* window = get_window();
	if (!window->layouted()) {
		return;
	}
