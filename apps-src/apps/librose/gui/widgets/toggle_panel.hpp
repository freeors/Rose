...
public:
	...
	/** Inherited from tselectable_. */
	void set_did_mouse_enter_leave(const std::function<void (ttoggle_panel&, const bool)>& did)
	{
		did_mouse_enter_leave_ = did; 
	}
	void set_did_state_pre_change(const std::function<bool (ttoggle_panel&)>& did)
	{
		did_state_pre_change_ = did; 
	}
	void set_did_state_changed(const std::function<void (ttoggle_panel&)>& did)
	{ 
		did_state_changed_ = did; 
	}
	void set_did_click(const std::function<void(ttoggle_panel&, bool)>& did)
	{
		did_click_ = did;
	}
	void set_did_double_click(const std::function<void (ttoggle_panel&)>& did)
	{ 
		did_double_click_ = did; 
	}
	void set_did_right_button_up(const std::function<void (ttoggle_panel&)>& did)
	{ 
...
private:
	...
	std::function<void (ttoggle_panel&, const bool)> did_mouse_enter_leave_;

	/** See tselectable_::set_did_state_change. */
	std::function<bool (ttoggle_panel&)> did_state_pre_change_;

	/** See tselectable_::set_did_state_changed. */
	std::function<void (ttoggle_panel&)> did_state_changed_;

	std::function<void(ttoggle_panel&, bool)> did_click_;

	/** Mouse left double click callback */
	std::function<void (ttoggle_panel&)> did_double_click_;

	std::function<void (ttoggle_panel&)> did_right_button_up_;
...
protected:
	...
	virtual void signal_handler_right_button_up(bool& handled, bool& halt, const tpoint& coordinate);
};
