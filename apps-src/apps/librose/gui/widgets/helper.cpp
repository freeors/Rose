...
void get_screen_size_variables(game_logic::map_formula_callable& variable)
{
	...
	int corner_height = 0;
	if (!game_config::fullscreen && (game_config::os == os_android || game_config::os == os_ios)) {
		statusbar_height = game_config::statusbar_height / twidget::hdpi_scale;
		corner_height = game_config::corner_height / twidget::hdpi_scale;
	}
	variable.add("statusbar_height", variant(statusbar_height));
	variable.add("corner_height", variant(corner_height));
}
