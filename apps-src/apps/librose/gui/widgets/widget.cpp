...
bool twidget::disalbe_popup_window = false;
bool twidget::disable_lua_unlimited_mem = false;
std::string twidget::tdisable_popup_window_lock::err_msg;
...
bool twidget::should_conside_orientation(const int width, const int height)
{
	...
	return game_config::mobile || width <= max_effectable_pixel || height <= max_effectable_pixel;
}
...
{
	if (dirty_w_ > w_ && drawing_action_ == DRAWN) {
		return create_rect(get_origin(), tpoint(dirty_w_, h_)); 
	}
...
void twidget::trigger_invalidate_layout(twindow* window)
{
	...
	if (!disalbe_invalidate_layout && !float_widget_) {
		if (window && window->layouted()) {
			...
			twidget* tmp = this;
			while (tmp) {
				if (tmp->can_invalidate_layout(*this)) {
...
bool twidget::me_or_parent_has_invisible(const twidget& widget) const
{
	if (widget.visible_ == twidget::INVISIBLE) {
		return true;
	}

	const twidget* result = &widget;
	while (result->parent_) {
		result = result->parent_;
		if (result->visible_ == twidget::INVISIBLE) {
			return true;
		}
	}
	return false;
...
bool twidget::me_or_parent_is_widget(const twidget& widget) const
{
	if (this == &widget) {
		return true;
	}
