...
public:
	struct tpopup_window_exception 
	{
		tpopup_window_exception(const std::string& window_id, const std::string& type_id, const std::string& widget_id)
			: window_id(window_id)
			, type_id(type_id)
			, widget_id(widget_id)
		{}
	...
	class tdisable_popup_window_lock
	{
	public:
		tdisable_popup_window_lock(const std::string& _err_msg)
			: original(disalbe_popup_window)
		{
			err_msg = _err_msg;
			// VALIDATE(!disalbe_popup_window, null_str);
			disalbe_popup_window = true;
		}
		~tdisable_popup_window_lock()
		{
			disalbe_popup_window = original;
		}

		static std::string err_msg;
		const bool original;
	};
	static bool disalbe_popup_window;

	class tdisable_lua_unlimited_mem_lock
	{
	public:
		tdisable_lua_unlimited_mem_lock()
			: original(disable_lua_unlimited_mem)
		{
			disable_lua_unlimited_mem = true;
		}
		~tdisable_lua_unlimited_mem_lock()
		{
			disable_lua_unlimited_mem = original;
		}
		const bool original;
	};
	static bool disable_lua_unlimited_mem;

	class twidget_exist_validator
	{
	public:
		twidget_exist_validator(twidget& widget)
			: widget_(widget)
		{
			VALIDATE(widget_.magic_ == 0x3141592, null_str);
		}
		~twidget_exist_validator()
		{
			VALIDATE(widget_.magic_ == 0x3141592, null_str);
		}
	...
	virtual void clear_text_maximum_width() {}
	...
	bool me_or_parent_has_invisible(const twidget& widget) const;

	bool me_or_parent_is_widget(const twidget& widget) const;
...
public:
	...
	void set_cookie(uint64_t cookie) { cookie_ = cookie; }
	...
	virtual void set_mtwusc() { mtwusc_ = true; }
	bool mtwusc() const { return mtwusc_; }
	void set_dirty_width(int w) { dirty_w_ = w; }
	...
	virtual bool is_grid() const { return false; }

	bool allow_handle_event() const { return allow_handle_event_; }
	void disable_allow_handle_event() { allow_handle_event_ = false; }

	virtual bool can_invalidate_layout(const twidget& widget) const { return false; }
	...
	const std::string& linked_group() const { return linked_group_; }
		...
		struct tregister_helper {                                          \
			...
			{                                                              \
				register_widget(#id, std::bind(                          \
						...
						, key));                                           \
                                                                           \
				register_builder_widget(#id, std::bind(                  \
