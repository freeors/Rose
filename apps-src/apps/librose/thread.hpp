...
class twebrtc_send_helper
{
public:
	struct tlock {
	public:
		tlock(twebrtc_send_helper& helper)
			: helper_(helper)
			, th_can_(!helper.deconstructed_)
		{
			if (th_can_) {
				helper.senders_ ++;
			}
		...
		// maybe deconstructed_ = true when execute sub-thread construct tlock.
		bool can_send() const { return th_can_ && !helper_.deconstructed_; }

	private:
		twebrtc_send_helper& helper_;
		// th is abbreviation of top half. bh is construct tlock instance in sub-thread context.
		const bool th_can_;
	};

	twebrtc_send_helper(rtc::MessageHandler* phandler)
		: phandler_(phandler)
		, senders_(0)
	...
	void reset_deconstructed() { deconstructed_ = false; }
	bool deconstructed() const { return deconstructed_; }

private:
	rtc::MessageHandler* phandler_;
	int senders_;
	bool deconstructed_;
};
