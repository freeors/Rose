...
void display::redraw_everything()
{
	...
	BOOST_FOREACH (std::function<void(display&)> f, redraw_observers_) {
		f(*this);
	}
}
...
void display::erase_area_anim(int id)
{
	...
	find->second->invalidate(frameTexture_width(), frameTexture_height());
	delete find->second;
...
// @size: valid size, don't include outline.
void draw_bar_to_renderer(SDL_Renderer* renderer, const std::string& image, int x, int y, int size, double filled, const SDL_Color& col, fixed_t alpha, bool vtl)
{
	filled = std::min<double>(std::max<double>(filled, 0.0), 1.0);
	...
	if (vtl) {
		...
		dst_clip.w = top.w;
		dst_clip.h = top.h;
		render_surface(renderer, surf, &top, &dst_clip);

		dst_clip.y += top.h;
		dst_clip.w = bot.w;
		dst_clip.h = bot.h;
		render_surface(renderer, surf, &bot, &dst_clip);
	} else {
		top.w = bar_loc.x;
		top.h = surf->h;
		bot.x = bar_loc.x + skip_rows;
	...
	if (unfilled < size && alpha >= ftofxp(0.3)) {
		...
		if (vtl) {
			dst_clip.x = x + bar_loc.x;
			dst_clip.y = y + bar_loc.y + unfilled;
			dst_clip.w = filled_surf->w;
			dst_clip.h = filled_surf->h;
			render_surface(renderer, filled_surf, NULL, &dst_clip);
		} else {
			dst_clip.x = x + bar_loc.x;
			dst_clip.y = y + bar_loc.y;
			// 1. right color
